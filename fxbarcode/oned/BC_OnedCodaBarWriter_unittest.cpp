...
// 2 wide and 5 narrow modules per char. 1 space between chars.
const int MODULES_PER_CHAR = 2 * 2 + 5 * 1 + 1;
...
TEST(OnedCodaBarWriterTest, Encode) {
  ...
  EXPECT_EQ(1, height);
  EXPECT_EQ(MODULES_FOR_DELIMITERS + 3 * MODULES_PER_CHAR, width);
  expected =
      "# ##  #  # "  // A Start
      "# # ##  # "   // 1
      "# #  # ## "   // 2
