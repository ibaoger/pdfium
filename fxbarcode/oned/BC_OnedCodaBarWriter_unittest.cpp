...
TEST(OnedCodaBarWriterTest, Encode) {
  ...
  EXPECT_EQ(1, height);
  EXPECT_EQ(MODULES_FOR_DELIMITERS, width);
  expected =
      "# ##  #  # "  // A Start
      "#  #  # ##";  // B End
  ...
  EXPECT_EQ(1, height);
  EXPECT_EQ(MODULES_FOR_DELIMITERS + 3 * MODULES_PER_NUMBER, width);
  expected =
      "# ##  #  # "  // A Start
      "# # ##  # "   // 1
      "# #  # ## "   // 2
  ...
  encoded = writer.Encode("-$./:+", BCFORMAT_CODABAR, width, height);
  EXPECT_EQ(1, height);
  EXPECT_EQ(MODULES_FOR_DELIMITERS + 2 * MODULES_PER_NUMBER +
                4 * MODULES_PER_PUNCTUATION,
            width);
  expected =
      "# ##  #  # "  // A Start
      "# #  ## # "   // -
  ...
  EXPECT_EQ(1, height);
  EXPECT_EQ(MODULES_FOR_DELIMITERS + 15 * MODULES_PER_NUMBER +
                2 * MODULES_PER_PUNCTUATION,
            width);
  expected =
      "# ##  #  # "  // A Start
      "# ## #  # "   // 4
...
TEST(OnedCodaBarWriterTest, SetDelimiters) {
  ...
  encoded = writer.Encode("987", BCFORMAT_CODABAR, width, height);
  EXPECT_EQ(1, height);
  EXPECT_EQ(MODULES_FOR_DELIMITERS + 3 * MODULES_PER_NUMBER, width);
  expected =
      "#  #  # ## "  // N (same as B) Start
