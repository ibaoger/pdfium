...
int32_t GetAlphaNumericCode(int32_t code) {
  if (code < 0)
    return -1;
  return static_cast<size_t>(code) < FX_ArraySize(g_alphaNumericTable)
             ? g_alphaNumericTable[code]
             : -1;
}
...
void InterleaveWithECBytes(CBC_QRCoderBitVector* bits,
  ...
  std::vector<std::unique_ptr<CBC_QRCoderBlockPair>> blocks;
  for (int32_t i = 0; i < numRSBlocks; i++) {
    ...
    blocks.push_back(pdfium::MakeUnique<CBC_QRCoderBlockPair>(
        std::move(dataBytes), std::move(ecBytes)));
