...
bool CPDF_TextPageFind::FindFirst(const CFX_WideString& findwhat,
  ...
  if (startPos == -1) {
    ...
    m_findPreStart = static_cast<FX_STRSIZE>(startPos);
    m_findNextStart = static_cast<FX_STRSIZE>(startPos);
  }
...
bool CPDF_TextPageFind::FindPrev() {
  ...
  while (ret) {
    ...
    if (ret) {
      ...
      if (!m_findPreStart.has_value() ||
          static_cast<FX_STRSIZE>((order1 + MatchedCount1) - 1) >
              m_findPreStart.value())
        break;
      order = order1;
