...
CFX_WideString CPDF_TextPage::GetPageText(int start, int nCount) const {
  ...
  if (nCount == -1)
    return CFX_WideString(m_TextBuf.AsStringC()(start, FX_STRSIZE_MAX));
  ...
  int nLast = start + nCount - nCountOffset;
  return CFX_WideString(m_TextBuf.AsStringC()(startindex, nLast));
}
