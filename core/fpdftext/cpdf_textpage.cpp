...
WideString CPDF_TextPage::GetPageText(int start, int count) const {
  if (start < 0 || count <= 0 || !m_bIsParsed || m_CharList.empty() ||
  ...
  int text_available = static_cast<int>(text_buf.GetLength()) - text_start;
  if (text_available <= 0)
    return L"";
  ...
  return WideString(text_buf.Mid(static_cast<size_t>(text_start),
