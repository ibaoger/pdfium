...
bool CPDF_TextPage::IsHyphen(wchar_t curChar) const {
  CFX_WideString curText;
  if (m_TempTextBuf.GetLength() != 0)
    curText = m_TempTextBuf.MakeString();
  else if (m_TextBuf.GetLength() != 0)
    curText = m_TextBuf.MakeString();
  else
    return false;

  curText.TrimRight(0x20);
