...
void CPDF_TextPage::ProcessTextObject(
  ...
  CFX_PointF prev_pos = prev_Obj.m_pTextObj->GetPos();

  prev_Obj.m_formMatrix.TransformPoint(prev_pos.x, prev_pos.y);
  m_DisplayMatrix.TransformPoint(prev_pos.x, prev_pos.y);
...
CPDF_TextPage::GenerateCharacter CPDF_TextPage::ProcessInsertObject(
  ...
  bool bNewline = false;
  if (WritingMode == TextOrientation::Horizontal) {
    CFX_FloatRect rect1 = CFX_FloatRect(m_pPreTextObj->m_Left, pObj->m_Bottom,
