...
// Remove characters from the end of a string |str|, delimited by |start| and
// |end|, up to and including |charToFind|. No-op is |charToFind| is not
// present. Updates |end| if characters were removed.
  ...
  if (start != -1) {
    ...
    if (len > off + 4) {                      // At least "://<char>" follows.
      ...
      if (str[off] == L':' && str[off + 1] == L'/' && str[off + 2] == L'/') {
        ...
        end = TrimExternalBracketsFromWebLink(str, start, end);
