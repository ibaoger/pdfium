...
TEST(CPDF_LinkExtractTest, CheckWebLink) {
  ...
  for (size_t i = 0; i < FX_ArraySize(invalid_cases); ++i) {
    ...
    EXPECT_FALSE(extractor.CheckWebLink(text_str, start_offset, count))
        << text_str.c_str();
    EXPECT_EQ(DEFAULT_VALUE, start_offset);
    EXPECT_EQ(DEFAULT_VALUE, count);
  }
  ...
  for (size_t i = 0; i < FX_ArraySize(valid_casess); ++i) {
    CFX_WideString text_str(valid_casess[i].input_string);
    int32_t start_offset = DEFAULT_VALUE;
    int32_t count = DEFAULT_VALUE;
    EXPECT_TRUE(extractor.CheckWebLink(text_str, start_offset, count))
        << text_str.c_str();
    EXPECT_STREQ(valid_casess[i].url_extracted, text_str.c_str());
    EXPECT_EQ(valid_casess[i].start_offset, start_offset);
    EXPECT_EQ(valid_casess[i].count, count);
  }
}
