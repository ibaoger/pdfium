...
TEST(CPDF_LinkExtractTest, CheckWebLink) {
  ...
  const ValidCase valid_cases[] = {
      ...
      {L"(http://www.abc.com)0", L"http://www.abc.com)0", 1,
       20},                                            // Trim chars before URL.
  ...
  for (size_t i = 0; i < FX_ArraySize(valid_cases); ++i) {
    ...
    EXPECT_EQ(valid_cases[i].start_offset, start_offset);
    EXPECT_EQ(valid_cases[i].count, count);
  }
}
