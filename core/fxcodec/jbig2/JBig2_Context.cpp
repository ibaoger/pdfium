...
int32_t CJBig2_Context::parseSymbolDict(CJBig2_Segment* pSegment) {
  ...
  if (!cache_hit) {
    if (bUseGbContext) {
      ...
      pSegment->m_SymbolDict = pdfium::WrapUnique<CJBig2_SymbolDict>(
          pSymbolDictDecoder->decode_Arith(pArithDecoder.get(), &gbContext,
                                           &grContext));
      ...
      pSegment->m_SymbolDict = pdfium::WrapUnique<CJBig2_SymbolDict>(
          pSymbolDictDecoder->decode_Huffman(m_pStream.get(), &gbContext,
                                             &grContext));
      if (!pSegment->m_SymbolDict)
        return JBIG2_ERROR_FATAL;
      m_pStream->alignByte();
...
int32_t CJBig2_Context::parsePatternDict(CJBig2_Segment* pSegment,
  ...
  if (pPDD->HDMMR == 0) {
    ...
    auto pArithDecoder =
        pdfium::MakeUnique<CJBig2_ArithDecoder>(m_pStream.get());
    pSegment->m_PatternDict = pdfium::WrapUnique<CJBig2_PatternDict>(
...
int32_t CJBig2_Context::parseHalftoneRegion(CJBig2_Segment* pSegment,
  ...
  CJBig2_PatternDict* pPatternDict = pSeg->m_PatternDict.get();
  ...
  if (pHRD->HMMR == 0) {
    ...
    auto pArithDecoder =
        pdfium::MakeUnique<CJBig2_ArithDecoder>(m_pStream.get());
    pSegment->m_Image = pdfium::WrapUnique<CJBig2_Image>(
    ...
    pSegment->m_Image =
        pdfium::WrapUnique<CJBig2_Image>(pHRD->decode_MMR(m_pStream.get()));
    if (!pSegment->m_Image)
      return JBIG2_ERROR_FATAL;
    m_pStream->alignByte();
  }
...
int32_t CJBig2_Context::parseTable(CJBig2_Segment* pSegment) {
  ...
  pSegment->m_HuffmanTable = nullptr;
