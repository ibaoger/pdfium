...
CJBig2_Image::CJBig2_Image(const CJBig2_Image& other)
  ...
  if (other.m_pData) {
    m_pData.Reset(std::unique_ptr<uint8_t, FxFreeDeleter>(
        FX_Alloc2D(uint8_t, m_nStride, m_nHeight)));
    JBIG2_memcpy(m_pData.Get(), other.m_pData.Get(), m_nStride * m_nHeight);
  }
}
...
void CJBig2_Image::copyLine(int32_t hTo, int32_t hFrom) {
  ...
  if (hFrom < 0 || hFrom >= m_nHeight) {
    JBIG2_memset(m_pData.Get() + hTo * m_nStride, 0, m_nStride);
  } else {
    JBIG2_memcpy(m_pData.Get() + hTo * m_nStride,
                 m_pData.Get() + hFrom * m_nStride, m_nStride);
  }
}
void CJBig2_Image::fill(bool v) {
  ...
  JBIG2_memset(m_pData.Get(), v ? 0xff : 0, m_nStride * m_nHeight);
}
...
std::unique_ptr<CJBig2_Image> CJBig2_Image::subImage(int32_t x,
  ...
  pLineSrc = m_pData.Get() + m_nStride * y;
  pLineDst = pImage->m_pData.Get();
  m = (x >> 5) << 2;
  n = x & 31;
  if (n == 0) {
    for (j = 0; j < h; j++) {
...
void CJBig2_Image::expand(int32_t h, bool v) {
  ...
  if (m_pData.IsOwned()) {
    ...
    uint8_t* pExternalBuffer = m_pData.Get();
    m_pData.Reset(std::unique_ptr<uint8_t, FxFreeDeleter>(
        FX_Alloc(uint8_t, h * m_nStride)));
    JBIG2_memcpy(m_pData.Get(), pExternalBuffer, m_nHeight * m_nStride);
  }
  JBIG2_memset(m_pData.Get() + m_nHeight * m_nStride, v ? 0xff : 0,
...
bool CJBig2_Image::composeTo_opt2(CJBig2_Image* pDst,
  ...
  maskM = maskL & maskR;
  uint8_t* lineSrc = m_pData.Get() + ys0 * m_nStride + ((xs0 >> 5) << 2);
  lineLeft = m_nStride - ((xs0 >> 5) << 2);
  uint8_t* lineDst =
      pDst->m_pData.Get() + yd0 * pDst->m_nStride + ((xd0 >> 5) << 2);
  if ((xd0 & ~31) == ((xd1 - 1) & ~31)) {
    if ((xs0 & ~31) == ((xs1 - 1) & ~31)) {
      if (s1 > d1) {
        shift = s1 - d1;
...
bool CJBig2_Image::composeTo_opt2(CJBig2_Image* pDst,
  ...
  uint8_t* lineSrc = m_pData.Get() + (pSrcRect->top + ys0) * m_nStride +
