...
CFX_GifDecodeStatus CFX_GifContext::ReadGifSignature() {
  ...
  strncpy(header_string, gif_header->signature, 3);
  strncpy(&header_string[3], gif_header->version, 3);
  if (strcmp(header_string, kGifSignature87) != 0 &&
      strcmp(header_string, kGifSignature89) != 0)
    return CFX_GifDecodeStatus::Error;
...
CFX_GifDecodeStatus CFX_GifContext::ReadLogicalScreenDescriptor() {
  ...
  if (gif_lsd->global_flags.global_pal) {
    uint32_t global_pal_size =
        unsigned(2 << gif_lsd->global_flags.pal_bits) * 3u;
  ...
  width_ = static_cast<int>(
      FXWORD_GET_MSBFIRST(reinterpret_cast<uint8_t*>(&gif_lsd->width)));
  height_ = static_cast<int>(
      FXWORD_GET_MSBFIRST(reinterpret_cast<uint8_t*>(&gif_lsd->height)));
