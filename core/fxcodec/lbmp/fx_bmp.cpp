...
bool validate(uint8_t val, bmp_decompress_struct_p bmp_ptr) {
        ...
        switch (*first_byte_ptr) {
          ...
          default: {
            ...
            for (size_t i = 0; i < *first_byte_ptr; ++i) {
              if (!validate(first_buf[i], bmp_ptr))
                return 0;
            }
    ...
    switch (*first_byte_ptr) {
      ...
      default: {
        ...
        for (uint8_t i = 0; i < *first_byte_ptr; i++) {
          ...
          if (!validate(second_byte, bmp_ptr))
            return 0;
