...
void color_apply_icc_profile(opj_image_t* image) {
  ...
  // TODO(tsepez): why is this safe?
  max = max_w * max_h;

  if (image->numcomps > 2) {
    if (prec <= 8) {
      cmsUInt32Number nr_samples = max * 3 * sizeof(unsigned char);
      std::unique_ptr<unsigned char, FxFreeDeleter> inbuf(
          FX_Alloc(unsigned char, nr_samples));
      std::unique_ptr<unsigned char, FxFreeDeleter> outbuf(
          FX_Alloc(unsigned char, nr_samples));
      unsigned char* in = inbuf.get();
      unsigned char* out = outbuf.get();
      ...
      cmsDoTransform(transform, inbuf.get(), outbuf.get(),
                     (cmsUInt32Number)max);
      r = image->comps[0].data;
      g = image->comps[1].data;
      b = image->comps[2].data;
      ...
      std::unique_ptr<unsigned short, FxFreeDeleter> inbuf(
          FX_Alloc(unsigned short, nr_samples));
      std::unique_ptr<unsigned short, FxFreeDeleter> outbuf(
          FX_Alloc(unsigned short, nr_samples));
      unsigned short* in = inbuf.get();
      unsigned short* out = outbuf.get();
      r = image->comps[0].data;
      g = image->comps[1].data;
      b = image->comps[2].data;
      for (int i = 0; i < max; ++i) {
    ...
    unsigned char* in = inbuf.get();
    unsigned char* out = outbuf.get();
    image->comps = static_cast<opj_image_comp_t*>(opj_realloc(
    ...
    image->comps[1].data =
        static_cast<int*>(opj_calloc(static_cast<size_t>(max), sizeof(int)));
    image->comps[2].data =
        static_cast<int*>(opj_calloc(static_cast<size_t>(max), sizeof(int)));
    image->numcomps += 2;
    r = image->comps[0].data;
    ...
    cmsDoTransform(transform, inbuf.get(), outbuf.get(), (cmsUInt32Number)max);
    r = image->comps[0].data;
    g = image->comps[1].data;
    b = image->comps[2].data;
    for (int i = 0; i < max; ++i) {
...
void color_apply_conversion(opj_image_t* image) {
  ...
  if (enumcs == 14) {
    ...
    red = static_cast<int*>(opj_calloc(max, sizeof(int)));
    green = static_cast<int*>(opj_calloc(max, sizeof(int)));
    blue = static_cast<int*>(opj_calloc(max, sizeof(int)));
