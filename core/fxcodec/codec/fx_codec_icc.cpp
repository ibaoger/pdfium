...
bool Check3Components(cmsColorSpaceSignature cs, int nComponents, bool bDst) {
  switch (cs) {
    case cmsSigGrayData:
      return false;
    case cmsSigCmykData:
...
std::unique_ptr<CLcmsCmm> CCodec_IccModule::CreateTransform_sRGB(
  ...
  if (!Check3Components(dstCS, 3, true)) {
    cmsCloseProfile(srcProfile);
    cmsCloseProfile(dstProfile);
    return nullptr;
  }
  ...
  auto pCmm = pdfium::MakeUnique<CLcmsCmm>();
  pCmm->m_nSrcComponents = nSrcComponents;
  pCmm->m_nDstComponents = 3;
