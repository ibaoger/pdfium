...
uint32_t FlateGetPossiblyTruncatedTotalOut(void* context) {
  return pdfium::base::saturated_cast<uint32_t>(
      ((z_stream*)context)->total_out);
}

uint32_t FlateGetPossiblyTruncatedTotalIn(void* context) {
  return pdfium::base::saturated_cast<uint32_t>(((z_stream*)context)->total_in);
}
...
void* FlateInit() {
  z_stream* p = (z_stream*)FX_Alloc2D(uint8_t, 1, sizeof(z_stream));
  if (!p)
    return nullptr;
