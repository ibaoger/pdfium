...
GifDecodeStatus gif_decode_extension(CGifDecompressor* gif_ptr) {
  ...
  switch (gif_ptr->decode_status) {
    ...
    case GIF_D_STATUS_EXT_PTE: {
      ...
      auto gif_plaintext = pdfium::MakeUnique<GifPlainText>();
      if (gif_ptr->m_GifGCE.get())
        gif_plaintext->gce_ptr = std::move(gif_ptr->m_GifGCE);
      gif_plaintext->pte_ptr = pdfium::MakeUnique<GifPTE>();
      gif_plaintext->pte_ptr->block_size = gif_pte_ptr->block_size;
      gif_plaintext->pte_ptr->grid_left =
          GetWord_LSBFirst((uint8_t*)&gif_pte_ptr->grid_left);
      gif_plaintext->pte_ptr->grid_top =
          GetWord_LSBFirst((uint8_t*)&gif_pte_ptr->grid_top);
      gif_plaintext->pte_ptr->grid_width =
          GetWord_LSBFirst((uint8_t*)&gif_pte_ptr->grid_width);
      gif_plaintext->pte_ptr->grid_height =
          GetWord_LSBFirst((uint8_t*)&gif_pte_ptr->grid_height);
      gif_plaintext->pte_ptr->char_width = gif_pte_ptr->char_width;
      gif_plaintext->pte_ptr->char_height = gif_pte_ptr->char_height;
      gif_plaintext->pte_ptr->fc_index = gif_pte_ptr->fc_index;
      gif_plaintext->pte_ptr->bc_index = gif_pte_ptr->bc_index;
      if (!gif_read_data(gif_ptr, &data_size_ptr, 1)) {
        gif_ptr->skip_size = skip_size_org;
        return GifDecodeStatus::Unfinished;
      }
      while (*data_size_ptr != GIF_BLOCK_TERMINAL) {
        ...
        gif_plaintext->string_ptr +=
            CFX_ByteString((const char*)data_ptr, data_size);
      }
      gif_ptr->m_GifPlainTexts.push_back(std::move(gif_plaintext));
    } break;
...
GifDecodeStatus gif_decode_image_info(CGifDecompressor* gif_ptr) {
  ...
  if (gif_img_info_lf_ptr->local_pal) {
    ...
    memcpy((uint8_t*)gif_image->m_LocalPalettes.data(), loc_pal_ptr,
           loc_pal_size);
  }
  ...
  gif_ptr->m_ImageVector.push_back(std::move(gif_image));
...
GifDecodeStatus gif_load_frame(CGifDecompressor* gif_ptr, int32_t frame_num) {
  if (!gif_ptr || !pdfium::IndexInBounds(gif_ptr->m_ImageVector, frame_num))
    return GifDecodeStatus::Error;

  uint8_t* data_size_ptr = nullptr;
  uint8_t* data_ptr = nullptr;
  uint32_t skip_size_org = gif_ptr->skip_size;
  GifImage* gif_image_ptr = gif_ptr->m_ImageVector[frame_num].get();
...
int32_t gif_get_frame_num(CGifDecompressor* gif_ptr) {
  return pdfium::CollectionSize<int32_t>(gif_ptr->m_ImageVector);
}
