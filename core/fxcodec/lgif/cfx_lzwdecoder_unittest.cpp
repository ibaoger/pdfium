...
TEST(CFX_LZWDecoder, MakeDecoderBadParams) {
  EXPECT_EQ(nullptr, CFX_LZWDecoder::MakeDecoder(nullptr, 0x1, 0x2));
  EXPECT_EQ(nullptr, CFX_LZWDecoder::MakeDecoder(nullptr, 0x10, 0x2));
  EXPECT_EQ(nullptr, CFX_LZWDecoder::MakeDecoder(nullptr, 0x4, 0x0F));
}

TEST(CFX_LZWDecoder, DecodeBadParams) {
  char error_msg[GIF_MAX_ERROR_SIZE];

  uint8_t palette_exp = 0x0;
  uint8_t code_exp = 0x2;

  auto decoder = CFX_LZWDecoder::MakeDecoder(error_msg, palette_exp, code_exp);
