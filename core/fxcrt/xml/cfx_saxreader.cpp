...
static const FX_SAXReader_LPFParse g_FX_SAXReader_LPFParse[] = {
...
static_assert(FX_ArraySize(g_FX_SAXReader_LPFParse) ==
                  static_cast<size_t>(CFX_SaxMode::MAX),
              "g_FX_SAXReader_LPFParse size mismatch");
...
int32_t CFX_SAXReader::CurrentNameIndex() const {
  return pdfium::CollectionSize<int32_t>(m_Name) - 1;
}
      ...
      if (m_CurByte == m_SkipChar) {
        ...
        if (m_SkipStack.empty() && m_CurByte == '>') {
          if (m_Data.size() >= 9 && memcmp(m_Data.data(), "[CDATA[", 7) == 0 &&
            ...
            Pop();
            m_Data.erase(m_Data.begin() - 2, m_Data.begin() + 7);
