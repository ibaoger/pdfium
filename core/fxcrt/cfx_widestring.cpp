...
CFX_WideString::CFX_WideString(const wchar_t* pStr, size_t nLen) {
  if (nLen)
...
void CFX_WideString::AssignCopy(const wchar_t* pSrcData, size_t nSrcLen) {
  AllocBeforeWrite(nSrcLen);
  m_pData->CopyContents(pSrcData, nSrcLen);
  m_pData->m_nDataLength = nSrcLen;
}

void CFX_WideString::ReallocBeforeWrite(size_t nNewLength) {
...
void CFX_WideString::AllocBeforeWrite(size_t nNewLength) {
  if (m_pData && m_pData->CanOperateInPlace(nNewLength))
    return;
...
void CFX_WideString::ReleaseBuffer(size_t nNewLength) {
  if (!m_pData)
    return;
...
void CFX_WideString::Reserve(size_t len) {
  GetBuffer(len);
}

wchar_t* CFX_WideString::GetBuffer(size_t nMinBufLength) {
  if (!m_pData) {
    if (nMinBufLength == 0)
      return nullptr;
...
size_t CFX_WideString::Delete(size_t index, size_t count) {
  if (!m_pData)
    return 0;
...
void CFX_WideString::Concat(const wchar_t* pSrcData, size_t nSrcLen) {
...
CFX_WideString CFX_WideString::Mid(size_t first, size_t count) const {
  if (!m_pData)
    return CFX_WideString();
...
CFX_WideString CFX_WideString::Left(size_t count) const {
  if (count == 0 || !IsValidLength(count))
    return CFX_WideString();
  return Mid(0, count);
}

CFX_WideString CFX_WideString::Right(size_t count) const {
  if (count == 0 || !IsValidLength(count))
    return CFX_WideString();
...
bool CFX_WideString::TryVSWPrintf(size_t size,
...
size_t CFX_WideString::Insert(size_t location, wchar_t ch) {
...
pdfium::Optional<size_t> CFX_WideString::Find(wchar_t ch, size_t start) const {
  if (!m_pData)
    return pdfium::Optional<size_t>();
...
pdfium::Optional<size_t> CFX_WideString::Find(const CFX_WideStringC& subStr,
                                              size_t start) const {
  if (!m_pData)
    return pdfium::Optional<size_t>();
...
size_t CFX_WideString::Remove(wchar_t chRemove) {
...
size_t CFX_WideString::Replace(const CFX_WideStringC& pOld,
  ...
  while (1) {
    const wchar_t* pTarget = FX_wcsstr(pStart, (size_t)(pEnd - pStart),
...
void CFX_WideString::SetAt(size_t index, wchar_t c) {
...
size_t CFX_WideString::WStringLength(const unsigned short* str) {
  size_t len = 0;
  if (str)
    while (str[len])
      len++;
