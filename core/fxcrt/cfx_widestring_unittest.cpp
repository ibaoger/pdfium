...
TEST(fxcrt, WideStringCOneCharIterator) {
  ...
  EXPECT_EQ(L'a', static_cast<int32_t>(sum));
}
...
TEST(fxcrt, WideStringCMultiCharIterator) {
  ...
  EXPECT_TRUE(any_present);
  EXPECT_EQ(L'a' + L'b' + L'c', static_cast<int32_t>(sum));
}

TEST(fxcrt, WideStringCAnyAllNoneOf) {
  CFX_WideStringC str(L"aaaaaaaaaaaaaaaaab");
  EXPECT_FALSE(std::all_of(str.begin(), str.end(), [](const uint8_t& c) {
    return c == static_cast<uint8_t>(L'a');
  }));

  EXPECT_FALSE(std::none_of(str.begin(), str.end(), [](const uint8_t& c) {
    return c == static_cast<uint8_t>(L'a');
  }));

  EXPECT_TRUE(std::any_of(str.begin(), str.end(), [](const uint8_t& c) {
    return c == static_cast<uint8_t>(L'a');
  }));
