...
inline int FXSYS_toHexDigit(const unsigned char c) {
  if (!std::isxdigit(c))
    return 0;
...
inline bool FXSYS_isDecimalDigit(const unsigned char c) {
...
inline int FXSYS_toDecimalDigit(const unsigned char c) {
