...
class CFX_RTemplate {
 public:
  ...
  void Set(const FXT_POINT& p1, const FXT_SIZE& s2) {
    TopLeft(p1);
    width = s2.x;
    height = s2.y;
  }
 ...
 private:
  void SetTopLeft(FXT_POINT tl) {
    left = tl.x;
    top = tl.y;
  }
  void SetTopRight(FXT_POINT tr) {
    width = tr.x - left;
    top = tr.y;
  }
  void SetBottomLeft(FXT_POINT bl) {
    left = bl.x;
    height = bl.y - top;
  }
  void SetBottomRight(FXT_POINT br) {
    width = br.x - left;
    height = br.y - top;
  }
};
