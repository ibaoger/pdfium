...
#include "core/fxcrt/cfx_pdfstreamstream.h"
...
CFX_PDFStreamStream::CFX_PDFStreamStream(
...
FX_FILESIZE CFX_PDFStreamStream::GetSize() {
  uint32_t dwSize = 0;
...
bool CFX_PDFStreamStream::ReadBlock(void* buffer,
                                    FX_FILESIZE offset,
                                    size_t size) {
...
size_t CFX_PDFStreamStream::ReadBlock(void* buffer, size_t size) {
  NOTREACHED();
  return 0;
}

FX_FILESIZE CFX_PDFStreamStream::GetPosition() {
  return 0;
}

bool CFX_PDFStreamStream::IsEOF() {
  return false;
}

bool CFX_PDFStreamStream::Flush() {
  NOTREACHED();
  return false;
}

bool CFX_PDFStreamStream::WriteBlock(const void* pData,
                                     FX_FILESIZE offset,
                                     size_t size) {
  NOTREACHED();
  return false;
