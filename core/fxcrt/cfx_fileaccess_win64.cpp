...
#include "core/fxcrt/cfx_fileaccess_win64.h"
...
std::unique_ptr<IFX_FileAccess> IFX_FileAccess::Create() {
  return pdfium::MakeUnique<CFX_FileAccess_Win64>();
}
...
CFX_FileAccess_Win64::CFX_FileAccess_Win64() : m_hFile(nullptr) {}

CFX_FileAccess_Win64::~CFX_FileAccess_Win64() {
  Close();
}

bool CFX_FileAccess_Win64::Open(const CFX_ByteStringC& fileName,
                                uint32_t dwMode) {
  if (m_hFile)
    return false;
...
bool CFX_FileAccess_Win64::Open(const CFX_WideStringC& fileName,
                                uint32_t dwMode) {
  if (m_hFile)
    return false;
...
void CFX_FileAccess_Win64::Close() {
  if (!m_hFile)
    return;
...
FX_FILESIZE CFX_FileAccess_Win64::GetSize() const {
  if (!m_hFile)
    return 0;
...
FX_FILESIZE CFX_FileAccess_Win64::GetPosition() const {
  if (!m_hFile)
...
FX_FILESIZE CFX_FileAccess_Win64::SetPosition(FX_FILESIZE pos) {
  if (!m_hFile)
    return (FX_FILESIZE)-1;
...
size_t CFX_FileAccess_Win64::Read(void* pBuffer, size_t szBuffer) {
  if (!m_hFile)
    return 0;
...
size_t CFX_FileAccess_Win64::Write(const void* pBuffer, size_t szBuffer) {
  if (!m_hFile)
    return 0;
...
size_t CFX_FileAccess_Win64::ReadPos(void* pBuffer,
                                     size_t szBuffer,
                                     FX_FILESIZE pos) {
  if (!m_hFile)
    return 0;
...
size_t CFX_FileAccess_Win64::WritePos(const void* pBuffer,
                                      size_t szBuffer,
                                      FX_FILESIZE pos) {
  if (!m_hFile) {
    return 0;
...
bool CFX_FileAccess_Win64::Flush() {
  if (!m_hFile)
    return false;
...
bool CFX_FileAccess_Win64::Truncate(FX_FILESIZE szFile) {
