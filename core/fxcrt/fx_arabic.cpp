...
const FX_ARBFORMTABLE* ParseChar(const CFX_Char* pTC,
  ...
  if (!pFT || eType >= charTYPE_ArabicNormal)
    eType = charTYPE_Unknown;
...
wchar_t GetFormChar(const CFX_Char* cur,
  ...
  if (eCur < charTYPE_ArabicAlef || eCur >= charTYPE_ArabicNormal) {
    return wCur;
  }
  charTYPE ePrev;
  wchar_t wPrev;
  ParseChar(prev, wPrev, ePrev);
  if (wPrev == 0x0644 && eCur == charTYPE_ArabicAlef) {
    return 0xFEFF;
  }
  charTYPE eNext;
  wchar_t wNext;
  ParseChar(next, wNext, eNext);
  bool bAlef = (eNext == charTYPE_ArabicAlef && wCur == 0x644);
  if (ePrev < charTYPE_ArabicAlef) {
    if (bAlef) {
      return FX_GetArabicFromAlefTable(wNext);
    }
    return (eNext < charTYPE_ArabicAlef) ? ft->wIsolated : ft->wInitial;
  }
  if (bAlef) {
    wCur = FX_GetArabicFromAlefTable(wNext);
    return (ePrev != charTYPE_ArabicDistortion) ? wCur : ++wCur;
  }
  if (ePrev == charTYPE_ArabicAlef || ePrev == charTYPE_ArabicSpecial) {
    return (eNext < charTYPE_ArabicAlef) ? ft->wIsolated : ft->wInitial;
  }
  return (eNext < charTYPE_ArabicAlef) ? ft->wFinal : ft->wMedial;
}
