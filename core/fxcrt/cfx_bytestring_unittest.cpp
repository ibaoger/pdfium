...
TEST(fxcrt, ByteStringCNull) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), null_string.GetLength());
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), copied_null_string.GetLength());
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), empty_string.GetLength());
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), assigned_null_string.GetLength());
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), assigned_nullptr_string.GetLength());
...
TEST(fxcrt, ByteStringInsert) {
  CFX_ByteString fred("FRED");
  EXPECT_EQ(static_cast<FX_STRSIZE>(4lu), fred.Insert(-1, 'X'));
  EXPECT_EQ("FRED", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(5lu), fred.Insert(0, 'S'));
  EXPECT_EQ("SFRED", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(6lu), fred.Insert(1, 'T'));
  EXPECT_EQ("STFRED", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(7lu), fred.Insert(4, 'U'));
  EXPECT_EQ("STFRUED", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(8lu), fred.Insert(7, 'V'));
  EXPECT_EQ("STFRUEDV", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(8lu), fred.Insert(12, 'P'));
  EXPECT_EQ("STFRUEDV", fred);
  {
    CFX_ByteString empty;
    EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), empty.Insert(-1, 'X'));
    EXPECT_NE("X", empty);
  }
  {
    CFX_ByteString empty;
    EXPECT_EQ(static_cast<FX_STRSIZE>(1lu), empty.Insert(0, 'X'));
    EXPECT_EQ("X", empty);
  }
  {
    CFX_ByteString empty;
    EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), empty.Insert(5, 'X'));
    EXPECT_NE("X", empty);
  }
}
...
TEST(fxcrt, ByteStringInsertAtFrontAndInsertAtBack) {
  {
    CFX_ByteString empty;
    EXPECT_EQ(static_cast<FX_STRSIZE>(1lu), empty.InsertAtFront('D'));
    EXPECT_EQ("D", empty);
    EXPECT_EQ(static_cast<FX_STRSIZE>(2lu), empty.InsertAtFront('E'));
    EXPECT_EQ("ED", empty);
    EXPECT_EQ(static_cast<FX_STRSIZE>(3lu), empty.InsertAtFront('R'));
    EXPECT_EQ("RED", empty);
    EXPECT_EQ(static_cast<FX_STRSIZE>(4lu), empty.InsertAtFront('F'));
    EXPECT_EQ("FRED", empty);
  }
  {
    CFX_ByteString empty;
    EXPECT_EQ(static_cast<FX_STRSIZE>(1lu), empty.InsertAtBack('F'));
    EXPECT_EQ("F", empty);
    EXPECT_EQ(static_cast<FX_STRSIZE>(2lu), empty.InsertAtBack('R'));
    EXPECT_EQ("FR", empty);
    EXPECT_EQ(static_cast<FX_STRSIZE>(3lu), empty.InsertAtBack('E'));
    EXPECT_EQ("FRE", empty);
    EXPECT_EQ(static_cast<FX_STRSIZE>(4lu), empty.InsertAtBack('D'));
    EXPECT_EQ("FRED", empty);
  }
  {
    CFX_ByteString empty;
    EXPECT_EQ(static_cast<FX_STRSIZE>(1lu), empty.InsertAtBack('E'));
    EXPECT_EQ("E", empty);
    EXPECT_EQ(static_cast<FX_STRSIZE>(2lu), empty.InsertAtFront('R'));
    EXPECT_EQ("RE", empty);
    EXPECT_EQ(static_cast<FX_STRSIZE>(3lu), empty.InsertAtBack('D'));
    EXPECT_EQ("RED", empty);
    EXPECT_EQ(static_cast<FX_STRSIZE>(4lu), empty.InsertAtFront('F'));
    EXPECT_EQ("FRED", empty);
  }
...
TEST(fxcrt, ByteStringDelete) {
  CFX_ByteString fred("FRED");
  EXPECT_EQ(static_cast<FX_STRSIZE>(4lu), fred.Delete(0, 0));
  EXPECT_EQ("FRED", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(2lu), fred.Delete(0, 2));
  EXPECT_EQ("ED", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(1lu), fred.Delete(1));
  EXPECT_EQ("E", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(1lu), fred.Delete(-1));
  EXPECT_EQ("E", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), fred.Delete(0));
  EXPECT_EQ("", fred);
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), fred.Delete(0));
  EXPECT_EQ("", fred);

  CFX_ByteString empty;
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), empty.Delete(0));
  EXPECT_EQ("", empty);
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), empty.Delete(-1));
  EXPECT_EQ("", empty);
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), empty.Delete(1));
  EXPECT_EQ("", empty);
}
...
TEST(fxcrt, ByteStringFind) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), result.value());
  ...
  ASSERT_TRUE(result.has_value());
  EXPECT_EQ(static_cast<FX_STRSIZE>(2lu), result.value());
  result = longer_string.Find('c', 3);
  ASSERT_TRUE(result.has_value());
  EXPECT_EQ(static_cast<FX_STRSIZE>(3lu), result.value());
...
TEST(fxcrt, ByteStringReverseFind) {
  ...
  ASSERT_TRUE(result.has_value());
  EXPECT_EQ(static_cast<FX_STRSIZE>(4lu), result.value());
...
TEST(fxcrt, ByteStringCNotNull) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(3lu), string3.GetLength());
  EXPECT_EQ(static_cast<FX_STRSIZE>(6lu), string6.GetLength());
  EXPECT_EQ(static_cast<FX_STRSIZE>(3lu), alternate_string3.GetLength());
  EXPECT_EQ(static_cast<FX_STRSIZE>(7lu), embedded_nul_string7.GetLength());
  EXPECT_EQ(static_cast<FX_STRSIZE>(7lu), illegal_string7.GetLength());
...
TEST(fxcrt, ByteStringCFromChar) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(1lu), nul_string_from_char.GetLength());
  EXPECT_NE(null_string, nul_string_from_char);

  EXPECT_EQ(static_cast<FX_STRSIZE>(1lu), lower_a_string_from_char.GetLength());
...
TEST(fxcrt, ByteStringCFromVector) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), cleared_string.GetLength());
...
TEST(fxcrt, EmptyByteString) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), empty_str.GetLength());
  const char* cstr = empty_str.c_str();
  EXPECT_EQ(static_cast<FX_STRSIZE>(0lu), FXSYS_strlen(cstr));
}
...
TEST(fxcrt, OStreamByteStringOverload) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(2lu), str.GetLength());
  stream.str("");
  stream << str;
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(4lu), str.GetLength());
  stream.str("");
  stream << str;
