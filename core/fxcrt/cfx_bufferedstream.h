...
class CFX_BufferedStream : public CFX_Retainable {
 public:
  enum class Pos {
    Begin = 0,
    Current,
  ...
  void Seek(CFX_BufferedStream::Pos eSeek, FX_FILESIZE iOffset);
 ...
 private:
  CFX_BufferedStream(const CFX_RetainPtr<IFX_SeekableStream>& stream,
                     bool isWriteSteam);
  CFX_BufferedStream(uint8_t* data, FX_STRSIZE size);
  ~CFX_BufferedStream() override;
