...
void CFX_WideString::FormatV(const wchar_t* pFormat, va_list argList) {
  ...
  if (nMaxLen > 0) {
    // Simple, but non-standard conforming case: the vendor returned the
    // required size (typically only MSFT), so use it directly.
    FX_VA_COPY(argListCopy, argList);
    TryVSWPrintf(nMaxLen, pFormat, argListCopy);
    va_end(argListCopy);
    return;
  }
