...
class CFX_StringCTemplate {
 public:
  ...
  CFX_StringCTemplate(const CharType* ptr, size_t len)
  ...
  CFX_StringCTemplate Mid(size_t first, size_t count) const {
    if (!m_Ptr.Get())
  ...
  CFX_StringCTemplate Left(size_t count) const {
    if (count == 0 || !IsValidLength(count))
      return CFX_StringCTemplate();
    return Mid(0, count);
  }

  CFX_StringCTemplate Right(size_t count) const {
    if (count == 0 || !IsValidLength(count))
      return CFX_StringCTemplate();
