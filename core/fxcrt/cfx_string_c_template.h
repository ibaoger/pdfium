...
class CFX_StringCTemplate {
 public:
  ...
  CFX_StringCTemplate operator[](
      std::pair<FX_STRSIZE, FX_STRSIZE> range) const {
    if (!m_Length)
      return CFX_StringCTemplate();
    FX_STRSIZE first = pdfium::clamp(range.first, 0, m_Length - 1);
    FX_STRSIZE last = pdfium::clamp(range.second, first, m_Length);
