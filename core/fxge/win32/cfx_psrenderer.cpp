...
void CFX_PSRenderer::Init(IFX_WriteStream* pStream,
                          int pslevel,
                          int width,
                          int height,
                          bool bCmykOutput) {
...
void CFX_PSRenderer::OutputPath(const CFX_PathData* pPathData,
  ...
  m_pOutput->WriteBuffer(buf.GetBuffer(), buf.GetSize());
}
...
void CFX_PSRenderer::SetClip_PathStroke(const CFX_PathData* pPathData,
  ...
  m_pStream->WriteString("strokepath W n") if (pObject2Device) {
...
void CFX_PSRenderer::WritePSBinary(const uint8_t* data, int len) {
  ...
  if (pEncoders &&
      pEncoders->GetBasicModule()->A85Encode(data, len, &dest_buf,
                                             &dest_size)) {
    m_pOutput->WriteBuffer(dest_buf, dest_size);
    FX_Free(dest_buf);
  } else {
    m_pOutput->WriteBuffer(data, len);
  }
}
