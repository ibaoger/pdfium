...
void CFX_SkiaDeviceDriver::SetClipMask(const FX_RECT& clipBox,
  ...
  const CFX_RetainPtr<CFX_DIBitmap>& pThisLayer = mask.Emplace();
...
const CFX_RetainPtr<CFX_DIBitmap>& CFX_SkiaDeviceDriver::GetBackDrop() {
  return m_pOriDevice;
}
...
class CFX_Renderer {
 ...
 public:
  ...
  void render(const Scanline& sl) {
    ...
    const CFX_RetainPtr<CFX_DIBitmap>& pAlphaMask = m_pDevice->m_pAlphaMask;
    if (pAlphaMask) {
