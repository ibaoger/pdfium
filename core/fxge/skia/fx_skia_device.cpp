...
class SkiaState {
 public:
    ...
    if (Accumulator::kPath == m_type || drawIndex != m_commandIndex ||
        (Accumulator::kText == m_type &&
         (FontChanged(pFont, pMatrix, font_size, scaleX, color) ||
          hasRSX == !m_rsxform.count() || oneAtATime))) {
      Flush();
    }
    if (oneAtATime) {
      return false;
  ...
  void FlushText() {
    ...
    SkASSERT(sizeof(wchar_t) == sizeof(SkUnichar));
    printf("%*ls", m_glyphs.count(), (wchar_t*)text.begin());
#endif
    if (m_rsxform.count()) {
...
bool CFX_SkiaDeviceDriver::DrawDeviceText(int nChars,
  ...
  char match[] = "J is F";
  if (glyphs.count() >= strlen(match)) {
    bool found = true;
    for (int i = 0; i < strlen(match); ++i) {
      printf("%lc", text[i]);
      found &= match[i] == text[i];
    }
    printf("\n");
    if (found)
      printf("");
  ...
  if (useMultipleMasters) {
    for (int index = 0; index < nChars; ++index) {
      const FXTEXT_CHARPOS& cp = pCharPos[index];
      pFont->AdjustMMParams(glyphs[index], cp.m_FontCharWidth,
                            pSubstFont->m_Weight);
    }
    typeface = SkTypeface::MakeFromStream(
