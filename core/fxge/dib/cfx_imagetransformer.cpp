...
void CFX_ImageTransformer::CalcMask(const CalcData& cdata) {
  if (IsBilinear()) {
    ...
    for (int row = 0; row < m_result.Height(); row++) {
      uint8_t* dest_pos_mask = (uint8_t*)cdata.pBitmap->GetScanline(row);
...
void CFX_ImageTransformer::CalcAlpha(const CalcData& cdata) {
  if (IsBilinear()) {
    ...
    CFX_BilinearMatrix result2stretch_fix(cdata.matrix, 8);
    for (int row = 0; row < m_result.Height(); row++) {
      uint8_t* dest_scan = (uint8_t*)cdata.pBitmap->GetScanline(row);
      for (int col = 0; col < m_result.Width(); col++) {
        int src_col_l = 0;
        int src_row_l = 0;
        int res_x = 0;
...
void CFX_ImageTransformer::CalcMono(const CalcData& cdata,
  ...
  int destBpp = cdata.pBitmap->GetBPP() / 8;
  if (IsBilinear()) {
    ...
    CFX_BilinearMatrix result2stretch_fix(cdata.matrix, 8);
    for (int row = 0; row < m_result.Height(); row++) {
      uint8_t* dest_pos = (uint8_t*)cdata.pBitmap->GetScanline(row);
      for (int col = 0; col < m_result.Width(); col++) {
        int src_col_l = 0;
        int src_row_l = 0;
        int res_x = 0;
...
void CFX_ImageTransformer::DoBilinearLoop(
  ...
  for (int row = 0; row < m_result.Height(); row++) {
    uint8_t* dest = (uint8_t*)cdata.pBitmap->GetScanline(row);
    for (int col = 0; col < m_result.Width(); col++) {
      BilinearData d;
      d.res_x = 0;
      d.res_y = 0;
