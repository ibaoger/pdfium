...
void CompositeRow_AToMask(uint8_t* dest_scan,
                          const uint8_t* src_scan,
                          int pixel_count,
  ...
  if (m_DestFormat == FXDIB_8bppMask) {
    if (m_SrcFormat & 0x0200) {
      if (m_SrcFormat == FXDIB_Argb) {
        CompositeRow_AToMask(dest_scan, src_scan, width, clip_scan, 4);
      } else {
        CompositeRow_AToMask(dest_scan, src_extra_alpha, width, clip_scan, 1);
      }
    } else {
