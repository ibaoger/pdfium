...
bool CStretchEngine::CWeightTable::Calc(int dest_len,
  ...
  m_ItemSize =
      sizeof(int) * 2 +
      static_cast<int>(sizeof(int) * ceil(fabs(static_cast<float>(scale))) +
                       ext_size);
  m_DestMin = dest_min;
  ...
  if ((flags & FXDIB_NOSMOOTH) != 0 || fabs(static_cast<float>(scale)) < 1.0f) {
    for (int dest_pixel = dest_min; dest_pixel < dest_max; ++dest_pixel) {
      ...
      if (flags & FXDIB_INTERPOL) {
        ...
        pixel_weights.m_SrcStart = floor(static_cast<float>(src_pos));
    ...
    switch (m_TransMethod) {
      ...
      case 2: {
        for (int col = m_DestClip.left; col < m_DestClip.right; ++col) {
          ...
          if (m_Flags & FXDIB_BICUBIC_INTERPOL)
            dest_a = pdfium::clamp(dest_a, 0, 16711680);
      ...
      case 4: {
        for (int col = m_DestClip.left; col < m_DestClip.right; ++col) {
          ...
          if (m_Flags & FXDIB_BICUBIC_INTERPOL) {
            dest_r = pdfium::clamp(dest_r, 0, 16711680);
      ...
      case 5: {
        for (int col = m_DestClip.left; col < m_DestClip.right; ++col) {
          ...
          if (m_Flags & FXDIB_BICUBIC_INTERPOL) {
            dest_r_y = pdfium::clamp(dest_r_y, 0, 16711680);
            dest_g_m = pdfium::clamp(dest_g_m, 0, 16711680);
            dest_b_c = pdfium::clamp(dest_b_c, 0, 16711680);
          }
    ...
    switch (m_TransMethod) {
      ...
      case 4: {
        for (int col = m_DestClip.left; col < m_DestClip.right; ++col) {
          ...
          if (m_Flags & FXDIB_BICUBIC_INTERPOL) {
            dest_k = pdfium::clamp(dest_k, 0, 16711680);
            dest_a = pdfium::clamp(dest_a, 0, 16711680);
          }
