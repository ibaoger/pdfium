...
bool CStretchEngine::CWeightTable::Calc(int dest_len,
  ...
  m_ItemSize =
      sizeof(int) * (2 + static_cast<int>(ceil(fabs(scale)) + ext_size));
  ...
  m_pWeightTables.reset(FX_TryAlloc(uint8_t, m_dwWeightTablesSize));
  if (!m_pWeightTables.get())
    return false;

  if ((flags & FXDIB_NOSMOOTH) != 0 || fabs(scale) < 1.0f) {
    for (int dest_pixel = dest_min; dest_pixel < dest_max; ++dest_pixel) {
      PixelWeight& pixel_weights = *GetPixelWeight(dest_pixel);
      float src_pos = dest_pixel * scale + scale / 2 + base;
      if (flags & FXDIB_INTERPOL) {
        pixel_weights.m_SrcStart = static_cast<int>(floor(src_pos) - 1.0f / 2);
        pixel_weights.m_SrcEnd = static_cast<int>(floor(src_pos) + 1.0f / 2);
        ...
        if (pixel_weights.m_SrcStart == pixel_weights.m_SrcEnd) {
          ...
          pixel_weights.m_Weights[1] = FXSYS_round(
              (src_pos - pixel_weights.m_SrcStart - 1.0f / 2) * 65536);
        ...
        pixel_weights.m_SrcEnd = std::min(pixel_weights.m_SrcEnd, src_max - 1);
        int weight =
            FXSYS_round((src_pos - pixel_weights.m_SrcStart - 1.0f / 2) * 256);
        if (start == end) {
        ...
        pixel_weights.m_SrcStart = static_cast<int>(floor(src_pos));
        pixel_weights.m_SrcEnd = pixel_weights.m_SrcStart;
        pixel_weights.m_SrcStart = std::max(pixel_weights.m_SrcStart, src_min);
        pixel_weights.m_SrcEnd = std::min(pixel_weights.m_SrcEnd, src_max - 1);
        pixel_weights.m_Weights[0] = 65536;
      }
...
PixelWeight* CStretchEngine::CWeightTable::GetPixelWeight(int pixel) const {
  ...
  return reinterpret_cast<PixelWeight*>(m_pWeightTables.get() +
...
CStretchEngine::CStretchEngine(IFX_ScanlineComposer* pDestBitmap,
  ...
  size += 31;
  size = size / 32 * 4;
  m_pDestScanline.reset(FX_TryAlloc(uint8_t, size));
  if (!m_pDestScanline.get())
    return;
  if (dest_format == FXDIB_Rgb32)
    memset(m_pDestScanline.get(), 255, size);
...
bool CStretchEngine::StartStretchHorz() {
  if (m_DestWidth == 0 || m_InterPitch == 0 || !m_pDestScanline.get())
    return false;
  ...
  m_pInterBuf.reset(
      FX_TryAlloc(unsigned char, m_SrcClip.Height() * m_InterPitch));
  if (!m_pInterBuf.get())
    return false;

  if (m_pSource && m_bHasAlpha && m_pSource->m_pAlphaMask) {
    m_pExtraAlphaBuf.reset(
        FX_Alloc2D(unsigned char, m_SrcClip.Height(), m_ExtraMaskPitch));
    uint32_t size = (m_DestClip.Width() * 8 + 31) / 32 * 4;
    m_pDestMaskScanline.reset(FX_TryAlloc(unsigned char, size));
    if (!m_pDestMaskScanline.get())
      return false;
  }
  bool ret =
...
void CStretchEngine::StretchVert() {
  ...
  for (int row = m_DestClip.top; row < m_DestClip.bottom; ++row) {
    unsigned char* dest_scan = m_pDestScanline.get();
    unsigned char* dest_scan_mask = m_pDestMaskScanline.get();
    ...
    switch (m_TransMethod) {
      ...
      case 3: {
        for (int col = m_DestClip.left; col < m_DestClip.right; ++col) {
          unsigned char* src_scan =
              m_pInterBuf.get() + (col - m_DestClip.left) * DestBpp;
          int dest_a = 0;
          for (int j = pWeights->m_SrcStart; j <= pWeights->m_SrcEnd; ++j) {
      ...
      case 4: {
        for (int col = m_DestClip.left; col < m_DestClip.right; ++col) {
          ...
          unsigned char* src_scan_mask =
              m_pExtraAlphaBuf.get() + (col - m_DestClip.left);
          int dest_a = 0;
          int dest_k = 0;
          for (int j = pWeights->m_SrcStart; j <= pWeights->m_SrcEnd; ++j) {
            int* pWeight = table.GetValueFromPixelWeight(pWeights, j);
      ...
      case 8: {
        for (int col = m_DestClip.left; col < m_DestClip.right; ++col) {
          ...
          if (m_DestFormat != FXDIB_Argb)
            src_scan_mask = m_pExtraAlphaBuf.get() + (col - m_DestClip.left);
          int dest_a = 0;
          int dest_r_y = 0;
          int dest_g_m = 0;
          int dest_b_c = 0;
    ...
    m_pDestBitmap->ComposeScanline(row - m_DestClip.top, m_pDestScanline.get(),
                                   m_pDestMaskScanline.get());
  }
}
