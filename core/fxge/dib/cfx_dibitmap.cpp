...
void CFX_DIBitmap::RGBConvert(uint32_t forecolor, uint32_t backcolor) {
...
void CFX_DIBitmap::CMYKConvert(uint32_t forecolor, uint32_t backcolor) {
...
bool CFX_DIBitmap::ConvertColorScale(uint32_t forecolor, uint32_t backcolor) {
  ...
  if (IsCmykImage())
    CMYKConvert(forecolor, backcolor);
  else
    RGBConvert(forecolor, backcolor);
  return true;
}
