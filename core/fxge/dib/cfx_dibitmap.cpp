...
bool CFX_DIBitmap::Create(int width,
  ...
  if (pBuffer) {
    ...
    if (oomlimit >= 0 && size >= oomlimit) {
      m_pBuffer.Reset(FX_TryAlloc(uint8_t, size));
      if (!m_pBuffer)
        return false;
    } else {
...
void CFX_DIBitmap::Clear(uint32_t color) {
  ...
  switch (GetFormat()) {
    case FXDIB_1bppMask:
      memset(m_pBuffer.Get(), (color & 0xff000000) ? 0xff : 0,
             m_Pitch * m_Height);
      break;
    case FXDIB_1bppRgb: {
      int index = FindPalette(color);
      memset(m_pBuffer.Get(), index ? 0xff : 0, m_Pitch * m_Height);
      break;
    }
    case FXDIB_8bppMask:
      memset(m_pBuffer.Get(), color >> 24, m_Pitch * m_Height);
      break;
    case FXDIB_8bppRgb: {
      int index = FindPalette(color);
      memset(m_pBuffer.Get(), index, m_Pitch * m_Height);
      break;
    }
    case FXDIB_Rgb:
    case FXDIB_Rgba: {
      ...
      if (r == g && g == b) {
        memset(m_pBuffer.Get(), r, m_Pitch * m_Height);
      } else {
        int byte_pos = 0;
        for (int col = 0; col < m_Width; col++) {
          m_pBuffer.Get()[byte_pos++] = b;
          m_pBuffer.Get()[byte_pos++] = g;
          m_pBuffer.Get()[byte_pos++] = r;
        }
        for (int row = 1; row < m_Height; row++) {
          memcpy(m_pBuffer.Get() + row * m_Pitch, m_pBuffer.Get(), m_Pitch);
        }
      }
      break;
    }
    ...
    case FXDIB_Argb: {
      ...
      for (int i = 0; i < m_Width; i++)
        reinterpret_cast<uint32_t*>(m_pBuffer.Get())[i] = color;
      for (int row = 1; row < m_Height; row++)
        memcpy(m_pBuffer.Get() + row * m_Pitch, m_pBuffer.Get(), m_Pitch);
      break;
    }
