...
bool CFX_BitmapComposer::SetInfo(int width,
  ...
  if (m_bVertical) {
    m_pScanlineV.reset(FX_Alloc(uint8_t, m_pBitmap->GetBPP() / 8 * width + 4));
    m_pClipScanV.reset(FX_Alloc(uint8_t, m_pBitmap->GetHeight()));
    if (m_pBitmap->m_pAlphaMask)
      m_pScanlineAlphaV.reset(FX_Alloc(uint8_t, width + 4));
  }
  if (m_BitmapAlpha < 255) {
    m_pAddClipScan.reset(FX_Alloc(
...
void CFX_BitmapComposer::DoCompose(uint8_t* dest_scan,
  ...
  uint8_t* pAddClipScan = m_pAddClipScan.get();
  if (m_BitmapAlpha < 255) {
    if (clip_scan) {
...
void CFX_BitmapComposer::ComposeScanlineV(int line,
  ...
  uint8_t* src_scan = m_pScanlineV.get();
  ...
  uint8_t* src_alpha_scan = m_pScanlineAlphaV.get();
  ...
  DoCompose(m_pScanlineV.get(), scanline, m_DestHeight, clip_scan,
            scan_extra_alpha, m_pScanlineAlphaV.get());
  src_scan = m_pScanlineV.get();
  dest_scan = dest_buf;
  for (int i = 0; i < m_DestHeight; ++i) {
    for (int j = 0; j < Bpp; ++j)
  ...
  src_alpha_scan = m_pScanlineAlphaV.get();
