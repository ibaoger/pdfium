...
void Outline_CheckEmptyContour(OUTLINE_PARAMS* param) {
  auto& points = param->m_pPath->GetPoints();

  if (param->m_PointCount >= 2 &&
      points[param->m_PointCount - 2].IsTypeAndOpen(FXPT_TYPE::MoveTo) &&
      points[param->m_PointCount - 2].m_PointX ==
          points[param->m_PointCount - 1].m_PointX &&
      points[param->m_PointCount - 2].m_PointY ==
          points[param->m_PointCount - 1].m_PointY) {
    param->m_PointCount -= 2;
  }
  if (param->m_PointCount >= 4 &&
      points[param->m_PointCount - 4].IsTypeAndOpen(FXPT_TYPE::MoveTo) &&
      points[param->m_PointCount - 3].IsTypeAndOpen(FXPT_TYPE::BezierTo) &&
      points[param->m_PointCount - 3].m_PointX ==
          points[param->m_PointCount - 4].m_PointX &&
      points[param->m_PointCount - 3].m_PointY ==
          points[param->m_PointCount - 4].m_PointY &&
      points[param->m_PointCount - 2].m_PointX ==
          points[param->m_PointCount - 4].m_PointX &&
      points[param->m_PointCount - 2].m_PointY ==
          points[param->m_PointCount - 4].m_PointY &&
  ...
  points.resize(param->m_PointCount);
}
...
CFX_PathData* CFX_Font::LoadGlyphPathImpl(uint32_t glyph_index,
  ...
  if (params.m_PointCount)
    pPath->GetPoints().back().m_CloseFigure = true;
