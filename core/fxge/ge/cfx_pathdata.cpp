...
void UpdateLineJoinPoints(CFX_FloatRect& rect,
  ...
  FX_FLOAT end_len = 0;
  FX_FLOAT end_dc = 0;
  FX_FLOAT one_twenty = 1.0f / 20;

  bool bStartVert = FXSYS_fabs(start_pos.x - mid_pos.x) < one_twenty;
  bool bEndVert = FXSYS_fabs(mid_pos.x - end_pos.x) < one_twenty;
  ...
  if (FXSYS_fabs(start_k - end_k) < one_twenty) {
