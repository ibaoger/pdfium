...
void CFX_PathData::AppendRect(FX_FLOAT left,
  ...
  pPoints[1].m_Type = pPoints[2].m_Type = pPoints[3].m_Type = FXPT_TYPE::LineTo;
...
CFX_FloatRect CFX_PathData::GetBoundingBox(FX_FLOAT line_width,
  ...
  while (iPoint < m_PointCount) {
    if (m_pPoints[iPoint].m_Type == FXPT_TYPE::MoveTo &&
        !m_pPoints[iPoint].m_CloseFigure) {
      iStartPoint = iPoint + 1;
      iEndPoint = iPoint;
      bJoin = false;
    } else {
      if (m_pPoints[iPoint].m_Type == FXPT_TYPE::BezierTo &&
          !m_pPoints[iPoint].m_CloseFigure) {
      ...
      if (iPoint == m_PointCount - 1 ||
          (m_pPoints[iPoint + 1].m_Type == FXPT_TYPE::MoveTo &&
           !m_pPoints[iPoint + 1].m_CloseFigure)) {
        iStartPoint = iPoint - 1;
        iEndPoint = iPoint;
        bJoin = false;
      } else {
