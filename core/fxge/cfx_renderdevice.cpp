...
void MergeGammaAdjust(uint8_t src, int channel, int alpha, uint8_t* dest) {
  *dest = FXDIB_ALPHA_MERGE(*dest, channel,
                            CalcAlpha(g_TextGammaAdjust[src], alpha));
}
...
void ApplyDestAlpha(uint8_t back_alpha,
  ...
  ApplyAlpha(dest, b, g, r, alpha_ratio);
  dest[3] = dest_alpha;
}

void NormalizeRgbDst(int src_value, int r, int g, int b, int a, uint8_t* dest) {
  ApplyAlpha(dest, b, g, r, CalcAlpha(g_TextGammaAdjust[src_value], a));
}

void NormalizeRgbSrc(int src_value, int r, int g, int b, int a, uint8_t* dest) {
  int src_alpha = CalcAlpha(g_TextGammaAdjust[src_value], a);
  if (src_alpha == 0)
    return;
...
void NormalizeArgbDest(int src_value,
  ...
  NormalizeArgb(src_value, r, g, b, a, dest,
                CalcAlpha(g_TextGammaAdjust[src_value], a));
}
