...
TEST(cpdf_nametree, AddIntoNames) {
  ...
  EXPECT_FALSE(
      nameTree.AddValueAndName(L"2.txt", pdfium::MakeUnique<CPDF_Number>(111)));

  // Insert in the beginning of the names array.
  EXPECT_TRUE(
      nameTree.AddValueAndName(L"1.txt", pdfium::MakeUnique<CPDF_Number>(111)));
  ...
  EXPECT_STREQ("7.txt", pNames->GetStringAt(4).c_str());
  EXPECT_EQ(777, pNames->GetIntegerAt(5));

  // Insert in the middle of the names array.
  EXPECT_TRUE(
      nameTree.AddValueAndName(L"5.txt", pdfium::MakeUnique<CPDF_Number>(555)));
  EXPECT_STREQ("1.txt", pNames->GetStringAt(0).c_str());
  EXPECT_EQ(111, pNames->GetIntegerAt(1));
  EXPECT_STREQ("2.txt", pNames->GetStringAt(2).c_str());
  EXPECT_EQ(222, pNames->GetIntegerAt(3));
  ...
  EXPECT_TRUE(
      nameTree.AddValueAndName(L"9.txt", pdfium::MakeUnique<CPDF_Number>(999)));
  EXPECT_STREQ("1.txt", pNames->GetStringAt(0).c_str());
  EXPECT_EQ(111, pNames->GetIntegerAt(1));
  EXPECT_STREQ("2.txt", pNames->GetStringAt(2).c_str());
  EXPECT_EQ(222, pNames->GetIntegerAt(3));
...
TEST(cpdf_nametree, AddIntoKids) {
  ...
  EXPECT_FALSE(
      nameTree.AddValueAndName(L"9.txt", pdfium::MakeUnique<CPDF_Number>(444)));

  // Add a name within the limits of a leaf node.
  EXPECT_TRUE(
      nameTree.AddValueAndName(L"4.txt", pdfium::MakeUnique<CPDF_Number>(444)));

  // Add a name that requires changing the limits of two bottom levels.
  EXPECT_TRUE(
      nameTree.AddValueAndName(L"6.txt", pdfium::MakeUnique<CPDF_Number>(666)));

  // Add a name that requires changing the limits of two top levels.
  EXPECT_TRUE(
      nameTree.AddValueAndName(L"99.txt", pdfium::MakeUnique<CPDF_Number>(99)));
  ...
  EXPECT_STREQ("1.txt", pLimits->GetStringAt(0).c_str());
