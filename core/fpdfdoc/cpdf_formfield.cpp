...
void CPDF_FormField::SyncFieldFlags() {
  CFX_ByteString type_name =
      FPDF_GetFieldAttr(m_pDict.Get(), "FT")
          ? FPDF_GetFieldAttr(m_pDict.Get(), "FT")->GetString()
          : CFX_ByteString();
  uint32_t flags = FPDF_GetFieldAttr(m_pDict.Get(), "Ff")
                       ? FPDF_GetFieldAttr(m_pDict.Get(), "Ff")->GetInteger()
                       : 0;
  m_Flags = 0;
