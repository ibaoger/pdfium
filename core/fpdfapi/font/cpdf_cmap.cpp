...
int CheckFourByteCodeRange(uint8_t* codes,
  ...
  for (size_t iSeg = ranges.size(); iSeg > 0; iSeg--) {
    if (ranges[iSeg - 1].m_CharSize < size)
      continue;
    FX_STRSIZE iChar = 0;
    while (iChar < size) {
      if (codes[iChar] < ranges[iSeg - 1].m_Lower[iChar] ||
          codes[iChar] > ranges[iSeg - 1].m_Upper[iChar]) {
        break;
      }
      ++iChar;
    }
    if (iChar == ranges[iSeg - 1].m_CharSize)
      return 2;
    if (iChar)
      return (size == ranges[iSeg - 1].m_CharSize) ? 2 : 1;
  }
  return 0;
}
