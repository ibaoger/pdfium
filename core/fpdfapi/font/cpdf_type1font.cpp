...
void CPDF_Type1Font::LoadGlyphMap() {
  ...
  if (!IsEmbedded() && (m_Base14Font < 12) && m_Font.IsTTFont()) {
    ...
    for (int charcode = 0; charcode < 256; charcode++) {
      ...
      CalcExtGID(charcode);
#endif
      if (!m_GlyphIndex[charcode] && !strcmp(name, ".notdef")) {
  ...
  if (bCoreText) {
    ...
    for (int charcode = 0; charcode < 256; charcode++) {
      ...
      if (strcmp(name, ".notdef") && strcmp(name, "space")) {
        m_GlyphIndex[charcode] = FXFT_Get_Char_Index(
            m_Font.GetFace(),
  ...
  bool bUnicode = !FXFT_Select_Charmap(m_Font.GetFace(), FXFT_ENCODING_UNICODE);
  for (int charcode = 0; charcode < 256; charcode++) {
