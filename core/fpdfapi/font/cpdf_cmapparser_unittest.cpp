...
TEST(cpdf_cmapparser, CMap_GetCodeRange) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(5), range.m_CharSize);

  EXPECT_TRUE(
      CPDF_CMapParser::CMap_GetCodeRange(range, "<12345678>", "<87654321>"));
  EXPECT_EQ(static_cast<FX_STRSIZE>(4), range.m_CharSize);
  {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(1), range.m_CharSize);
