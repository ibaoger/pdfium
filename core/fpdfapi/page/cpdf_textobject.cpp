...
int CPDF_TextObject::CountItems() const {
  return pdfium::CollectionSize<int>(m_pCharCodes);
}

void CPDF_TextObject::GetItemInfo(int index, CPDF_TextObjectItem* pInfo) const {
  pInfo->m_CharCode = m_pCharCodes[index];
...
int CPDF_TextObject::CountChars() const {
  if (m_pCharCodes.size() == 1)
    return 1;

  int count = 0;
  for (uint32_t charcode : m_pCharCodes) {
...
void CPDF_TextObject::GetCharInfo(int index,
  ...
  if (m_pCharCodes.size() == 1) {
    *charcode = m_pCharCodes[0];
    *kerning = 0;
    return;
  }
  int count = 0;
  for (int i = 0; i < pdfium::CollectionSize<int>(m_pCharCodes); ++i) {
    if (m_pCharCodes[i] == CPDF_Font::kInvalidCharCode)
      continue;
    if (count++ != index)
      continue;
    *charcode = m_pCharCodes[i];
    if (i == pdfium::CollectionSize<int>(m_pCharCodes) - 1 ||
...
void CPDF_TextObject::SetSegments(const CFX_ByteString* pStrs,
  ...
  nChars += nsegs - 1;
  m_pCharCodes.resize(nChars);
  m_pCharPos.resize(nChars - 1);
  int index = 0;
  for (int i = 0; i < nsegs; ++i) {
