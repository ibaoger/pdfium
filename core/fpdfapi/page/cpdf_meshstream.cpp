...
CFX_PointF CPDF_MeshStream::ReadCoords() {
  ...
  CFX_PointF pos;
  if (m_nCoordBits == 32) {
    pos.x = m_xmin + static_cast<FX_FLOAT>(m_BitStream.GetBits(m_nCoordBits) *
                                           (m_xmax - m_xmin) /
                                           static_cast<double>(m_CoordMax));
    pos.y = m_ymin + static_cast<FX_FLOAT>(m_BitStream.GetBits(m_nCoordBits) *
                                           (m_ymax - m_ymin) /
                                           static_cast<double>(m_CoordMax));
  } else {
    pos.x = m_xmin +
...
CPDF_MeshVertex CPDF_MeshStream::ReadVertex(uint32_t* flag,
                                            const CFX_Matrix& pObject2Bitmap) {
  *flag = ReadFlag();

  CPDF_MeshVertex vertex;
  vertex.origin = pObject2Bitmap.Transform(ReadCoords());
...
bool CPDF_MeshStream::ReadVertexRow(CPDF_MeshVertex* vertex,
                                    int count,
                                    const CFX_Matrix& pObject2Bitmap) {
  for (int i = 0; i < count; i++) {
    ...
    vertex[i].origin = pObject2Bitmap.Transform(ReadCoords());
