...
void CPDF_StreamContentParser::Handle_ClosePath() {
  if (m_pPathPoints.empty())
    return;

  if (m_PathStartX != m_PathCurrentX || m_PathStartY != m_PathCurrentY) {
    AddPathPoint(m_PathStartX, m_PathStartY, FXPT_TYPE::LineTo, true);
  } else if (m_pPathPoints.back().m_Type != FXPT_TYPE::MoveTo) {
    m_pPathPoints.back().m_CloseFigure = true;
  }
}
...
void CPDF_StreamContentParser::AddPathPoint(float x,
                                            ...
                                            FXPT_TYPE type,
                                            bool close) {
  // If the path point is the same as the previous one and neither of them
  // closes the path, then just skip it.
  if (!close && !m_pPathPoints.empty() && !m_pPathPoints.back().m_CloseFigure &&
      m_pPathPoints.back().m_Type == type && m_PathCurrentX == x &&
      m_PathCurrentY == y) {
    return;
  }
  ...
  if (type == FXPT_TYPE::MoveTo && !close) {
    ...
    m_PathStartY = y;
    if (!m_pPathPoints.empty() &&
        m_pPathPoints.back().IsTypeAndOpen(FXPT_TYPE::MoveTo)) {
      m_pPathPoints.back().m_Point = CFX_PointF(x, y);
      return;
    }
  } else if (m_pPathPoints.empty()) {
    return;
  }
  m_pPathPoints.push_back(FX_PATHPOINT(CFX_PointF(x, y), type, close));
}

void CPDF_StreamContentParser::AddPathObject(int FillType, bool bStroke) {
  std::vector<FX_PATHPOINT> PathPoints;
  PathPoints.swap(m_pPathPoints);
