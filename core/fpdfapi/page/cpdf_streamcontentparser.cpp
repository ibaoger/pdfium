...
void CPDF_StreamContentParser::AddPathPoint(FX_FLOAT x,
  ...
  if (type == FXPT_TYPE::MoveTo && !close) {
    ...
    m_PathStartY = y;
    if (m_PathPointCount &&
        m_pPathPoints[m_PathPointCount - 1].m_Type == FXPT_TYPE::MoveTo &&
...
void CPDF_StreamContentParser::AddPathObject(int FillType, bool bStroke) {
  ...
  if (PathPointCount &&
      m_pPathPoints[PathPointCount - 1].m_Type == FXPT_TYPE::MoveTo &&
      !m_pPathPoints[PathPointCount - 1].m_CloseFigure) {
    PathPointCount--;
  }
  CPDF_Path Path;
