...
TEST(CPDF_ObjectWalkerTest, ArrayIndex) {
  auto array = pdfium::MakeUnique<CPDF_Array>();
  // Add 5 number objects, wich value equals to its index.
  for (int i = 0; i < 5; ++i) {
    array->AddNew<CPDF_Number>(i);
  }

  CPDF_ObjectWalker walker(array.get());
  while (const CPDF_Object* obj = walker.GetNext()) {
    if (obj == array.get()) {
      // Ignore root array object
      continue;
    }
    // Test that, index is correct.
    EXPECT_EQ(obj->GetInteger(), static_cast<int>(walker.array_index()));
  }
}
