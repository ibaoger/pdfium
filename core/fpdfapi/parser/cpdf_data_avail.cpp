...
std::unique_ptr<CPDF_Document> CPDF_DataAvail::TryParseDocument(
    const char* password,
    CPDF_Parser::Error* error) {
  ASSERT(error);
  if (!m_pDocument)
    *error = ParseDocumentInternal(password);

  if (!m_pDocument)
    return nullptr;

  if (m_pDocument.IsOwned()) {
    std::unique_ptr<CPDF_Document> result = m_pDocument.Release();
    m_pDocument.Reset(result.get());
    *error = CPDF_Parser::SUCCESS;
    return result;
  }

  // We already returned parsed document.
  *error = CPDF_Parser::HANDLER_ERROR;
  return nullptr;
}
