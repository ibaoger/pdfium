...
class SubobjectIterator {
 ...
 protected:
  explicit SubobjectIterator(const CPDF_Object* object) : object_(object) {
    ASSERT(object_);
  }
...
class StreamIterator : public SubobjectIterator {
 public:
...
class DictionaryIterator : public SubobjectIterator {
 public:
...
class ArrayIterator : public SubobjectIterator {
 public:
...
// static
std::unique_ptr<internal::SubobjectIterator> CPDF_ObjectWalker::MakeIterator(
    const CPDF_Object* object) {
  if (object->IsStream())
    return pdfium::MakeUnique<internal::StreamIterator>(object->AsStream());
  if (object->IsDictionary())
    return pdfium::MakeUnique<internal::DictionaryIterator>(
        object->AsDictionary());
  if (object->IsArray())
    return pdfium::MakeUnique<internal::ArrayIterator>(object->AsArray());
  return nullptr;
}
...
const CPDF_Object* CPDF_ObjectWalker::GetNext() {
  while (!stack_.empty() || next_object_) {
    ...
    internal::SubobjectIterator* it = stack_.top().get();
    if (it->IsFinished()) {
      stack_.pop();
    } else {
      ...
      dict_key_ =
          parent_object_->IsDictionary()
              ? static_cast<internal::DictionaryIterator*>(it)->dict_key()
              : CFX_ByteString();
