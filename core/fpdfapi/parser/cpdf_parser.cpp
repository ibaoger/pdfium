...
class CPDF_Parser::TrailersData {
 public:
  TrailersData() {}
  ~TrailersData() {}
  ...
  void AddMainTrailer(std::unique_ptr<CPDF_Dictionary> trailer) {
    ASSERT(trailer);
    main_trailer_ = std::move(trailer);
    ApplyTrailer(main_trailer_.get());
  }

  void AddTrailer(std::unique_ptr<CPDF_Dictionary> trailer) {
    ASSERT(trailer);
...
CPDF_Parser::CPDF_Parser()
    : m_pSyntax(pdfium::MakeUnique<CPDF_SyntaxParser>()),
      ...
      m_TrailersData(pdfium::MakeUnique<TrailersData>()),
...
bool CPDF_Parser::LoadAllCrossRefV4(FX_FILESIZE xrefpos) {
  ...
  m_TrailersData->AddMainTrailer(std::move(trailer));
  ...
  while (xrefpos) {
    ...
    m_TrailersData->AddTrailer(std::move(pDict));
  }
                ...
                if (pObj->IsDictionary() || pObj->AsStream()) {
                  ...
                  if (CPDF_Dictionary* pTrailer =
                    ...
                    if (GetTrailer()) {
                      ...
                      if (pObj->IsStream())
                        m_TrailersData->AddMainTrailer(
                            ToDictionary(pTrailer->Clone()));
                      else
                        m_TrailersData->AddMainTrailer(
                            ToDictionary(std::move(pObj)));
...
bool CPDF_Parser::LoadCrossRefV5(FX_FILESIZE* pos, bool bMainXRef) {
  ...
  *pos = pDict->GetIntegerFor("Prev");
  ...
  if (bMainXRef) {
    m_TrailersData->AddMainTrailer(std::move(pNewTrailer));
    ShrinkObjectMap(size);
    for (auto& it : m_ObjectInfo)
      it.second.type = ObjectType::kFree;
  } else {
    m_TrailersData->AddTrailer(std::move(pNewTrailer));
  }
...
CPDF_Dictionary* CPDF_Parser::GetTrailer() const {
  return m_TrailersData->GetMainTrailer();
}

uint32_t CPDF_Parser::GetInfoObjNum() {
  return m_TrailersData->GetInfoObjNum();
}
