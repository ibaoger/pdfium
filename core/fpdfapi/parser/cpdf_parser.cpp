...
CPDF_Parser::Error CPDF_Parser::SetEncryptHandler() {
  ReleaseEncryptHandler();
  if (m_TrailerPos == CPDF_Parser::kInvalidPos)
    return FORMAT_ERROR;
...
bool CPDF_Parser::LoadAllCrossRefV4(FX_FILESIZE xrefpos) {
  ...
  AddTrailer(std::move(trailer), true);
  ...
  while (xrefpos) {
    ...
    AddTrailer(std::move(pDict), false);
  }
...
bool CPDF_Parser::RebuildCrossRef() {
                  ...
                  if (CPDF_Dictionary* pDict = pStream->GetDict()) {
                    if ((pDict->KeyExist("Type")) &&
                      ...
                      if (pRoot && pRoot->GetDict() &&
                        ...
                        AddTrailer(ToDictionary(pDict->Clone()), true);
                      }
                    }
                  }
                }
                ...
                if (pObj->IsDictionary() || pObj->AsStream()) {
                  ...
                  if (CPDF_Dictionary* pTrailer =
                    ...
                    if (GetTrailer()) {
                      ...
                      if (pObj->IsStream())
                        AddTrailer(ToDictionary(pTrailer->Clone()), true);
                      else
                        AddTrailer(ToDictionary(std::move(pObj)), true);
  ...
  return m_TrailerPos != CPDF_Parser::kInvalidPos && !m_ObjectInfo.empty();
}
...
bool CPDF_Parser::LoadCrossRefV5(FX_FILESIZE* pos, bool bMainXRef) {
  ...
  *pos = pDict->GetIntegerFor("Prev");
  ...
  if (bMainXRef) {
    AddTrailer(std::move(pNewTrailer), true);
    ShrinkObjectMap(size);
    for (auto& it : m_ObjectInfo)
      it.second.type = 0;
  } else {
    AddTrailer(std::move(pNewTrailer), false);
  }
...
void CPDF_Parser::AddTrailer(std::unique_ptr<CPDF_Dictionary> trailer,
                             bool isMain) {
