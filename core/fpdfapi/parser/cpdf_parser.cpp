...
void CPDF_Parser::GetIndirectBinary(uint32_t objnum,
  ...
  if (!m_pSyntax->ParseObject(false, nullptr, &raw_body_data))
    return;

  size = (uint32_t)raw_body_data->GetSize();
  pBuffer = FX_Alloc(uint8_t, size);
  raw_body_data->ReadBlock(pBuffer, 0, size);
...
std::unique_ptr<CPDF_Object> CPDF_Parser::ParseIndirectObjectAt(
  ...
  std::unique_ptr<CPDF_Object> result =
      m_pSyntax->ParseObject(true, &child_references);
  if (result && pObjList) {
