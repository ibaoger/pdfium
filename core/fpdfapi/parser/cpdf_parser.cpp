...
class CPDF_Parser::TrailerData {
 public:
  ...
  void MergeOnto(std::unique_ptr<TrailerData> other) {
    if (!other)
      return;

    if (other->type_ != Type::kNone && type_ != other->type_)
      type_ = Type::kMixed;
  ...
  void MergeUnderto(std::unique_ptr<TrailerData> other) {
    if (!other)
      return;

    std::swap(type_, other->type_);
    std::swap(trailer_, other->trailer_);
    MergeOnto(std::move(other));
  }
...
bool CPDF_Parser::LoadAllCrossRefV4(FX_FILESIZE xrefpos) {
  ...
  m_TrailerData->MergeOnto(pdfium::MakeUnique<TrailerData>(
