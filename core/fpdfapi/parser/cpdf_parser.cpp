...
FX_FILESIZE CPDF_Parser::ParseStartXRef() {
  ...
  m_pSyntax->SetPos(m_pSyntax->m_FileLen - m_pSyntax->m_HeaderOffset -
                    FX_ArraySize(kStartXRefKeyword) - 1);
  ...
  // Read XRef offset.
  bool bNumber;
  const CFX_ByteString xrefpos_str = m_pSyntax->GetNextWord(&bNumber);
...
bool CPDF_Parser::LoadLinearizedAllCrossRefV4(FX_FILESIZE xrefpos) {
  if (!LoadCrossRefV4(xrefpos, 0, false))
    return false;
...
CPDF_Parser::Error CPDF_Parser::LoadLinearizedMainXRefTable() {
  ...
  const uint32_t dwSaveMetadataObjnum = m_pSyntax->m_MetadataObjnum;
