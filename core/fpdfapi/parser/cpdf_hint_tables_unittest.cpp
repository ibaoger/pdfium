...
TEST_F(CPDF_HintTablesTest, PageAndGroupInfos) {
  ...
  EXPECT_EQ(2u, hint_tables->PageInfos()[0].Identifiers().size());
  ...
  EXPECT_EQ(3u, hint_tables->PageInfos()[1].Identifiers().size());
  ...
  // SharedGroupInfo
  EXPECT_EQ(6u, hint_tables->SharedGroupInfos().size());
