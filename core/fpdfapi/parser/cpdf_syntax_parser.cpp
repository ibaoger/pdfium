...
bool CPDF_SyntaxParser::GetCharAt(FX_FILESIZE pos, uint8_t& ch) {
  fxcrt::AutoRestorer<FX_FILESIZE> save_pos(&m_Pos);
  m_Pos = pos;
...
ByteString CPDF_SyntaxParser::PeekNextWord(bool* bIsNumber) {
  const fxcrt::AutoRestorer<FX_FILESIZE> save_pos(&m_Pos);
...
std::unique_ptr<CPDF_Object> CPDF_SyntaxParser::GetObjectInternal(
  ...
  fxcrt::AutoRestorer<int> restorer(&s_CurrentRecursionDepth);
