...
std::unique_ptr<CPDF_Object> CPDF_SyntaxParser::GetObject(
    ...
    bool bDecrypt) {
  const CPDF_ReadValidator::Session read_session(GetValidator().Get());
  auto result = GetObjectInternal(pObjList, objnum, gennum, bDecrypt, false);
...
std::unique_ptr<CPDF_Object> CPDF_SyntaxParser::GetObjectInternal(
  ...
  if (word == "[") {
    ...
    return (m_WordBuffer[0] == ']' || !strict_parse) ? std::move(pArray)
                                                     : nullptr;
  }
  if (word[0] == '/') {
  ...
  if (word == "<<") {
    ...
    if (m_pCryptoHandler && pDict->IsSignatureDict() && dwSignValuePos) {
      CFX_AutoRestorer<FX_FILESIZE> save_pos(&m_Pos);
...
std::unique_ptr<CPDF_Object> CPDF_SyntaxParser::GetObjectForStrict(
    ...
    uint32_t gennum) {
  const CPDF_ReadValidator::Session read_session(GetValidator().Get());
  auto result = GetObjectInternal(pObjList, objnum, gennum, false, true);
  if (GetValidator()->has_read_problems())
    return nullptr;
  return result;
}
