...
bool CPDF_SyntaxParser::GoToWholeWordStart(const ByteStringView& word) {
  const FX_FILESIZE saved_pos = GetPos();
...
bool CPDF_SyntaxParser::GoToStreamEnd() {
  ...
  FX_FILESIZE endStreamOffset = -1;
  if (GoToWholeWordStart(kEndStreamStr))
    endStreamOffset = GetPos() - saved_pos;

  SetPos(saved_pos);

  FX_FILESIZE endObjOffset = -1;
  if (GoToWholeWordStart(kEndObjStr))
    endObjOffset = GetPos() - saved_pos;
