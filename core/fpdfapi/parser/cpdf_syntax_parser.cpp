...
std::unique_ptr<CPDF_Object> CPDF_SyntaxParser::GetObjectInternal(
  ...
  const bool strict_parse = parse_type == ParseType::kStrict;
  ...
  if (word == "[") {
    ...
    return (!strict_parse || m_WordBuffer[0] == ']') ? std::move(pArray)
                                                     : nullptr;
  }
  if (word[0] == '/') {
...
std::unique_ptr<CPDF_Object> CPDF_SyntaxParser::GetObjectForStrict(
  ...
  const CPDF_ReadValidator::Session read_session(GetValidator().Get());
  auto result =
      GetObjectInternal(pObjList, objnum, gennum, false, ParseType::kStrict);
  if (GetValidator()->has_read_problems())
    return nullptr;
  return result;
}
