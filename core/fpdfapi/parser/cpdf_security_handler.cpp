...
void Revision6_Hash(const uint8_t* password,
  ...
  crypto::SHA256 sha;
  sha.Start();
  sha.Update(password, size);
  sha.Update(salt, 8);
  if (vector)
    sha.Update(vector, 48);
  ...
  while (i < 64 || i < E[iBufLen - 1] + 32) {
    ...
    if (iHash == 0) {
      crypto::SHA256::Generate(E, iBufLen, input);
    } else if (iHash == 1) {
...
bool CPDF_SecurityHandler::AES256_CheckPassword(const uint8_t* password,
  ...
  if (m_Revision >= 6) {
    ...
    sha.Start();
    sha.Update(password, size);
    sha.Update(pkey + 32, 8);
    if (bOwner)
      sha.Update(ukey.raw_str(), 48);
...
void CPDF_SecurityHandler::OnCreate(CPDF_Dictionary* pEncryptDict,
  ...
  if (m_Revision >= 5) {
    ...
    crypto::SHA256 sha;
    sha.Start();
    sha.Update((uint8_t*)&t, sizeof t);
    sha.Update(m_EncryptKey, 32);
    sha.Update((uint8_t*)"there", 5);
    sha.Finish(m_EncryptKey);
...
void CPDF_SecurityHandler::AES256_SetPassword(CPDF_Dictionary* pEncryptDict,
  ...
  if (m_Revision >= 6) {
    ...
    crypto::SHA256 sha2;
    sha2.Start();
    sha2.Update(password, size);
    sha2.Update(digest, 8);
    if (bOwner)
      sha2.Update(ukey.raw_str(), ukey.GetLength());
