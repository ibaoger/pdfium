...
bool CPDF_Creator::WriteStream(const CPDF_Object* pStream,
  ...
  if (!encoder.GetDict()->WriteDirectTo(m_Archive.get(), objnum, true,
...
bool CPDF_Creator::WriteIndirectObj(uint32_t objnum, const CPDF_Object* pObj) {
  ...
  if (pObj->IsStream()) {
    ...
    if (!WriteStream(pObj, objnum, pHandler))
      return false;
  } else if (pObj == m_pEncryptDict && !pObj->WriteTo(m_Archive.get())) {
    return false;
  } else if (!pObj->WriteDirectTo(m_Archive.get(), objnum, true,
                                  m_pCryptoHandler.Get())) {
    return false;
  }
