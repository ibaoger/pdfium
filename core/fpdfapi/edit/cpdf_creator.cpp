...
class CFX_FileBufferArchive : public IFX_ArchiveStream {
 public:
  explicit CFX_FileBufferArchive(const CFX_RetainPtr<IFX_WriteStream>& pFile);
...
bool CFX_FileBufferArchive::WriteBlock(const void* pBuf, size_t size) {
  ...
  pdfium::base::CheckedNumeric<FX_FILESIZE> o = offset_;
  o += size;
  if (!o.IsValid())
    return false;

  offset_ = o.ValueOrDie();
  return true;
}
...
int32_t OutputIndex(IFX_ArchiveStream* pFile, FX_FILESIZE offset) {
  if (!pFile->WriteByte(static_cast<uint8_t>(offset >> 24)) ||
      !pFile->WriteByte(static_cast<uint8_t>(offset >> 16)) ||
      !pFile->WriteByte(static_cast<uint8_t>(offset >> 8)) ||
      !pFile->WriteByte(static_cast<uint8_t>(offset)) || !pFile->WriteByte(0)) {
    return -1;
  }
  return 0;
}
...
CPDF_Creator::CPDF_Creator(CPDF_Document* pDoc,
    ...
    : m_pDocument(pDoc),
      ...
      m_File(pdfium::MakeUnique<CFX_FileBufferArchive>(pFile)),
      m_SavedOffset(0),
      m_iStage(-1),
      m_dwFlags(0),
      m_CurObjNum(0),
...
bool CPDF_Creator::WriteIndirectObjectToStream(const CPDF_Object* pObj) {
  if (!m_pXRefStream)
    return true;

  uint32_t objnum = pObj->GetObjNum();
  if ((m_pParser && m_pParser->GetObjectGenNum(objnum) > 0) || pObj->IsNumber())
    return true;
...
bool CPDF_Creator::WriteStream(const CPDF_Object* pStream,
  ...
  if (!WriteDirectObj(objnum, encoder.GetDict(), true) ||
      !m_File->WriteString("stream\r\n") ||
      !m_File->WriteBlock(encryptor.GetData(), encryptor.GetSize()) ||
      !m_File->WriteString("\r\nendstream")) {
    return false;
  }
...
bool CPDF_Creator::WriteIndirectObj(uint32_t objnum, const CPDF_Object* pObj) {
  if (!m_File->WriteDWord(objnum) || !m_File->WriteString(" 0 obj\r\n"))
    return false;
...
bool CPDF_Creator::WriteIndirectObj(const CPDF_Object* pObj) {
  return WriteIndirectObjectToStream(pObj) &&
         WriteIndirectObj(pObj->GetObjNum(), pObj);
}
...
bool CPDF_Creator::WriteDirectObj(uint32_t objnum,
  ...
  switch (pObj->GetType()) {
    ...
    case CPDF_Object::STRING: {
      ...
      if (!m_File->WriteString(content.AsStringC()))
        return false;
      break;
    }
    ...
    case CPDF_Object::ARRAY: {
      ...
      for (size_t i = 0; i < p->GetCount(); i++) {
        ...
        if (!pElement->IsInline()) {
          if (!m_File->WriteString(" ") ||
              !m_File->WriteDWord(pElement->GetObjNum()) ||
              !m_File->WriteString(" 0 R")) {
            return false;
          }
    ...
    case CPDF_Object::DICTIONARY: {
      ...
      for (const auto& it : *p) {
        ...
        if (!m_File->WriteString("/") ||
            !m_File->WriteString(PDF_NameEncode(key).AsStringC())) {
          return false;
        }
        ...
        if (!pValue->IsInline()) {
          if (!m_File->WriteString(" ") ||
              !m_File->WriteDWord(pValue->GetObjNum()) ||
              !m_File->WriteString(" 0 R ")) {
            return false;
          }
...
bool CPDF_Creator::WriteOldIndirectObject(uint32_t objnum) {
  ...
  m_ObjectOffsets[objnum] = m_File->CurrentOffset();
  ...
  if (m_pParser->IsVersionUpdated() || m_bSecurityChanged || bExistInMap ||
    ...
    if (object_type == 2) {
      if (m_pXRefStream) {
        ...
        if (!m_File->WriteDWord(objnum) || !m_File->WriteString(" 0 obj ") ||
            !m_File->WriteBlock(pBuffer, size) ||
            !m_File->WriteString("\r\nendobj\r\n")) {
          return false;
        }
      }
    } else {
      if (!m_File->WriteBlock(pBuffer, size) ||
          !AppendObjectNumberToXRef(objnum)) {
        return false;
      }
    }
...
int32_t CPDF_Creator::WriteDoc_Stage1() {
  ...
  if (m_iStage == 10) {
    if (!IsIncremental()) {
      ...
      if (!m_File->WriteDWord(version % 10) ||
          !m_File->WriteString("\r\n%\xA1\xB3\xC5\xD7\r\n")) {
        return -1;
      }
    ...
    if (IsOriginal() && m_SavedOffset > 0) {
      ...
      while (src_size) {
        ...
        if (!pSrcFile->ReadBlock(buffer.data(),
                                 m_File->CurrentOffset() - src_size,
                                 block_size)) {
          return -1;
        }
        if (!m_File->WriteBlock(buffer.data(), block_size))
          return -1;
...
int32_t CPDF_Creator::WriteDoc_Stage2() {
  ...
  if (m_iStage == 27) {
    if (m_pEncryptDict && m_pEncryptDict->IsInline()) {
      m_dwLastObjNum += 1;
      FX_FILESIZE saveOffset = m_File->CurrentOffset();
...
int32_t CPDF_Creator::WriteDoc_Stage3() {
  ...
  if (m_iStage == 80) {
    m_XrefStart = m_File->CurrentOffset();
    if (HasObjectStream()) {
      ...
      if (!IsIncremental() || m_pParser->GetLastXRefOffset() == 0) {
        ...
        str = pdfium::ContainsKey(m_ObjectOffsets, 1)
                  ? "xref\r\n"
                  : "xref\r\n0 1\r\n0000000000 65535 f\r\n";
        if (!m_File->WriteString(str.AsStringC()))
          return -1;

        m_CurObjNum = 1;
        m_iStage = 81;
      } else {
        if (!m_File->WriteString("xref\r\n"))
          return -1;
  ...
  if (m_iStage == 81) {
    ...
    while (i <= dwLastObjNum) {
      ...
      if (!m_File->WriteBlock(str.c_str(), str.GetLength()))
        return -1;
...
int32_t CPDF_Creator::WriteDoc_Stage4() {
  ...
  if (!HasObjectStream()) {
    ...
    if (!bXRefStream) {
      if (!m_File->WriteString("trailer\r\n<<"))
        return -1;
    } else {
      if (!m_File->WriteDWord(m_pDocument->GetLastObjNum() + 1) ||
          !m_File->WriteString(" 0 obj <<")) {
        return -1;
      }
    }
    ...
    if (m_pParser) {
      ...
      for (const auto& it : *p) {
        ...
        } else if (!pValue->WriteTo(m_File.get())) {
          return -1;
        }
      }
    } else {
      if (!m_File->WriteString("\r\n/Root ") ||
          !m_File->WriteDWord(m_pDocument->GetRoot()->GetObjNum()) ||
          !m_File->WriteString(" 0 R\r\n")) {
        return -1;
      }
      if (m_pDocument->GetInfo()) {
        if (!m_File->WriteString("/Info ") ||
            !m_File->WriteDWord(m_pDocument->GetInfo()->GetObjNum()) ||
            !m_File->WriteString(" 0 R\r\n")) {
          return -1;
        }
      }
    ...
    if (m_pEncryptDict) {
      if (!m_File->WriteString("/Encrypt"))
        return -1;

      uint32_t dwObjNum = m_pEncryptDict->GetObjNum();
      if (dwObjNum == 0)
        dwObjNum = m_pDocument->GetLastObjNum() + 1;
      if (!m_File->WriteString(" ") || !m_File->WriteDWord(dwObjNum) ||
          !m_File->WriteString(" 0 R ")) {
        return -1;
      }
    }

    if (!m_File->WriteString("/Size ") ||
        !m_File->WriteDWord(m_dwLastObjNum + (bXRefStream ? 2 : 1))) {
      return -1;
    }
    if (IsIncremental()) {
      FX_FILESIZE prev = m_pParser->GetLastXRefOffset();
      if (prev) {
        if (!m_File->WriteString("/Prev "))
          return -1;

        char offset_buf[20];
        memset(offset_buf, 0, sizeof(offset_buf));
        FXSYS_i64toa(prev, offset_buf, 10);
        if (!m_File->WriteBlock(offset_buf, FXSYS_strlen(offset_buf)))
          return -1;
      }
    }
    if (m_pIDArray) {
      if (!m_File->WriteString(("/ID")) || !m_pIDArray->WriteTo(m_File.get()))
        return -1;
    }
    if (!bXRefStream) {
      if (!m_File->WriteString(">>"))
      ...
      if (!m_File->WriteString("/W[0 4 1]/Index["))
        return -1;
      if (IsIncremental() && m_pParser && m_pParser->GetLastXRefOffset() == 0) {
        ...
        for (i = 0; i < m_dwLastObjNum; i++) {
          if (!pdfium::ContainsKey(m_ObjectOffsets, i))
            continue;
          if (!m_File->WriteDWord(i) || !m_File->WriteString(" 1 "))
            return -1;
        }
        if (!m_File->WriteString("]/Length ") ||
            !m_File->WriteDWord(m_dwLastObjNum * 5) ||
            !m_File->WriteString(">>stream\r\n")) {
          return -1;
        }
        for (i = 0; i < m_dwLastObjNum; i++) {
          ...
          if (it == m_ObjectOffsets.end())
            continue;
          OutputIndex(m_File.get(), it->second);
        }
      } else {
        size_t count = m_NewObjNumArray.size();
        size_t i = 0;
        for (i = 0; i < count; i++) {
          if (!m_File->WriteDWord(m_NewObjNumArray[i]) ||
              !m_File->WriteString(" 1 ")) {
            return -1;
          }
        }
        if (!m_File->WriteString("]/Length ") ||
            !m_File->WriteDWord(count * 5) ||
            !m_File->WriteString(">>stream\r\n")) {
          return -1;
        }
        for (i = 0; i < count; ++i)
          OutputIndex(m_File.get(), m_ObjectOffsets[m_NewObjNumArray[i]]);
      }
      if (!m_File->WriteString("\r\nendstream"))
        return -1;
    }
