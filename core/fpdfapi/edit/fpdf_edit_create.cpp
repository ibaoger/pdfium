...
uint32_t kXRefStreamMaxSize = 10000;
int kObjectStreamMaxLength = 256 * 1024;
  ...
  if (pParser) {
    ...
    for (const auto& it : *p) {
      ...
      if (!pValue->IsInline()) {
        ...
        if (CPDF_Creator::AppendObject(pValue, pFile, offset) < 0)
          return -1;
      }
    }
    if (pIDArray) {
      ...
      offset += 3;
      if (CPDF_Creator::AppendObject(pIDArray, pFile, offset) < 0)
        return -1;
    }
    return offset;
  }
...
void AppendIndex1(CFX_ByteTextBuf& buffer, FX_FILESIZE offset) {
  buffer.AppendByte(1);
  buffer.AppendByte(offset >> 24);
  buffer.AppendByte(offset >> 16);
  buffer.AppendByte(offset >> 8);
  buffer.AppendByte(offset);
  buffer.AppendByte(0);
  buffer.AppendByte(0);
}
...
void AppendIndex2(CFX_ByteTextBuf& buffer, uint32_t objnum, int32_t index) {
  buffer.AppendByte(2);
  buffer.AppendByte(objnum >> 24);
  buffer.AppendByte(objnum >> 16);
  buffer.AppendByte(objnum >> 8);
  buffer.AppendByte(objnum);
  buffer.AppendByte(index >> 8);
  buffer.AppendByte(index);
}

int32_t OutputIndex(CFX_FileBufferArchive* pFile, FX_FILESIZE offset) {
  if (pFile->AppendByte(offset >> 24) < 0)
    return -1;
  if (pFile->AppendByte(offset >> 16) < 0)
    return -1;
  if (pFile->AppendByte(offset >> 8) < 0)
    return -1;
  if (pFile->AppendByte(offset) < 0)
    return -1;
...
bool CPDF_XRefStream::GenerateXRefStream(CPDF_Creator* pCreator, bool bEOF) {
  ...
  if (bEOF) {
    ...
    if (CPDF_Dictionary* encryptDict = pCreator->GetEncryptDict()) {
      ...
      if ((len = WriteEncrypt(encryptDict, dwEncryptObjNum, pFile)) < 0) {
        return false;
      }
      pCreator->IncrementOffset(len);
    }
  ...
  if (!pCreator->IsIncremental()) {
    ...
    for (uint32_t i = 1; i < pCreator->GetLastObjectNumber() + 1; i++) {
      if (pCreator->HasObjectNumber(i))
        AppendIndex1(m_Buffer, pCreator->OffsetForObject(i));
      else
        AppendIndex0(m_Buffer, false);
    }
  } else {
...
// static
int32_t CPDF_Creator::AppendObject(const CPDF_Object* pObj,
                                   CFX_FileBufferArchive* pFile,
                                   FX_FILESIZE& offset) {
  int32_t len = 0;
  if (!pObj) {
  ...
  switch (pObj->GetType()) {
    ...
    case CPDF_Object::DICTIONARY: {
      if (!m_pCryptoHandler || pObj == m_pEncryptDict)
        return CPDF_Creator::AppendObject(pObj, &m_File, m_Offset);
      if (m_File.AppendString("<<") < 0)
        return -1;
...
int32_t CPDF_Creator::WriteDoc_Stage4() {
  ...
  if ((m_dwFlags & FPDFCREATE_OBJECTSTREAM) == 0) {
    ...
    if (m_pParser) {
      ...
      for (const auto& it : *p) {
        ...
        if (!pValue->IsInline()) {
          ...
          FX_FILESIZE offset = 0;
          if (CPDF_Creator::AppendObject(pValue, &m_File, offset) < 0)
            return -1;
        }
      }
    } else {
    ...
    if (m_pIDArray) {
      ...
      FX_FILESIZE offset = 0;
      if (CPDF_Creator::AppendObject(m_pIDArray.get(), &m_File, offset) < 0)
        return -1;
    }
    if (!bXRefStream) {
      if (m_File.AppendString(">>") < 0)
