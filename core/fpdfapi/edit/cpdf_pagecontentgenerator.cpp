  ...
  *buf << pPathObj->m_Matrix << " cm ";
  ...
  if (pPathObj->m_Path.IsRect()) {
    ...
    *buf << pPoints[0].m_Point.x << " " << pPoints[0].m_Point.y << " " << diff.x
    ...
    char buffer[absl::numbers_internal::kSixDigitsToBufferSize];
    for (size_t i = 0; i < pPoints.size(); i++) {
      if (i > 0)
        *buf << " ";

      absl::numbers_internal::SixDigitsToBuffer(pPoints[i].m_Point.x, buffer);
      *buf << buffer << " ";
      absl::numbers_internal::SixDigitsToBuffer(pPoints[i].m_Point.y, buffer);
      *buf << buffer;
