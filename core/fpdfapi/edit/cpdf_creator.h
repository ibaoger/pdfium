...
class CPDF_Creator {
 public:
  ...
  void IncrementOffset(FX_FILESIZE inc) { m_Offset += inc; }
  ...
  FX_FILESIZE OffsetForObject(uint32_t objnum) {
    ASSERT(HasObjectNumber(objnum));
    return m_ObjectOffsets[objnum];
  }
