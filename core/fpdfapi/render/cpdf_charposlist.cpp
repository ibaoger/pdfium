...
void CPDF_CharPosList::Load(const std::vector<uint32_t>& pCharCodes,
                            const std::vector<FX_FLOAT>& pCharPos,
                            CPDF_Font* pFont,
                            FX_FLOAT FontSize) {
  int nChars = pdfium::CollectionSize<int>(pCharCodes);
  m_pCharPos = FX_Alloc(FXTEXT_CHARPOS, nChars);
  m_nChars = 0;
  CPDF_CIDFont* pCIDFont = pFont->AsCIDFont();
  bool bVertWriting = pCIDFont && pCIDFont->IsVertWriting();
  for (int iChar = 0; iChar < nChars; iChar++) {
    uint32_t CharCode = pCharCodes[iChar];
