  ...
  if (bTextClip) {
    ...
    for (uint32_t i = 0; i < pPageObj->m_ClipPath.GetTextCount(); i++) {
      ...
      CPDF_TextRenderer::DrawTextPath(
          &text_device, textobj->m_pCharCodes, textobj->m_pCharPos,
...
bool CPDF_RenderStatus::ProcessText(CPDF_TextObject* textobj,
  ...
  if (textobj->m_pCharCodes.size() == 0)
    return true;
  ...
  if (bClip || bStroke) {
    ...
    return CPDF_TextRenderer::DrawTextPath(
        m_pDevice, textobj->m_pCharCodes, textobj->m_pCharPos, pFont, font_size,
...
bool CPDF_RenderStatus::ProcessType3Text(CPDF_TextObject* textobj,
  ...
  if (device_class == FXDC_DISPLAY)
    glyphs.resize(textobj->m_pCharCodes.size());
  else if (fill_alpha < 255)
    return false;

  CPDF_RefType3Cache refTypeCache(pType3Font);
  for (int iChar = 0;
       iChar < pdfium::CollectionSize<int>(textobj->m_pCharCodes); iChar++) {
    uint32_t charcode = textobj->m_pCharCodes[iChar];
...
void CPDF_RenderStatus::DrawTextPathWithPattern(const CPDF_TextObject* textobj,
  ...
  CharPosList.Load(textobj->m_pCharCodes, textobj->m_pCharPos, pFont,
                   font_size);
