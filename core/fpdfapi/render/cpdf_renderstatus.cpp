...
void DrawGouraud(CFX_DIBitmap* pBitmap,
  ...
  FX_FLOAT min_y = triangle[0].origin.y;
  FX_FLOAT max_y = triangle[0].origin.y;
  for (int i = 1; i < 3; i++) {
    min_y = std::min(min_y, triangle[i].origin.y);
    max_y = std::max(max_y, triangle[i].origin.y);
  }
  if (min_y == max_y)
    return;
  ...
  for (int y = min_yi; y <= max_yi; y++) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      CFX_PointF& origin1 = vertex1.origin;
      CFX_PointF& origin2 = vertex2.origin;
      bool bIntersect =
          GetScanlineIntersect(y, origin1, origin2, &inter_x[nIntersects]);
      if (!bIntersect)
        continue;

      FX_FLOAT y_dist = (y - origin1.y) / (origin2.y - origin1.y);
...
void DrawFreeGouraudShading(
  ...
  while (!stream.BitStream()->IsEOF()) {
    uint32_t flag;
    CPDF_MeshVertex vertex = stream.ReadVertex(&flag, *pObject2Bitmap);
    if (flag == 0) {
      triangle[0] = vertex;
      for (int j = 1; j < 3; j++) {
        uint32_t tflag;
        triangle[j] = stream.ReadVertex(&tflag, *pObject2Bitmap);
      }
    } else {
      if (flag == 1)
...
void DrawLatticeGouraudShading(
  ...
  if (!stream.ReadVertexRow(vertex.get(), row_verts, *pObject2Bitmap))
    return;
  ...
  while (1) {
    ...
    if (!stream.ReadVertexRow(this_row, row_verts, *pObject2Bitmap))
      return;
