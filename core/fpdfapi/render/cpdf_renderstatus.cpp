...
void DrawAxialShading(CFX_DIBitmap* pBitmap,
  ...
  float pResults[std::max(16U, total_results)];
...
void CPDF_RenderStatus::DrawShading(CPDF_ShadingPattern* pPattern,
  ...
  if (!pPattern->IsShadingObject() && pDict->KeyExist("Background")) {
    ...
    if (pBackColor &&
      ...
      float comps[std::max(16U, pColorSpace->CountComponents())];
...
std::unique_ptr<CFX_DIBitmap> CPDF_RenderStatus::LoadSMask(
  ...
  if (bLuminosity) {
    ...
    if (pBC) {
      ...
      if (pCS) {
        ...
        float pFloats[std::max(8U, comps)];
  ...
  if (pFunc) {
    float results[std::max(16U, pFunc->CountOutputs())];
    for (int i = 0; i < 256; i++) {
