...
// Get the bitmap of |image_object|. The returned bitmap will be owned by the
// user, and FPDFBitmap_Destroy() must be called on the returned bitmap when it
// is no longer needed.
//
//   image_object - handle to an image object.
//
...
// stored in the PDF without removing any filters. |buffer| is only modified if
...
// removed. |buffer| is only modified if |buflen| is longer than the length of
...
DLLEXPORT unsigned long STDCALL
FPDFImageObj_GetImageData(FPDF_PAGEOBJECT image_object,
                          void* buffer,
                          unsigned long buflen);
