...
const char* AnnotSubtypeToString(FPDF_ANNOTATION_SUBTYPE subtype) {
...
const char* PageObjectTypeToString(int type) {
  ...
  for (int i = 0; i < annot_count; ++i) {
    ...
    if (obj_count > 0) {
      ...
      for (int j = 0; j < obj_count; ++j) {
        const char* type = PageObjectTypeToString(
