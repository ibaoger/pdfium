...
bool RenderPage(const std::string& name,
  ...
  if (bitmap) {
    ...
    FPDF_RenderPageBitmap(bitmap, page.get(), 0, 0, width, height, 0,
                          FPDF_ANNOT);

    FPDF_FFLDraw(form, bitmap, page.get(), 0, 0, width, height, 0, FPDF_ANNOT);
    ...
    if (bIsLinearized) {
      ...
      if (nRet == PDF_DATA_ERROR) {
        fprintf(stderr, "Unknown error in checking if page %d is available.\n",
                i);
        FPDFDOC_ExitFormFillEnvironment(form.get());
        return;
      }
    }
