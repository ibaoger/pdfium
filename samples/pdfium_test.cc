...
void WriteAnnot(FPDF_PAGE page, const char* pdf_name, int num) {
  ...
  for (int i = 0; i < annot_count; i++) {
    ...
    len = FPDFAnnotation_GetText(annot, buffer, initial_size);
    if (len + 1 > initial_size) {
      delete[] buffer;
      buffer = new char[len + 1];
      FPDFAnnotation_GetText(annot, buffer, len + 1);
    }
    fprintf(fp, "Content: %s\n", buffer);
    fprintf(fp, "Length: %lu\n", len);
    delete[] buffer;
    buffer = new char[initial_size];
    len = FPDFAnnotation_GetText(annot, buffer, initial_size, Author);
    if (len + 1 > initial_size) {
      delete[] buffer;
      buffer = new char[len + 1];
      FPDFAnnotation_GetText(annot, buffer, len + 1);
