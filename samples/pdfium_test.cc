...
void RenderPdf(const std::string& name,
  ...
  if (options.save_attachments) {
    for (int i = 0; i < FPDFDoc_GetAttachmentCount(doc.get()); ++i) {
      ...
      unsigned long len = FPDFDoc_GetAttachmentName(doc.get(), i, nullptr, 0);
      std::vector<char> buf(len);
      FPDFDoc_GetAttachmentName(doc.get(), i, buf.data(), len);
      ...
      char saveName[256];
      int chars_formatted =
          snprintf(saveName, sizeof(saveName), "%s.attachment.%s", name.c_str(),
                   attachmentName.c_str());
      if (chars_formatted < 0 ||
          static_cast<size_t>(chars_formatted) >= sizeof(saveName)) {
        fprintf(stderr, "Filename %s is too long\n", saveName);
        continue;
      }
      ...
      len = FPDFDoc_GetAttachmentFile(doc.get(), i, nullptr, 0);
      buf.clear();
      buf.resize(len);
      FPDFDoc_GetAttachmentFile(doc.get(), i, buf.data(), len);
      fwrite(buf.data(), sizeof(char), len, fp);
      fclose(fp);
