...
void color::ConvertArrayToPWLColor(CJS_Runtime* pRuntime,
  ...
  CJS_Value value(array.GetElement(pRuntime, 0));
  ByteString sSpace = value.ToByteString(pRuntime);
  ...
  if (nArrayLen > 1) {
    value = CJS_Value(array.GetElement(pRuntime, 1));
    d1 = value.ToDouble(pRuntime);
  }

  if (nArrayLen > 2) {
    value = CJS_Value(array.GetElement(pRuntime, 2));
    d2 = value.ToDouble(pRuntime);
  }

  if (nArrayLen > 3) {
    value = CJS_Value(array.GetElement(pRuntime, 3));
    d3 = value.ToDouble(pRuntime);
  }

  if (nArrayLen > 4) {
    value = CJS_Value(array.GetElement(pRuntime, 4));
...
bool color::SetPropertyHelper(CJS_Runtime* pRuntime,
  ...
  CJS_Array array = vp.GetJSValue()->ToArray(pRuntime);
