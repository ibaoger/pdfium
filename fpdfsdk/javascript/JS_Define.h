...
void JSSpecialPropGet(const char* class_name,
  ...
  if (!pObj->GetProperty(pRuntime, propname.c_str(), &value)) {
    pRuntime->Error(JSFormatErrorString(class_name, "GetProperty",
                                        L"failed to get property"));
    return;
  }
  info.GetReturnValue().Set(value.GetJSValue()->ToV8Value(pRuntime));
}
...
void JSSpecialPropDel(const char* class_name,
  ...
  if (!pObj->DelProperty(pRuntime, propname.c_str())) {
    ...
    pRuntime->Error(JSFormatErrorString(class_name, "DelProperty",
                                        L"failed to delete property"));
  }
}
