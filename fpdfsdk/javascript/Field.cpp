...
void CJS_Field::DefineJSObjects(CFXJS_Engine* pEngine, FXJSOBJTYPE eObjType) {
  ...
  CJS_Object::DefineProps(pEngine, g_fieldObjId, PropertySpecs);
  CJS_Object::DefineMethods(pEngine, g_fieldObjId, MethodSpecs);
}
...
CJS_Return Field::getArray(CJS_Runtime* pRuntime,
  ...
  for (const auto& pStr : swSort) {
    v8::Local<v8::Object> pObj = pRuntime->NewFxDynamicObj(g_fieldObjId);
    if (pObj.IsEmpty())
      return CJS_Return(false);
