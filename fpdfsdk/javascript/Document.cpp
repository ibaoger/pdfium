...
CJS_Return Document::resetForm(
  ...
  for (int i = 0, isz = pRuntime->GetArrayLength(array); i < isz; ++i) {
    WideString swVal =
...
CJS_Return Document::submitForm(
  ...
  int aFieldsLen = aFields.IsEmpty() ? 0 : pRuntime->GetArrayLength(aFields);
  if (aFieldsLen == 0 && bEmpty) {
  ...
  for (int i = 0, sz = aFieldsLen; i < sz; ++i) {
    WideString sName =
