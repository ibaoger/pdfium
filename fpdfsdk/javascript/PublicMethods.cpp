...
v8::Local<v8::Array> CJS_PublicMethods::AF_MakeArrayFromList(
  ...
  v8::Local<v8::Array> StrArray;
  while (*p) {
    const char* pTemp = strchr(p, ch);
    if (!pTemp) {
      if (StrArray.IsEmpty())
        StrArray = pRuntime->NewArray();
...
CJS_Return CJS_PublicMethods::AFSimple_Calculate(
  ...
  int nFieldsCount = 0;
  int fieldNameLen =
      FieldNameArray.IsEmpty() ? 0 : pRuntime->GetArrayLength(FieldNameArray);
  for (int i = 0, isz = fieldNameLen; i < isz; i++) {
...
CJS_Return CJS_PublicMethods::AFExtractNums(
  ...
  if (!nums.IsEmpty() && pRuntime->GetArrayLength(nums) > 0)
