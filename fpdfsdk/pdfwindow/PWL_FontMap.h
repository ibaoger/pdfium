...
inline CPWL_FontMap_Data::CPWL_FontMap_Data() = default;
inline CPWL_FontMap_Data::~CPWL_FontMap_Data() = default;
...
class CPWL_FontMap : public IPVT_FontMap {
 public:
  ...
  // IPVT_FontMap
  CFX_RetainPtr<CPDF_Font> GetPDFFont(int32_t nFontIndex) override;
 ...
 protected:
  ...
  virtual CFX_RetainPtr<CPDF_Font> FindFontSameCharset(
      CFX_ByteString& sFontAlias,
      int32_t nCharset);
  virtual void AddedFont(const CFX_RetainPtr<CPDF_Font>& pFont,
  ...
  int32_t GetFontIndex(const CFX_ByteString& sFontName,
                       int32_t nCharset,
                       bool bFind);
  int32_t AddFontData(const CFX_RetainPtr<CPDF_Font>& pFont,
                      const CFX_ByteString& sFontAlias,
 ...
 private:
  ...
  CFX_RetainPtr<CPDF_Font> AddFontToDocument(CPDF_Document* pDoc,
                                             CFX_ByteString& sFontName,
                                             uint8_t nCharset);
  bool IsStandardFont(const CFX_ByteString& sFontName);
  CFX_RetainPtr<CPDF_Font> AddStandardFont(CPDF_Document* pDoc,
                                           CFX_ByteString& sFontName);
  CFX_RetainPtr<CPDF_Font> AddSystemFont(CPDF_Document* pDoc,
                                         CFX_ByteString& sFontName,
                                         uint8_t nCharset);
