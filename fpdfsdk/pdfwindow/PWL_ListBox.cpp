...
void CPWL_List_Notify::OnSetScrollInfoY(float fPlateMin,
  ...
  float fContentWidth = Info.fContentMax - Info.fContentMin;
  bool bShouldBeVisible = IsFloatSmaller(Info.fPlateWidth, fContentWidth);
  if (bShouldBeVisible == pScroll->IsVisible())
    return;

  pScroll->SetVisible(bShouldBeVisible);
