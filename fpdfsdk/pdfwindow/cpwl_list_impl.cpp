...
void CPWL_ListItem::SetFontMap(IPVT_FontMap* pFontMap) {
  m_pEdit->SetFontMap(pFontMap);
}

void CPWL_ListItem::SetText(const CFX_WideString& text) {
  m_pEdit->SetText(text);
}

void CPWL_ListItem::SetFontSize(float fFontSize) {
  m_pEdit->SetFontSize(fFontSize);
}

float CPWL_ListItem::GetItemHeight() const {
  return m_pEdit->GetContentRect().Height();
}

uint16_t CPWL_ListItem::GetFirstChar() const {
  CPVT_Word word;
...
CFX_WideString CPWL_ListItem::GetText() const {
...
void CPWL_ListCtrl::AddItem(const CFX_WideString& str) {
  auto pListItem = pdfium::MakeUnique<CPWL_ListItem>();
...
int32_t CPWL_ListCtrl::FindNext(int32_t nIndex, wchar_t nChar) const {
  ...
  for (int32_t i = 0; i < sz; i++) {
    ...
    if (CPWL_ListItem* pListItem = m_ListItems[nCircleIndex].get()) {
