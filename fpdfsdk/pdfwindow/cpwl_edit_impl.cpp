...
void CPWL_EditImpl::SetSel(int32_t nStartChar, int32_t nEndChar) {
  if (m_pVT->IsValid()) {
...
void CPWL_EditImpl::SetSel(const CPVT_WordPlace& begin,
...
void CPWL_EditImpl::GetSel(int32_t& nStartChar, int32_t& nEndChar) const {
  nStartChar = -1;
  nEndChar = -1;
  if (!m_pVT->IsValid())
    return;
...
CFX_WideString CPWL_EditImpl::GetSelText() const {
