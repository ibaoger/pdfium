...
class CPWLCBEditEmbeddertest : public EmbedderTest {
 protected:
  void SetUp() override {
    EmbedderTest::SetUp();
  ...
  void CreateAndInitializeFormComboboxPDF() {
    ...
    m_pFormFillEnv = static_cast<CPDFSDK_FormFillEnvironment*>(form_handle());
    {
      ...
      ASSERT_EQ(CPDF_Annot::Subtype::WIDGET,
                m_pAnnotEditable->GetAnnotSubtype());
    }
  }

  void FormFillerAndWindowSetup(bool user_editable) {
    CFFL_InteractiveFormFiller* pInteractiveFormFiller =
        m_pFormFillEnv->GetInteractiveFormFiller();
    {
      CPDFSDK_Annot::ObservedPtr pObserved(user_editable ? m_pAnnotEditable
                                                         : m_pAnnotNormal);
      EXPECT_TRUE(pInteractiveFormFiller->OnSetFocus(&pObserved, 0));
    }

    m_pFormFiller = pInteractiveFormFiller->GetFormFiller(
        user_editable ? m_pAnnotEditable : m_pAnnotNormal, false);
  ...
  CPWL_ComboBox* GetCPWLComboBox() { return m_pComboBox; }
  CFFL_FormFiller* GetCFFLFormFiller() { return m_pFormFiller; }
  CPDFSDK_Annot* GetCPDFSDKAnnotNormal() { return m_pAnnotNormal; }
  CPDFSDK_Annot* GetCPDFSDKAnnotUserEditable() { return m_pAnnotEditable; }
...
TEST_F(CPWLCBEditEmbeddertest, GetSelectedTextEmptyAndBasicNormalCB) {
  FormFillerAndWindowSetup(false);
  ...
  EXPECT_TRUE(!GetCFFLFormFiller()->OnChar(GetCPDFSDKAnnotNormal(), 'a', 0));
}

TEST_F(CPWLCBEditEmbeddertest, GetSelectedTextFragmentsNormalCB) {
  FormFillerAndWindowSetup(false);
  EXPECT_STREQ(L"Banana", GetCPWLComboBox()->GetText().c_str());
...
TEST_F(CPWLCBEditEmbeddertest, GetSelectedTextEmptyAndBasicEditableCB) {
  FormFillerAndWindowSetup(true);
  EXPECT_TRUE(GetCPWLComboBox()->GetText().IsEmpty());
...
TEST_F(CPWLCBEditEmbeddertest, GetSelectedTextFragmentsEditableCB) {
  FormFillerAndWindowSetup(true);
  for (int i = 0; i < 50; ++i) {
    EXPECT_TRUE(
