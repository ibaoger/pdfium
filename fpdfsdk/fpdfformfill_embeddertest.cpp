...
class FPDFFormFillEmbeddertest : public EmbedderTest, public TestSaver {
 protected:
  ...
  void SelectOption(FPDF_PAGE page, int32_t item_index, double x, double y) {
    ...
    double option_y = y;
    for (int32_t i = 0; i <= item_index; ++i) {
      option_y -= 15;
    }
...
TEST_F(FPDFFormFillEmbeddertest, GetSelectedTextEmptyAndBasicKeyboard) {
  ...
  TypeTextIntoTextfield(page, 3, false, 120.0, 120.0);
...
TEST_F(FPDFFormFillEmbeddertest, GetSelectedTextFragmentsKeyBoard) {
  ...
  TypeTextIntoTextfield(page, 12, false, 120.0, 120.0);
...
TEST_F(FPDFFormFillEmbeddertest, GetSelectedTextEmptyAndBasicNormalCB) {
...
TEST_F(FPDFFormFillEmbeddertest,
       GetSelectedTextEmptyAndBasicEditableCBKeyboard) {
  // Open file with form comboboxes.
  EXPECT_TRUE(OpenDocument("combobox_form.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);
  ...
  TypeTextIntoTextfield(page, 3, true, 102.0, 62.0);
...
TEST_F(FPDFFormFillEmbeddertest, GetSelectedTextEmptyAndBasicEditableCBMouse) {
  // Open file with form comboboxes.
  EXPECT_TRUE(OpenDocument("combobox_form.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);
...
TEST_F(FPDFFormFillEmbeddertest, GetSelectedTextFragmentsNormalCB) {
  // Open file with form comboboxes.
  EXPECT_TRUE(OpenDocument("combobox_form.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);
...
TEST_F(FPDFFormFillEmbeddertest, GetSelectedTextFragmentsEditableCBKeyboard) {
  // Open file with form comboboxes.
  EXPECT_TRUE(OpenDocument("combobox_form.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);

  TypeTextIntoTextfield(page, 10, true, 102.0, 62.0);
...
TEST_F(FPDFFormFillEmbeddertest, GetSelectedTextFragmentsEditableCBMouse) {
  // Open file with form comboboxes.
  EXPECT_TRUE(OpenDocument("combobox_form.pdf"));
  FPDF_PAGE page = LoadPage(0);
  ASSERT_TRUE(page);
