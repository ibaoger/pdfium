...
class FPDFFormFillTextFormEmbeddertest : public FPDFFormFillEmbeddertest {
 protected:
  ...
  virtual const char* GetDocumentName() const {
  ...
  virtual int GetFormType() const { return FPDF_FORMFIELD_TEXTFIELD; }
 ...
 protected:
  ...
  void FormSanityChecks() override {
    EXPECT_EQ(FPDF_FORMFIELD_COMBOBOX,
              FPDFPage_HasFormFieldAtPoint(form_handle(), page_, 102.0, 113.0));
  }
...
TEST_F(FPDFFormFillTextFormEmbeddertest, DeleteTextFieldEntireSelection) {
  ...
  FORM_ReplaceSelection(form_handle(), page_, nullptr);
...
TEST_F(FPDFFormFillTextFormEmbeddertest, InsertTextInEmptyTextField) {
  ...
  FORM_ReplaceSelection(form_handle(), page_, text_to_insert.get());
