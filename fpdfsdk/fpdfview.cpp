...
FS_MATRIX GetRenderToBitmapMatrix(double scale_x,
  ...
  switch (rotate) {
    case (0):
      ...
      c = 0;
      d = -scale_y;
      offset_x = -scale_x * mask_box.left;
      offset_y = scale_y * mask_box.top;
      break;
    case (1):
      a = 0;
      b = scale_x;
    ...
    case (2):
      ...
      c = 0;
      d = scale_y;
      offset_x = scale_x * mask_box.right;
      offset_y = -scale_y * mask_box.bottom;
      break;
    case (3):
      a = 0;
      b = -scale_x;
...
void FPDF_RenderMaskToBitmap(FPDF_BITMAP* pBitmap,
  ...
  *pBitmap = FPDFBitmap_Create(size_x_bm, size_y_bm, 1);
  ...
  CFX_DIBitmap* pCFXBitmap = CFXBitmapFromFPDFBitmap(*pBitmap);
  pDevice->Attach(pCFXBitmap, !!(flags & FPDF_REVERSE_BYTE_ORDER), nullptr,
                  false);

  CFX_Matrix transform_matrix = pPage->GetPageMatrix();
  transform_matrix.Concat(CFX_Matrix(fs_matrix.a, fs_matrix.b, fs_matrix.c,
                                     fs_matrix.d, fs_matrix.e, fs_matrix.f));

  CFX_FloatRect clipping_rect = {0.0f, 0.0f, static_cast<float>(size_x_bm),
                                 static_cast<float>(size_y_bm)};
...
DLLEXPORT void STDCALL FPDF_RenderPage(HDC dc,
  ...
  if (bHasMask) {
    ...
    for (size_t i = 0; i < mask_boxes.size(); i++) {
      // Render to bitmap
      FPDF_RenderMaskToBitmap(&bitmaps[i], i, page, mask_boxes[i], start_x,
                              start_y, size_x, size_y, rotate, flags);
    }
    // Begin rendering. Add flag to indicate the renderer should pause after
    // each image mask.
    CPDF_PageRenderContext* pContext = new CPDF_PageRenderContext;
    pPage->SetRenderContext(pdfium::WrapUnique(pContext));
