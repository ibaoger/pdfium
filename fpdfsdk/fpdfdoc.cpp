...
FPDF_EXPORT FPDF_BOOKMARK CDECL
...
FPDF_EXPORT unsigned long CDECL FPDFBookmark_GetTitle(FPDF_BOOKMARK pDict,
                                                      void* buffer,
...
FPDF_EXPORT FPDF_BOOKMARK CDECL FPDFBookmark_Find(FPDF_DOCUMENT document,
...
FPDF_EXPORT FPDF_DEST CDECL FPDFBookmark_GetDest(FPDF_DOCUMENT document,
                                                 FPDF_BOOKMARK pDict) {
  if (!pDict)
    return nullptr;
  CPDF_Document* pDoc = CPDFDocumentFromFPDFDocument(document);
...
FPDF_EXPORT FPDF_ACTION CDECL FPDFBookmark_GetAction(FPDF_BOOKMARK pDict) {
  if (!pDict)
    return nullptr;
  CPDF_Bookmark bookmark(ToDictionary(static_cast<CPDF_Object*>(pDict)));
...
FPDF_EXPORT unsigned long CDECL FPDFAction_GetType(FPDF_ACTION pDict) {
  if (!pDict)
...
FPDF_EXPORT FPDF_DEST CDECL FPDFAction_GetDest(FPDF_DOCUMENT document,
                                               FPDF_ACTION pDict) {
  if (!pDict)
    return nullptr;
  CPDF_Document* pDoc = CPDFDocumentFromFPDFDocument(document);
...
FPDF_EXPORT unsigned long CDECL FPDFAction_GetFilePath(FPDF_ACTION pDict,
                                                       void* buffer,
                                                       unsigned long buflen) {
...
FPDF_EXPORT unsigned long CDECL FPDFAction_GetURIPath(FPDF_DOCUMENT document,
                                                      FPDF_ACTION pDict,
                                                      void* buffer,
                                                      unsigned long buflen) {
  if (!pDict)
...
FPDF_EXPORT unsigned long CDECL FPDFDest_GetPageIndex(FPDF_DOCUMENT document,
                                                      FPDF_DEST pDict) {
  if (!pDict)
    return 0;
  CPDF_Document* pDoc = CPDFDocumentFromFPDFDocument(document);
...
FPDF_EXPORT FPDF_BOOL CDECL FPDFDest_GetLocationInPage(FPDF_DEST pDict,
                                                       FPDF_BOOL* hasXVal,
                                                       FPDF_BOOL* hasYVal,
...
FPDF_EXPORT FPDF_LINK CDECL FPDFLink_GetLinkAtPoint(FPDF_PAGE page,
                                                    double x,
                                                    double y) {
...
FPDF_EXPORT int CDECL FPDFLink_GetLinkZOrderAtPoint(FPDF_PAGE page,
                                                    double x,
                                                    double y) {
  CPDF_Page* pPage = CPDFPageFromFPDFPage(page);
  if (!pPage)
...
FPDF_EXPORT FPDF_DEST CDECL FPDFLink_GetDest(FPDF_DOCUMENT document,
                                             FPDF_LINK pDict) {
  if (!pDict)
    return nullptr;
  CPDF_Document* pDoc = CPDFDocumentFromFPDFDocument(document);
...
FPDF_EXPORT FPDF_ACTION CDECL FPDFLink_GetAction(FPDF_LINK pDict) {
  if (!pDict)
    return nullptr;
...
FPDF_EXPORT FPDF_BOOL CDECL FPDFLink_Enumerate(FPDF_PAGE page,
                                               int* startPos,
                                               FPDF_LINK* linkAnnot) {
...
FPDF_EXPORT FPDF_BOOL CDECL FPDFLink_GetAnnotRect(FPDF_LINK linkAnnot,
                                                  FS_RECTF* rect) {
  if (!linkAnnot || !rect)
    return false;
...
FPDF_EXPORT int CDECL FPDFLink_CountQuadPoints(FPDF_LINK linkAnnot) {
  if (!linkAnnot)
    return 0;
  CPDF_Dictionary* pAnnotDict =
      ToDictionary(static_cast<CPDF_Object*>(linkAnnot));
...
FPDF_EXPORT FPDF_BOOL CDECL FPDFLink_GetQuadPoints(FPDF_LINK linkAnnot,
                                                   int quadIndex,
...
FPDF_EXPORT unsigned long CDECL FPDF_GetMetaText(FPDF_DOCUMENT document,
                                                 FPDF_BYTESTRING tag,
                                                 void* buffer,
                                                 unsigned long buflen) {
  if (!tag)
...
FPDF_EXPORT unsigned long CDECL FPDF_GetPageLabel(FPDF_DOCUMENT document,
                                                  int page_index,
                                                  void* buffer,
                                                  unsigned long buflen) {
  if (page_index < 0)
