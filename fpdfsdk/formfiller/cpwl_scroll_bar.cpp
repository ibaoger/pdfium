...
#include "fpdfsdk/formfiller/cpwl_draw_helpers.h"
...
void CPWL_SBButton::DrawThisAppearance(CFX_RenderDevice* pDevice,
  ...
  CPWL_DrawHelpers::DrawStrokeRect(pDevice, pUser2Device, rcDraw,
  ...
  if (m_eSBButtonType != PSBT_POS) {
    ...
    if (IsEnabled()) {
      CPWL_DrawHelpers::DrawShadow(pDevice, pUser2Device, true, false, rcDraw,
                                   nTransparency, 80, 220);
    } else {
      CPWL_DrawHelpers::DrawFillRect(pDevice, pUser2Device, rcDraw,
    ...
    if (rectWnd.top - rectWnd.bottom > 6.0f) {
      ...
      CPWL_DrawHelpers::DrawFillArea(
          pDevice, pUser2Device, pts.data(), 7,
          IsEnabled() ? ArgbEncode(nTransparency, 255, 255, 255)
  ...
  if (IsEnabled()) {
    ...
    for (auto* it = std::begin(refs); it < std::end(refs); ++it) {
      CPWL_DrawHelpers::DrawStrokeLine(pDevice, pUser2Device, ptTop, ptBottom,
                                       *it, 1.0f);
  ...
  for (size_t i = 0; i < 3; ++i) {
    CPWL_DrawHelpers::DrawStrokeLine(pDevice, pUser2Device, ptLeft, ptRight,
                                     crStroke, 1.0f);
    ptLeft.y += 2.0f;
    ptRight.y += 2.0f;
  }
...
void CPWL_ScrollBar::DrawThisAppearance(CFX_RenderDevice* pDevice,
  ...
  if (IsVisible() && !rectWnd.IsEmpty()) {
    CPWL_DrawHelpers::DrawFillRect(pDevice, pUser2Device, rectWnd,
