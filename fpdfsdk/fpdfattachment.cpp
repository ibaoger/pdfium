...
FPDFDoc_GetAttachmentDate(FPDF_DOCUMENT document,
  ...
  if (type != FPDFATTACHMENT_DATETYPE_Creation &&
      type != FPDFATTACHMENT_DATETYPE_Modification) {
    return 0;
  }
...
FPDFDoc_GetAttachmentCheckSum(FPDF_DOCUMENT document,
  ...
  CPDF_String* stringValue = pParamsDict->GetObjectFor(key)->AsString();
  CFX_WideString value =
      stringValue->IsHex()
          ? CPDF_String(nullptr,
                        PDF_EncodeString(stringValue->GetString(), true), false)
                .GetUnicodeText()
