...
DLLEXPORT FPDF_BOOL STDCALL FPDFAnnot_UpdatePathObject(FPDF_ANNOTATION annot,
  ...
  if (!pAnnot || !pAnnot->GetAnnotDict() || !pPathObj || !pPathObj->IsPath())
    return false;
  ...
  // Check that the annotation already has a form object set up, since an
...
DLLEXPORT FPDF_BOOL STDCALL FPDFAnnot_AppendPathObject(FPDF_ANNOTATION annot,
  ...
  // Check that the path object did not come from the same annotation.
