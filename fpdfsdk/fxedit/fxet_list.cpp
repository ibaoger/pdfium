...
void CPLST_Select::Add(int32_t nItemIndex) {
  ...
  if (nIndex < 0) {
    m_aItems.push_back(pdfium::MakeUnique<CPLST_Select_Item>(nItemIndex, 1));
    return;
  }
  m_aItems[nIndex]->nState = 1;
}
...
int32_t CPLST_Select::GetCount() const {
  return pdfium::CollectionSize<int32_t>(m_aItems);
}

int32_t CPLST_Select::GetItemIndex(int32_t nIndex) const {
  if (!pdfium::IndexInBounds(m_aItems, nIndex))
    return -1;

  return m_aItems[nIndex]->nItemIndex;
}
...
void CPLST_Select::Done() {
  m_aItems.erase(
      std::remove_if(m_aItems.begin(), m_aItems.end(),
                     [](const std::unique_ptr<CPLST_Select_Item>& pItem) {
