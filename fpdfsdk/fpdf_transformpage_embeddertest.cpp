...
TEST_F(FPDFTransformPageEmbeddertest, FPDFTransFromPageWithNullptr) {
  ASSERT_TRUE(OpenDocument("rectangles.pdf"));
  FPDF_PAGE page = LoadPage(0);
  EXPECT_NE(nullptr, page);
  EXPECT_EQ(0, FPDFPage_TransFormWithClip(page, nullptr, nullptr));
  UnloadPage(page);
}

TEST_F(FPDFTransformPageEmbeddertest, FPDF_TransFormPageWithClip) {
  const char kRotatedMD5[] = "0a90de37f52127619c3dfb642b5fa2fe";
  ASSERT_TRUE(OpenDocument("rectangles.pdf"));
  FPDF_PAGE page = LoadPage(0);
  EXPECT_NE(nullptr, page);
  ...
  EXPECT_EQ(1, FPDFPage_TransFormWithClip(page, &matrix, &cliprect));
