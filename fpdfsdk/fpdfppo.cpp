...
#include "third_party/base/nup.h"
...
class CPDF_PageOrganizer {
 public:
  ...
  bool ExportNPage(const std::vector<uint16_t>& pageNums, int nIndex, int nup);
  void AddSubPage(CPDF_Dictionary* pPageDict,
                  ...
                  bool landscape,
                  ByteString& content,
                  const CFX_FloatRect* clipRect = nullptr);
  CPDF_Object* CPDFPage_XObject(CPDF_Dictionary* pSrcPageDict,
 ...
 private:
  ...
  float pageWidth;
  float pageHeight;
  std::map<ByteString, CPDF_Object*> xobjs;
};

CPDF_PageOrganizer::CPDF_PageOrganizer(CPDF_Document* pDestPDFDoc,
                                       CPDF_Document* pSrcPDFDoc)
    : m_pDestPDFDoc(pDestPDFDoc), m_pSrcPDFDoc(pSrcPDFDoc), xoNum(0) {}
...
void CPDF_PageOrganizer::AddSubPage(CPDF_Dictionary* pPageDict,
  ...
  ByteString xoName = ByteString::Format("X%d", ++xoNum);
  ...
  if (landscape) {
    ...
    matrix.Translate(pageWidth, 0.0);
  }
  xobjs[xoName] = CPDFPage_XObject(pPageDict, m_pDestPDFDoc.Get());
...
CPDF_Object* CPDF_PageOrganizer::CPDFPage_XObject(CPDF_Dictionary* pSrcPageDict,
                                                  CPDF_Document* pDestDoc) {
...
bool CPDF_PageOrganizer::ExportNPage(const std::vector<uint16_t>& pageNums,
                                     int nIndex,
                                     int nup) {
  int curpage = nIndex;
  auto pObjNumberMap = pdfium::MakeUnique<ObjectNumberMap>();
  ...
  if (nup == 1) {
    ExportPage(pageNums, nIndex);
  } else {
    NupParameters nupParam;
    nupParam.width = pageWidth;
    nupParam.height = pageHeight;
    NupParameters::preset(nup, nupParam);
    Rotation rotate = ROT_0;
    if (nupParam.landscape) {
      std::swap(nupParam.width, nupParam.height);
      rotate = rotate + ROT_90;
    }
    NupState nupState(nupParam);
    NupPageEdit pgEdit;
    for (int i = 0; i < nSize; i++) {
      // If number of pages on a page = nup, create a new page.
      bool newPage = nupState.nextPage(pageWidth, pageHeight, pgEdit);
      if (newPage) {
        if (i > 0) {
      ...
      AddSubPage(pSrcPageDict, pgEdit.xpos, pgEdit.ypos, pgEdit.xscale,
                 pgEdit.yscale, rotate, nupParam.landscape, content);
      ++inPages;
    }
    ...
    //   const ByteString key = it.first;
    //   CPDF_Object* pObj = it.second;
    //   pPageXObject->SetNewFor<CPDF_Reference>(key,
    //                                           m_pDestPDFDoc.Get(),
    //                                           pObj->GetObjNum());
    // }
...
FPDF_EXPORT FPDF_BOOL FPDF_CALLCONV FPDF_ImportNPages(FPDF_DOCUMENT dest_doc,
  ...
  CPDF_PageOrganizer pageOrg(pDestDoc, pSrcDoc);
  return pageOrg.PDFDocInit() && pageOrg.ExportNPage(pageArray, index, nup);
}
