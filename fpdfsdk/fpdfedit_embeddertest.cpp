...
TEST_F(FPDFEditEmbeddertest, AddPaths) {
  ...
  FPDF_PATHPOINTOBJECT point = FPDFPath_GetPathPointObject(green_rect, 0);
  float x;
  float y;
  EXPECT_TRUE(FPDFPathPoint_GetPoint(point, &x, &y));
  EXPECT_EQ(100, x);
  EXPECT_EQ(100, y);
  EXPECT_EQ(FPDF_PATHPOINTOBJ_MOVETO, FPDFPathPoint_GetType(point));
  EXPECT_FALSE(FPDFPathPoint_GetClose(point));
  point = FPDFPath_GetPathPointObject(green_rect, 1);
  EXPECT_TRUE(FPDFPathPoint_GetPoint(point, &x, &y));
  EXPECT_EQ(100, x);
  EXPECT_EQ(140, y);
  EXPECT_EQ(FPDF_PATHPOINTOBJ_LINETO, FPDFPathPoint_GetType(point));
  EXPECT_FALSE(FPDFPathPoint_GetClose(point));
  point = FPDFPath_GetPathPointObject(green_rect, 2);
  EXPECT_TRUE(FPDFPathPoint_GetPoint(point, &x, &y));
  EXPECT_EQ(140, x);
  EXPECT_EQ(140, y);
  EXPECT_EQ(FPDF_PATHPOINTOBJ_LINETO, FPDFPathPoint_GetType(point));
  EXPECT_FALSE(FPDFPathPoint_GetClose(point));
  point = FPDFPath_GetPathPointObject(green_rect, 3);
  EXPECT_TRUE(FPDFPathPoint_GetPoint(point, &x, &y));
  EXPECT_EQ(140, x);
  EXPECT_EQ(100, y);
  EXPECT_EQ(FPDF_PATHPOINTOBJ_LINETO, FPDFPathPoint_GetType(point));
  EXPECT_FALSE(FPDFPathPoint_GetClose(point));
  point = FPDFPath_GetPathPointObject(green_rect, 4);
  EXPECT_TRUE(FPDFPathPoint_GetPoint(point, &x, &y));
  EXPECT_EQ(100, x);
  EXPECT_EQ(100, y);
  EXPECT_EQ(FPDF_PATHPOINTOBJ_LINETO, FPDFPathPoint_GetType(point));
  EXPECT_TRUE(FPDFPathPoint_GetClose(point));
  ...
  // Verify actual coordinates.
  point = FPDFPath_GetPathPointObject(black_path, 0);
  EXPECT_TRUE(FPDFPathPoint_GetPoint(point, &x, &y));
  EXPECT_EQ(400, x);
  EXPECT_EQ(100, y);
  EXPECT_EQ(FPDF_PATHPOINTOBJ_MOVETO, FPDFPathPoint_GetType(point));
  EXPECT_FALSE(FPDFPathPoint_GetClose(point));
  point = FPDFPath_GetPathPointObject(black_path, 1);
  EXPECT_TRUE(FPDFPathPoint_GetPoint(point, &x, &y));
  EXPECT_EQ(400, x);
  EXPECT_EQ(200, y);
  EXPECT_EQ(FPDF_PATHPOINTOBJ_LINETO, FPDFPathPoint_GetType(point));
  EXPECT_FALSE(FPDFPathPoint_GetClose(point));
  point = FPDFPath_GetPathPointObject(black_path, 2);
  EXPECT_TRUE(FPDFPathPoint_GetPoint(point, &x, &y));
  EXPECT_EQ(300, x);
  EXPECT_EQ(100, y);
  EXPECT_EQ(FPDF_PATHPOINTOBJ_LINETO, FPDFPathPoint_GetType(point));
  ...
  EXPECT_EQ(nullptr, FPDFPath_GetPathPointObject(black_path, 3));
...
TEST_F(FPDFEditEmbeddertest, PathsPoints) {
  ...
  // FPDFPath_GetPathPointObject() with a non-path.
  ASSERT_EQ(nullptr, FPDFPath_GetPathPointObject(img, 0));
  // FPDFPath_GetPathPointObject() with a NULL path.
  ASSERT_EQ(nullptr, FPDFPath_GetPathPointObject(nullptr, 0));
  float x;
  float y;
  // FPDFPathPoint_GetPoint() with a NULL point.
  EXPECT_FALSE(FPDFPathPoint_GetPoint(nullptr, &x, &y));

  // FPDFPathPoint_GetType() with a NULL point.
  ASSERT_EQ(FPDF_PATHPOINTOBJ_UNKNOWN, FPDFPathPoint_GetType(nullptr));

  // FPDFPathPoint_GetClose() with a NULL point.
  EXPECT_FALSE(FPDFPathPoint_GetClose(nullptr));
