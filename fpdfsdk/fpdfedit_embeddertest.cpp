...
// Linux only test because it used NanumGothic.
#if _FXM_PLATFORM_ == _FXM_PLATFORM_LINUX_
TEST_F(FPDFEditEmbeddertest, AddCIDFontText) {
  // Start with a blank page
  FPDF_PAGE page = FPDFPage_New(CreateNewDocument(), 0, 612, 792);
  {
    // First, get the data from NanumGothic font
    CFX_SubstFont dummy;
    FXFT_Face fontFace = CFX_GEModule::Get()->GetFontMgr()->FindSubstFont(
        "NanumBarunGothic", 1, 0, 400, 0, 949, &dummy);
    EXPECT_EQ("NanumBarunGothic", FXFT_Get_Postscript_Name(fontFace));
    const uint8_t* data = FXFT_Get_Face_Stream_Base(fontFace);
    const uint32_t size = FXFT_Get_Face_Stream_Size(fontFace);
    ...
    // And add some Hangul characters
    FPDF_PAGEOBJECT text_object2 =
        FPDFPageObj_CreateTextObj(document(), font, 18.0f);
    ASSERT_TRUE(text_object2);
    std::wstring wstr2 =
        L"Korean: 나는 한국어가 너무 재미 있기 때문에 한국어로 쓰고 있습니다.";
    std::unique_ptr<unsigned short, pdfium::FreeDeleter> text2 =
  ...
  EXPECT_TRUE(FPDFPage_GenerateContent(page));
  FPDF_BITMAP page_bitmap = RenderPage(page);
  const char md5[] = "4af56a073d416199f4b3ff51b1170b98";
  CompareBitmap(page_bitmap, 612, 792, md5);
  FPDFBitmap_Destroy(page_bitmap);
