...
TEST_F(FPDFEditEmbeddertest, EditOverExistingContent) {
  ...
  FPDFPage_InsertObjectOver(page, red_rect2);

  // Add an opaque rectangle on top of the existing content
  FPDF_PAGEOBJECT red_rect = FPDFPageObj_CreateNewRect(115, 700, 25, 50);
  EXPECT_TRUE(FPDFPath_SetFillColor(red_rect, 255, 0, 0, 255));
  EXPECT_TRUE(FPDFPath_SetDrawMode(red_rect, FPDF_FILLMODE_ALTERNATE, 0));
  FPDFPage_InsertObjectOver(page, red_rect);

  EXPECT_TRUE(FPDFPage_GenerateContentOver(page));
  ...
  FPDFPage_InsertObjectOver(new_page, green_rect);
  ...
  FPDFPage_InsertObjectOver(new_page, green_rect2);
  EXPECT_TRUE(FPDFPage_GenerateContentOver(new_page));
