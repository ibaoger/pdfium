...
TEST_F(FPDFEditEmbeddertest, Type1Font) {
  ...
  FPDF_FONT font = FPDFText_GetType1Font(doc, data, size);
  ASSERT_TRUE(font);
  ...
  int first = font_dict->GetIntegerFor("FirstChar");
  int last = font_dict->GetIntegerFor("LastChar");
  ASSERT_TRUE(font_dict->KeyExist("Widths"));
  EXPECT_EQ(last - first + 1,
            static_cast<int>(font_dict->GetArrayFor("Widths")->GetCount()));
  ...
  EXPECT_TRUE(font_desc->KeyExist("Flags"));
  ASSERT_TRUE(font_desc->KeyExist("FontBBox"));
  EXPECT_EQ(4,
            static_cast<int>(font_desc->GetArrayFor("FontBBox")->GetCount()));
