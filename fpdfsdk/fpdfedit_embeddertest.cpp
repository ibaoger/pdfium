...
TEST_F(FPDFEditEmbeddertest, AddCIDFontText) {
  ...
  CFX_Font nanumFont;
  {
    ...
    nanumFont.LoadSubst("NanumGothic", 1, 0, 400, 0, 949, 0);
    printf("Font loaded: %s\n", nanumFont.GetFaceName().c_str());
    EXPECT_EQ("NanumBarunGothic", nanumFont.GetFaceName());
    const uint8_t* data = nanumFont.GetFontData();
    const uint32_t size = nanumFont.GetSize();
    ...
    // TODO(npm): OMG why do I need to do this to avoid asan problem
    FPDF_FONT font2 =
        FPDFText_LoadFont(document(), data, size, FPDF_FONT_TRUETYPE, 1);
    ASSERT_TRUE(font2);

    FPDF_PAGEOBJECT text_object2 =
        FPDFPageObj_CreateTextObj(document(), font2, 18.0f);
