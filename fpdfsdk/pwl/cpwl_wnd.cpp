...
CPWL_Wnd::CREATEPARAM::CREATEPARAM()
    : rcRectWnd(0, 0, 0, 0),
...
CPWL_Wnd::CREATEPARAM::CREATEPARAM(const CREATEPARAM& other) = default;
...
void CPWL_Wnd::Create(const CREATEPARAM& cp) {
  if (IsValid())
    return;

  m_CreationParam = cp;
  OnCreate(m_CreationParam);

  m_CreationParam.rcRectWnd.Normalize();
  m_rcWindow = m_CreationParam.rcRectWnd;
  m_rcClip = m_rcWindow;
  if (!m_rcClip.IsEmpty()) {
  ...
  if (m_CreationParam.pParentWnd)
    m_CreationParam.pParentWnd->AddChild(this);

  CREATEPARAM ccp = m_CreationParam;
...
void CPWL_Wnd::OnCreate(CREATEPARAM& cp) {}
...
void CPWL_Wnd::InvalidateFocusHandler(IPWL_FocusHandler* handler) {
  if (m_CreationParam.pFocusHandler == handler)
    m_CreationParam.pFocusHandler = nullptr;
}

void CPWL_Wnd::InvalidateProvider(IPWL_Provider* provider) {
  if (m_CreationParam.pProvider.Get() == provider)
    m_CreationParam.pProvider.Reset();
}
...
void CPWL_Wnd::Destroy() {
  ...
  if (m_bCreated) {
    ...
    if (m_CreationParam.pParentWnd)
      m_CreationParam.pParentWnd->RemoveChild(this);

    m_bCreated = false;
  }
  DestroyMsgControl();
  m_CreationParam.Reset();
  m_Children.clear();
}
...
void CPWL_Wnd::Move(const CFX_FloatRect& rcNew, bool bReset, bool bRefresh) {
  ...
  m_CreationParam.rcRectWnd = m_rcWindow;
}
...
void CPWL_Wnd::InvalidateRect(CFX_FloatRect* pRect) {
  ...
  if (CFX_SystemHandler* pSH = GetSystemHandler()) {
    if (CPDFSDK_Widget* widget = static_cast<CPDFSDK_Widget*>(
            m_CreationParam.pAttachedWidget.Get())) {
...
CPWL_Wnd* CPWL_Wnd::GetParentWindow() const {
  return m_CreationParam.pParentWnd;
}
...
bool CPWL_Wnd::HasFlag(uint32_t dwFlags) const {
  return (m_CreationParam.dwFlags & dwFlags) != 0;
}

void CPWL_Wnd::RemoveFlag(uint32_t dwFlags) {
  m_CreationParam.dwFlags &= ~dwFlags;
}
...
CFX_Color CPWL_Wnd::GetBackgroundColor() const {
  return m_CreationParam.sBackgroundColor;
}

void CPWL_Wnd::SetBackgroundColor(const CFX_Color& color) {
  m_CreationParam.sBackgroundColor = color;
}

CFX_Color CPWL_Wnd::GetTextColor() const {
  return m_CreationParam.sTextColor;
}

BorderStyle CPWL_Wnd::GetBorderStyle() const {
  return m_CreationParam.nBorderStyle;
}

void CPWL_Wnd::SetBorderStyle(BorderStyle nBorderStyle) {
  if (HasFlag(PWS_BORDER))
    m_CreationParam.nBorderStyle = nBorderStyle;
}

int32_t CPWL_Wnd::GetBorderWidth() const {
  return HasFlag(PWS_BORDER) ? m_CreationParam.dwBorderWidth : 0;
}
...
CFX_Color CPWL_Wnd::GetBorderColor() const {
  return HasFlag(PWS_BORDER) ? m_CreationParam.sBorderColor : CFX_Color();
}

const CPWL_Dash& CPWL_Wnd::GetBorderDash() const {
  return m_CreationParam.sDash;
}

void* CPWL_Wnd::GetAttachedData() const {
  return m_CreationParam.pAttachedData;
}
...
void CPWL_Wnd::CreateScrollBar(const CREATEPARAM& cp) {
  CreateVScrollBar(cp);
}

void CPWL_Wnd::CreateVScrollBar(const CREATEPARAM& cp) {
...
const CPWL_Wnd* CPWL_Wnd::GetRootWnd() const {
  auto* pParent = m_CreationParam.pParentWnd;
...
void CPWL_Wnd::CreateChildWnd(const CREATEPARAM& cp) {}
...
void CPWL_Wnd::CreateMsgControl() {
  if (!m_CreationParam.pMsgControl)
    m_CreationParam.pMsgControl = new CPWL_MsgControl(this);
}
...
CPWL_MsgControl* CPWL_Wnd::GetMsgControl() const {
  return m_CreationParam.pMsgControl;
}
...
int32_t CPWL_Wnd::GetTransparency() {
  return m_CreationParam.nTransparency;
}
...
void CPWL_Wnd::SetTransparency(int32_t nTransparency) {
  ...
  m_CreationParam.nTransparency = nTransparency;
}
...
CFX_Matrix CPWL_Wnd::GetChildMatrix() const {
  return HasFlag(PWS_CHILD) ? m_CreationParam.mtChild : CFX_Matrix();
}

void CPWL_Wnd::SetChildMatrix(const CFX_Matrix& mt) {
  m_CreationParam.mtChild = mt;
}
