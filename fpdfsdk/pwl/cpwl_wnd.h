...
class CPWL_Wnd : public CPWL_TimerHandler, public CFX_Observable<CPWL_Wnd> {
 public:
  struct CREATEPARAM {
   public:
    CREATEPARAM();
    CREATEPARAM(const CREATEPARAM& other);
  ...
  void Create(const CREATEPARAM& cp);
  void Destroy();
 ...
 protected:
  ...
  virtual void CreateChildWnd(const CREATEPARAM& cp);
  virtual void RePosChildWnd();

  virtual void DrawThisAppearance(CFX_RenderDevice* pDevice,
                                  const CFX_Matrix& mtUser2Device);

  virtual void OnCreate(CREATEPARAM& cp);
  ...
  const CREATEPARAM& GetCreationParam() const { return m_CreationParam; }
 ...
 private:
  ...
  void CreateScrollBar(const CREATEPARAM& cp);
  void CreateVScrollBar(const CREATEPARAM& cp);
