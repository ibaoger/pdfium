...
void CPWL_SBButton::OnCreate(CREATEPARAM& cp) {
...
void CPWL_ScrollBar::OnCreate(CREATEPARAM& cp) {
  cp.eCursorType = FXCT_ARROW;
}
...
void CPWL_ScrollBar::CreateButtons(const CREATEPARAM& cp) {
  CREATEPARAM scp = cp;
  scp.pParentWnd = this;
  scp.dwBorderWidth = 2;
...
void CPWL_ScrollBar::CreateChildWnd(const CREATEPARAM& cp) {
  CreateButtons(cp);
}
