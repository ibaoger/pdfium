...
DLLEXPORT void STDCALL FPDFPath_SetLineJoin(FPDF_PAGEOBJECT path,
  ...
  if (lineJoin < CFX_GraphStateData::LineJoin::LineJoinMiter ||
      lineJoin > CFX_GraphStateData::LineJoin::LineJoinBevel)
    return;
...
DLLEXPORT void STDCALL FPDFPath_SetLineCap(FPDF_PAGEOBJECT path, int line_cap) {
  ...
  if (lineCap < CFX_GraphStateData::LineCap::LineCapButt ||
      lineCap > CFX_GraphStateData::LineCap::LineCapSquare)
    return;
