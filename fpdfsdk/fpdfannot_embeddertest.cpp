...
TEST_F(FPDFAnnotEmbeddertest, ExtractHighlightLongContent) {
  ...
  EXPECT_EQ(13u, FPDFAnnot_GetText(annot, FPDFANNOT_TEXTTYPE_Author, buf.data(),
                                   len + 1));
  EXPECT_STREQ("Jae Hyun Park", buf.data());

  // Check that the content is correct.
  len = FPDFAnnot_GetText(annot, FPDFANNOT_TEXTTYPE_Contents, nullptr, 0);
  buf.clear();
  buf.resize(len + 1);
  EXPECT_EQ(1344u, FPDFAnnot_GetText(annot, FPDFANNOT_TEXTTYPE_Contents,
                                     buf.data(), len + 1));
  const char contents[] =
  ...
  EXPECT_STREQ(contents, buf.data());
