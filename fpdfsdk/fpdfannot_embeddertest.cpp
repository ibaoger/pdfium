...
TEST_F(FPDFAnnotEmbeddertest, GetInteractiveFormAnnotationFlagsTextField) {
  ...
  int flags = FPDFAnnot_GetInteractiveFormFlags(annot);
  ...
  // Check that the flag values are as expected.
  flags = FPDFAnnot_GetInteractiveFormFlags(annot);
...
TEST_F(FPDFAnnotEmbeddertest, GetInteractiveFormAnnotationFlagsComboBox) {
  ...
  int flags = FPDFAnnot_GetInteractiveFormFlags(annot);
  EXPECT_FALSE(flags & FPDF_FORMFLAG_READONLY);
  EXPECT_TRUE(flags & FPDF_FORMFLAG_COMBO);
  EXPECT_TRUE(flags & FPDF_FORMFLAG_EDIT);
  FPDFPage_CloseAnnot(annot);
  ...
  EXPECT_FALSE(flags & FPDF_FORMFLAG_READONLY);
  EXPECT_TRUE(flags & FPDF_FORMFLAG_COMBO);
  EXPECT_FALSE(flags & FPDF_FORMFLAG_EDIT);
  FPDFPage_CloseAnnot(annot);
