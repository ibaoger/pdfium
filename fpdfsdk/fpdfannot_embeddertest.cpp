...
TEST_F(FPDFAnnotEmbeddertest, AddAndModifyPath) {
  ...
  FPDF_BITMAP bitmap = RenderPageWithFlags(page, FPDF_ANNOT, form_handle_);
#if _FXM_PLATFORM_ == _FXM_PLATFORM_APPLE_
  const char md5[] = "c35408717759562d1f8bf33d317483d2";
#elif _FXM_PLATFORM_ == _FXM_PLATFORM_WINDOWS_
  const char md5[] = "83d0969a82aac5e9bd05814b4d38de95";
#else
  ...
  EXPECT_FALSE(FPDFAnnot_GetPathObject(annot, 1, &path));
  EXPECT_TRUE(FPDFAnnot_GetPathObject(annot, 0, &path));

  // Modify the color of the path object.
  EXPECT_TRUE(FPDFPath_SetStrokeColor(path, 0, 0, 0, 255));
  EXPECT_TRUE(FPDFAnnot_SetPathObject(annot, path));
  FPDFPage_CloseAnnot(annot);

  // Check that the page with the modified annotation renders correctly.
  bitmap = RenderPageWithFlags(page, FPDF_ANNOT, form_handle_);
#if _FXM_PLATFORM_ == _FXM_PLATFORM_APPLE_
  const char md5_2[] = "cf3cea74bd46497520ff6c4d1ea228c8";
#elif _FXM_PLATFORM_ == _FXM_PLATFORM_WINDOWS_
  const char md5_2[] = "8b9cbe0c1e8df715a74a93e6b18eb28f";
#else
  ...
  EXPECT_TRUE(FPDFAnnot_SetPathObject(annot, check));
  EXPECT_EQ(1, FPDFAnnot_GetPathObjectCount(annot));
  ...
  bitmap = RenderPageWithFlags(new_page, FPDF_ANNOT, nullptr);
#if _FXM_PLATFORM_ == _FXM_PLATFORM_APPLE_
  const char md5_3[] = "ee5372b31fede117fc83b9384598aa25";
#elif _FXM_PLATFORM_ == _FXM_PLATFORM_WINDOWS_
  const char md5_3[] = "8cbbaa622c90d87378e958269dca538d";
#else
