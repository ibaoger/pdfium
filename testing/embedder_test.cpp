...
bool EmbedderTest::OpenDocument(const std::string& filename,
  ...
  network_simulator_ = pdfium::MakeUnique<NetworkSimulator>(&file_access_);
  return OpenDocumentHelper(password, must_linearize, network_simulator_.get(),
  ...
  *avail = FPDFAvail_Create(network_simulator->GetFileAvail(),
  ...
  if (FPDFAvail_IsLinearized(*avail) == PDF_LINEARIZED) {
    ...
    *document = FPDFAvail_GetDocument(*avail, password);
    ...
    if (must_linearize)
      return false;
    network_simulator->FlushWholeFile();
    *document =
...
int EmbedderTest::GetFirstPageNum() {
  ...
  (void)FPDFAvail_IsPageAvail(avail_, first_page,
                              network_simulator_->GetDownloadHints());
  return first_page;
}
...
void EmbedderTest::TestSaved(int width,
  ...
  saved_network_simulator_ =
      pdfium::MakeUnique<NetworkSimulator>(&saved_file_access_);

  ASSERT_TRUE(
      OpenDocumentHelper(password, false, saved_network_simulator_.get(),
                         &m_SavedDocument, &m_SavedAvail, &m_SavedForm));
...
void EmbedderTest::FlushWholeFile() {
  ASSERT(network_simulator_);
  network_simulator_->FlushWholeFile();
}
