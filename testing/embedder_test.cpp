...
int GetBitmapBytePerPixel(FPDF_BITMAP bitmap) {
...
std::string EmbedderTest::HashBitmap(FPDF_BITMAP bitmap,
  ...
  CRYPT_MD5Generate(
      ...
      expected_width * GetBitmapBytePerPixel(bitmap) * expected_height, digest);
...
void EmbedderTest::CompareBitmap(FPDF_BITMAP bitmap,
  ...
  const int expected_stride = expected_width * GetBitmapBytePerPixel(bitmap);
