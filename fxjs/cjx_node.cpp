...
void CJX_Node::Script_Som_DefaultValue(CFXJSE_Value* pValue,
  ...
  if (bSetting) {
    ...
    if (GetXFANode()->GetPacketID() == XFA_XDPPACKET_Datasets) {
      ...
      for (UnownedPtr<CXFA_Node>& pFormNode : GetXFANode()->GetBindItems()) {
    ...
    case XFA_ObjectType::ContainerNode: {
      if (XFA_FieldIsMultiListBox(GetXFANode())) {
        ...
        if (bSyncData && pBind) {
          ...
          for (UnownedPtr<CXFA_Node>& pArrayNode : pBind->GetBindItems()) {
      ...
      if (pBindNode && bSyncData) {
        pBindNode->JSNode()->SetContent(wsContent, wsXMLValue, bNotify,
                                        bScriptModify, false);
        for (UnownedPtr<CXFA_Node>& pArrayNode : pBindNode->GetBindItems()) {
          if (pArrayNode != GetXFANode()) {
            pArrayNode->JSNode()->SetContent(wsContent, wsContent, bNotify,
                                             true, false);
          }
...
std::unique_ptr<CXFA_CalcData> CJX_Node::ReleaseCalcData() {
  return pdfium::WrapUnique<CXFA_CalcData>(calc_data_.release());
}
