...
void XFA_CopyWideString(void*& pData) {
  ...
  WideString* pNewData = new WideString(*reinterpret_cast<WideString*>(pData));
  pData = pNewData;
}
...
CXFA_Node* CJX_Node::GetProperty(int32_t index,
  ...
  if (!GetXFANode()->HasProperty(eProperty) ||
      index >= GetXFANode()->PropertyOccuranceCount(eProperty))
    return nullptr;
