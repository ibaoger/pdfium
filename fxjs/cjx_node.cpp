...
WideString CJX_Node::GetAttribute(const WideStringView& attr) {
  pdfium::Optional<WideString> ret = TryAttribute(attr, true);
...
bool CJX_Node::GetBoolean(XFA_Attribute eAttr) {
  pdfium::Optional<bool> value = TryBoolean(eAttr, true);
...
int32_t CJX_Node::GetInteger(XFA_Attribute eAttr) {
  pdfium::Optional<int32_t> iValue = TryInteger(eAttr, true);
...
XFA_ATTRIBUTEENUM CJX_Node::GetEnum(XFA_Attribute eAttr) {
  pdfium::Optional<XFA_ATTRIBUTEENUM> value = TryEnum(eAttr, true);
  return value ? *value : XFA_ATTRIBUTEENUM_Unknown;
}
...
WideString CJX_Node::GetCData(XFA_Attribute eAttr) {
  pdfium::Optional<WideString> ret = TryCData(eAttr, true);
  return ret ? *ret : WideString();
}
...
WideString CJX_Node::GetContent(bool bScriptModify) {
  pdfium::Optional<WideString> ret = TryContent(bScriptModify, true);
  return ret ? *ret : WideString();
}
