...
bool CJX_Object::HasMethodFromSpecs(const CJX_MethodSpec* specs,
                                    const WideString& func) const {
  ByteString byteFunc = func.UTF8Encode();
  for (int i = 0;; ++i) {
    if (specs[i].pMethodCall == nullptr)
      break;
    if (ByteString(specs[i].pName) == byteFunc)
      return true;
  }
  return false;
}

bool CJX_Object::RunMethodFromSpecs(const CJX_MethodSpec* specs,
