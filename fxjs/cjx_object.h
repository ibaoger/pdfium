...
struct CJX_MethodSpec {
  const char* pName;
  void (*pMethodCall)(CJX_Object* obj, CFXJSE_Arguments* args);
};
...
class CJX_Object {
 public:
  ...
  virtual bool HasMethod(const WideString& func) const;
  virtual void RunMethod(const WideString& func, CFXJSE_Arguments* args);

  bool HasMethodFromSpecs(const CJX_MethodSpec* specs,
                          const WideString& func) const;
  bool RunMethodFromSpecs(const CJX_MethodSpec* specs,
