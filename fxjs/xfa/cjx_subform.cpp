...
bool CJX_Subform::HasMethod(const WideString& func) const {
  if (HasMethodFromSpecs(MethodSpecs, func))
    return true;
  return CJX_Container::HasMethod(func);
}

void CJX_Subform::RunMethod(const WideString& func, CFXJSE_Arguments* args) {
  if (RunMethodFromSpecs(MethodSpecs, func, args))
    return;
  CJX_Container::RunMethod(func, args);
}
