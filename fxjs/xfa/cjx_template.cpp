...
bool CJX_Template::HasMethod(const WideString& func) const {
  if (HasMethodFromSpecs(MethodSpecs, func))
    return true;
  return CJX_Model::HasMethod(func);
}

void CJX_Template::RunMethod(const WideString& func, CFXJSE_Arguments* args) {
  if (RunMethodFromSpecs(MethodSpecs, func, args))
    return;
  CJX_Model::RunMethod(func, args);
}
