...
class CFXJS_ObjDefinition {
 public:
  ...
  const CFX_ByteString m_ObjName;
...
int CFXJS_Engine::DefineObj(const CFX_ByteString& sObjName,
                            FXJSOBJTYPE eObjType,
                            CFXJS_Engine::Constructor pConstructor,
                            CFXJS_Engine::Destructor pDestructor) {
  ...
  for (int i = 0; i < maxID; ++i) {
    ...
    if (pObjDef->m_ObjType == FXJSOBJTYPE_GLOBAL) {
      ...
      v8::Local<v8::String> m_ObjName =
          v8::String::NewFromUtf8(m_isolate, pObjDef->m_ObjName.c_str(),
                                  v8::NewStringType::kNormal,
                                  pObjDef->m_ObjName.GetLength())
              .ToLocalChecked();
