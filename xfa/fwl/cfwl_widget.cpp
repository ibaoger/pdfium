...
void CFWL_Widget::SetThemeProvider(const IFWL_ThemeProvider* pThemeProvider) {
...
const IFWL_ThemeProvider* CFWL_Widget::GetAvailableTheme() {
  ...
  do {
    ...
    if (pUp) {
      const IFWL_ThemeProvider* pRet = pUp->GetThemeProvider();
      if (pRet)
        return pRet;
    }
  } while (pUp);
