...
void CFWL_MonthCalendar::DrawWeek(CFX_Graphics* pGraphics,
  ...
  for (int32_t i = 0; i < 7; i++) {
    rtDayOfWeek.Set(m_rtWeek.left + i * (m_szCell.width + MONTHCAL_HMARGIN * 2),
                    m_rtWeek.top, m_szCell.width, m_szCell.height);
...
void CFWL_MonthCalendar::CalcHeadSize() {
  ...
  m_rtHeadText.Set(m_rtClient.left + fHeadHMargin,
                   ...
                   m_szHead.width, m_szHead.height);
}
...
void CFWL_MonthCalendar::CalcTodaySize() {
  ...
  m_rtToday.Set(
      m_rtClient.left + MONTHCAL_HEADER_BTN_HMARGIN + m_szCell.width +
          MONTHCAL_HMARGIN * 2,
      m_rtDates.bottom() + MONTHCAL_HEADER_BTN_VMARGIN + MONTHCAL_VMARGIN,
      m_szToday.width, m_szToday.height);
}
...
void CFWL_MonthCalendar::Layout() {
  ...
  m_rtLBtn.Set(m_rtClient.left + MONTHCAL_HEADER_BTN_HMARGIN,
               m_rtClient.top + MONTHCAL_HEADER_BTN_VMARGIN, m_szCell.width,
               m_szCell.width);
