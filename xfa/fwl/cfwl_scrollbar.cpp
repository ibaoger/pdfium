...
CFX_RectF CFWL_ScrollBar::CalcMinButtonRect() {
  CFX_RectF rect;
  if (IsVertical())
    rect = CFX_RectF(m_rtClient.TopLeft(), m_rtClient.width, m_fButtonLen);
  else
    rect = CFX_RectF(m_rtClient.TopLeft(), m_fButtonLen, m_rtClient.height);
  return rect;
}
...
CFX_RectF CFWL_ScrollBar::CalcMaxButtonRect() {
  CFX_RectF rect;
  if (IsVertical()) {
    rect = CFX_RectF(m_rtClient.left, m_rtClient.bottom() - m_fButtonLen,
                     m_rtClient.width, m_fButtonLen);
  } else {
    rect = CFX_RectF(m_rtClient.right() - m_fButtonLen, m_rtClient.top,
...
CFX_RectF CFWL_ScrollBar::CalcThumbButtonRect(const CFX_RectF& rtThumb) {
  ...
  if (fRange < 0) {
    if (IsVertical()) {
      rect =
          CFX_RectF(m_rtClient.left, m_rtMaxBtn.bottom(), m_rtClient.width, 0);
    } else {
      rect =
          CFX_RectF(m_rtMaxBtn.right(), m_rtClient.top, 0, m_rtClient.height);
    }
    return rect;
  }
