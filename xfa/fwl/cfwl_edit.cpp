...
void CFWL_Edit::SetLimit(int32_t nLimit) {
  ...
  if (m_nLimit > 0) {
    m_EdtEngine.HasCharacterLimit(true);
    m_EdtEngine.SetCharacterLimit(nLimit);
  } else {
    m_EdtEngine.HasCharacterLimit(false);
  }
}
...
bool CFWL_Edit::Paste(const CFX_WideString& wsPaste) {
  if (m_EdtEngine.HasSelection()) {
    ...
    // TODO(dsinclair): What is the insert position ....
    m_EdtEngine.Insert(0, wsPaste);
  }
  return true;
}
...
void CFWL_Edit::UpdateEditParams() {
  ...
  m_EdtEngine.SetCombText(false);
  if (m_pProperties->m_dwStyleExes & FWL_STYLEEXT_EDT_CombText)
    m_EdtEngine.SetCombText(true);

  // TODO(dsinclair): What does this actually do? It seems like it just adds
  // a line to the height of the content?
...
bool CFWL_Edit::UpdateOffset() {
  // TODO(dsinclair): Handle offset update ...
...
void CFWL_Edit::UpdateVAlignment() {
  ...
  if (m_pProperties->m_dwStyleExes & FWL_STYLEEXT_EDT_VCenter) {
    fOffsetY = (m_rtEngine.height - contents_bounds.height) / 2;
...
bool CFWL_Edit::ValidateNumberChar(wchar_t cNum) {
  ...
  int32_t caretPos = 0;  // m_EdtEngine.GetCaretPos();
...
void CFWL_Edit::OnButtonDblClk(CFWL_MessageMouse* pMsg) {
  // TODO(dsinclair): Handle OnButtonDblClk
  //  CFDE_TxtEdtPage* pPage = m_EdtEngine.GetPage(0);
  //  if (!pPage)
  //    return;
  //
...
void CFWL_Edit::OnMouseMove(CFWL_MessageMouse* pMsg) {
  ...
  m_EdtEngine.SetSelection(std::min(old_cursor_pos, m_CursorPosition),
                           std::max(old_cursor_pos, m_CursorPosition));
}
...
void CFWL_Edit::OnKeyDown(CFWL_MessageKey* pMsg) {
  ...
  switch (pMsg->m_dwKeyCode) {
    case FWL_VKEY_Left:
      m_CursorPosition = m_EdtEngine.GetIndexLeft(m_CursorPosition);
      break;
    case FWL_VKEY_Right:
      m_CursorPosition = m_EdtEngine.GetIndexRight(m_CursorPosition);
      break;
    case FWL_VKEY_Up:
      m_CursorPosition = m_EdtEngine.GetIndexUp(m_CursorPosition);
      break;
    case FWL_VKEY_Down:
      m_CursorPosition = m_EdtEngine.GetIndexDown(m_CursorPosition);
      break;
    case FWL_VKEY_Home:
      m_CursorPosition =
    ...
    case FWL_VKEY_Delete: {
      ...
      if (m_CursorPosition > 0) {
        m_CursorPosition = m_EdtEngine.GetIndexBefore(m_CursorPosition);
