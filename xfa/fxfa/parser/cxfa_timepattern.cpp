// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_timepattern.h"

// static
bool CXFA_TimePattern::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_LocaleSet & packet);
}

CXFA_TimePattern::CXFA_TimePattern(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::TimePattern,
                L"timePattern") {}

CXFA_TimePattern::~CXFA_TimePattern() {}

WideStringView CXFA_TimePattern::GetName() const {
  return L"timePattern";
}

XFA_Element CXFA_TimePattern::GetElementType() const {
  return XFA_Element::TimePattern;
}
