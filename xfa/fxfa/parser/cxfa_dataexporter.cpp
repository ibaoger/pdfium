...
void RegenerateFormFile_Container(CXFA_Node* pNode,
  ...
  if (eType == XFA_Element::Field || eType == XFA_Element::Draw ||
    ...
    if (nLen > 0)
      pStream->WriteString(buf.MakeString());
    return;
  }

  CFX_WideString wsElement(pNode->GetClassName());
...
void XFA_DataExporter_RegenerateFormFile(
  ...
  if (pNode->IsModelNode()) {
    ...
    if (pChecksum) {
      ...
      pStream->WriteString(wsChecksum);
      pStream->WriteString(L"\"");
    }
    pStream->WriteString(L" xmlns=\"");

    const wchar_t* pURI = XFA_GetPacketByIndex(XFA_PACKET_Form)->pURI;
    pStream->WriteString(CFX_WideString(pURI, FXSYS_wcslen(pURI)));
    ...
    wsVersionNumber += L"/\"\n>";
    pStream->WriteString(wsVersionNumber);
