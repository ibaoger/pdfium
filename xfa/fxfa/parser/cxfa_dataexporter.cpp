  ...
  switch (pNode->GetObjectType()) {
    case XFA_ObjectType::ContentNode: {
      ...
      if (pRawValueNode->GetElementType() == XFA_Element::SharpxHTML &&
        ...
        auto pTempStream =
            pdfium::MakeRetain<CFX_BufferedStream>(pMemStream, true);
...
void RegenerateFormFile_Container(
    CXFA_Node* pNode,
    const CFX_RetainPtr<CFX_BufferedStream>& pStream,
    bool bSaveXML) {
...
bool CXFA_DataExporter::Export(const CFX_RetainPtr<IFX_SeekableStream>& pWrite,
  ...
  auto pStream = pdfium::MakeRetain<CFX_BufferedStream>(pWrite, true);
  pStream->SetCodePage(FX_CODEPAGE_UTF8);
  return Export(pStream, pNode, dwFlag, pChecksum);
}

bool CXFA_DataExporter::Export(const CFX_RetainPtr<CFX_BufferedStream>& pStream,
                               CXFA_Node* pNode,
                               uint32_t dwFlag,
                               const char* pChecksum) {
