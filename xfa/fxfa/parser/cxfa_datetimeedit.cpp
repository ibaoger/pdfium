// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_datetimeedit.h"

// static
bool CXFA_DateTimeEdit::IsValidPacket(XFA_XDPPACKET packet) {
  return !!((XFA_XDPPACKET_Template | XFA_XDPPACKET_Form) & packet);
}

CXFA_DateTimeEdit::CXFA_DateTimeEdit(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::Node,
                XFA_Element::DateTimeEdit,
                L"dateTimeEdit") {}

CXFA_DateTimeEdit::~CXFA_DateTimeEdit() {}

WideStringView CXFA_DateTimeEdit::GetName() const {
  return L"dateTimeEdit";
}

XFA_Element CXFA_DateTimeEdit::GetElementType() const {
  return XFA_Element::DateTimeEdit;
}
