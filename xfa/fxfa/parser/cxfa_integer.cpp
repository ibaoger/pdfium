// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_integer.h"

// static
bool CXFA_Integer::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(
      (XFA_XDPPACKET_SourceSet | XFA_XDPPACKET_Template | XFA_XDPPACKET_Form) &
      packet);
}

CXFA_Integer::CXFA_Integer(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::Integer,
                L"integer") {}

CXFA_Integer::~CXFA_Integer() {}

WideStringView CXFA_Integer::GetName() const {
  return L"integer";
}

XFA_Element CXFA_Integer::GetElementType() const {
  return XFA_Element::Integer;
}
