...
bool CXFA_ArrayNodeList::Insert(CXFA_Node* pNewNode, CXFA_Node* pBeforeNode) {
  ...
  auto iter = std::find(m_array.begin(), m_array.end(), pBeforeNode);
  if (iter != m_array.end())
    m_array.insert(iter, pNewNode);
  return true;
}

bool CXFA_ArrayNodeList::Remove(CXFA_Node* pNode) {
  auto iter = std::find(m_array.begin(), m_array.end(), pNode);
  if (iter != m_array.end())
    m_array.erase(iter);
  return true;
}
