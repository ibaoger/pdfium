// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_effectiveoutputpolicy.h"

// static
bool CXFA_EffectiveOutputPolicy::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_ConnectionSet & packet);
}

CXFA_EffectiveOutputPolicy::CXFA_EffectiveOutputPolicy(CXFA_Document* doc,
                                                       XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::Node,
                XFA_Element::EffectiveOutputPolicy,
                L"effectiveOutputPolicy") {}

CXFA_EffectiveOutputPolicy::~CXFA_EffectiveOutputPolicy() {}

WideStringView CXFA_EffectiveOutputPolicy::GetName() const {
  return L"effectiveOutputPolicy";
}

XFA_Element CXFA_EffectiveOutputPolicy::GetElementType() const {
  return XFA_Element::EffectiveOutputPolicy;
}
