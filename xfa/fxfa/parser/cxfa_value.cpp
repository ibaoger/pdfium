// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_value.h"

namespace {

const CXFA_Node::PropertyData property_data[] = {
    {XFA_Element::Arc, 1, 0},     {XFA_Element::Text, 1, 0},
    {XFA_Element::Time, 1, 0},    {XFA_Element::DateTime, 1, 0},
    {XFA_Element::Image, 1, 0},   {XFA_Element::Decimal, 1, 0},
    {XFA_Element::Boolean, 1, 0}, {XFA_Element::Integer, 1, 0},
    {XFA_Element::ExData, 1, 0},  {XFA_Element::Rectangle, 1, 0},
    {XFA_Element::Date, 1, 0},    {XFA_Element::Float, 1, 0},
    {XFA_Element::Line, 1, 0},    {XFA_Element::Unknown, 0, 0}};
const XFA_Attribute attribute_data[] = {
    XFA_Attribute::Id,      XFA_Attribute::Use,      XFA_Attribute::Relevant,
    XFA_Attribute::Usehref, XFA_Attribute::Override, XFA_Attribute::Unknown};

const wchar_t* kName = L"value";

}  // namespace

CXFA_Value::CXFA_Value(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                (XFA_XDPPACKET_Template | XFA_XDPPACKET_Form),
                XFA_ObjectType::Node,
                XFA_Element::Value,
                property_data,
                attribute_data,
                kName) {}

CXFA_Value::~CXFA_Value() {}
