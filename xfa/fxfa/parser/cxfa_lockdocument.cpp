// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_lockdocument.h"

// static
bool CXFA_LockDocument::IsValidPacket(XFA_XDPPACKET packet) {
  return !!((XFA_XDPPACKET_Template | XFA_XDPPACKET_Form) & packet);
}

CXFA_LockDocument::CXFA_LockDocument(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::LockDocument,
                L"lockDocument") {}

CXFA_LockDocument::~CXFA_LockDocument() {}

WideStringView CXFA_LockDocument::GetName() const {
  return L"lockDocument";
}

XFA_Element CXFA_LockDocument::GetElementType() const {
  return XFA_Element::LockDocument;
}
