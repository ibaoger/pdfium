// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_packet.h"

// static
bool CXFA_Packet::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_XDP & packet);
}

CXFA_Packet::CXFA_Packet(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::NodeC,
                XFA_Element::Packet,
                L"packet") {}

CXFA_Packet::~CXFA_Packet() {}

WideStringView CXFA_Packet::GetName() const {
  return L"packet";
}

XFA_Element CXFA_Packet::GetElementType() const {
  return XFA_Element::Packet;
}
