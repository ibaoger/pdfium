// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_encoding.h"

// static
bool CXFA_Encoding::IsValidPacket(XFA_XDPPACKET packet) {
  return !!((XFA_XDPPACKET_Template | XFA_XDPPACKET_Form) & packet);
}

CXFA_Encoding::CXFA_Encoding(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::NodeC,
                XFA_Element::Encoding,
                L"encoding") {}

CXFA_Encoding::~CXFA_Encoding() {}

WideStringView CXFA_Encoding::GetName() const {
  return L"encoding";
}

XFA_Element CXFA_Encoding::GetElementType() const {
  return XFA_Element::Encoding;
}
