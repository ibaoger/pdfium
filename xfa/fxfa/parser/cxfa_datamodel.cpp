// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_datamodel.h"

// static
bool CXFA_DataModel::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_Datasets & packet);
}

CXFA_DataModel::CXFA_DataModel(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ModelNode,
                XFA_Element::DataModel,
                L"dataModel") {}

CXFA_DataModel::~CXFA_DataModel() {}

WideStringView CXFA_DataModel::GetName() const {
  return L"dataModel";
}

XFA_Element CXFA_DataModel::GetElementType() const {
  return XFA_Element::DataModel;
}
