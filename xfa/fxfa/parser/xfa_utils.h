...
class CXFA_NodeIteratorTemplate {
 public:
  ...
  NodeType* MoveToPrev() {
    if (!m_pRoot)
      return nullptr;
    if (!m_pCurrent) {
      m_pCurrent = LeftmostDescendent(m_pRoot);
      return m_pCurrent;
    }
    NodeType* pSibling = PreviousSiblingRespectingRoot(m_pCurrent);
    if (pSibling) {
      m_pCurrent = LeftmostDescendent(pSibling);
      return m_pCurrent;
    }
    NodeType* pParent = ParentRespectingRoot(m_pCurrent);
    if (pParent) {
      m_pCurrent = pParent;
      return m_pCurrent;
    }
  ...
  NodeType* SkipChildrenAndMoveToNext() {
    ...
    while (pNode) {
      NodeType* pSibling = NextSiblingRespectingRoot(pNode);
      if (pSibling) {
        m_pCurrent = pSibling;
        return m_pCurrent;
      }
      pNode = ParentRespectingRoot(pNode);
    }
    m_pCurrent = nullptr;
    return m_pCurrent;
  }
 ...
 protected:
  NodeType* ParentRespectingRoot(NodeType* pNode) {
  ...
  NodeType* NextSiblingRespectingRoot(NodeType* pNode) {
    if (pNode == m_pRoot)
      return nullptr;
    return TraverseStrategy::GetNextSibling(pNode);
  }

  NodeType* PreviousSiblingRespectingRoot(NodeType* pNode) {
    NodeType* pParent = ParentRespectingRoot(pNode);
    if (!pParent)
      return nullptr;
  ...
  NodeType* LastChild(NodeType* pNode) {
    ...
    while (pChild) {
      pPrevious = pChild;
      pChild = NextSiblingRespectingRoot(pChild);
    }
    return pPrevious;
  }

  NodeType* LeftmostDescendent(NodeType* pNode) {
    NodeType* pChild = LastChild(pNode);
    return pChild ? LeftmostDescendent(pChild) : pNode;
  }
