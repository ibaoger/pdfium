...
WideString CXFA_Node::AttributeEnumToName(XFA_AttributeEnum item) {
  const XFA_AttributeEnumINFO* e = g_XFAEnumData + static_cast<int32_t>(item);
  return e ? e->pName : L"";
}
...
CXFA_Node* CXFA_Node::GetBindData() {
  ASSERT(GetPacketID() == XFA_PacketType::Form);
...
bool CXFA_Node::HasBindItem() {
  return GetPacketID() == XFA_PacketType::Datasets &&
...
CXFA_WidgetData* CXFA_Node::GetContainerWidgetData() {
  if (GetPacketID() != XFA_PacketType::Form)
    return nullptr;
...
bool CXFA_Node::IsNeedSavingXMLNode() {
  return m_pXMLNode && (GetPacketID() == XFA_PacketType::Datasets ||
