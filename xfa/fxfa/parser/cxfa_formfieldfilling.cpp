// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_formfieldfilling.h"

// static
bool CXFA_FormFieldFilling::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_Config & packet);
}

CXFA_FormFieldFilling::CXFA_FormFieldFilling(CXFA_Document* doc,
                                             XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::FormFieldFilling,
                L"formFieldFilling") {}

CXFA_FormFieldFilling::~CXFA_FormFieldFilling() {}

WideStringView CXFA_FormFieldFilling::GetName() const {
  return L"formFieldFilling";
}

XFA_Element CXFA_FormFieldFilling::GetElementType() const {
  return XFA_Element::FormFieldFilling;
}
