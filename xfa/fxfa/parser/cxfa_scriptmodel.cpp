// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_scriptmodel.h"

// static
bool CXFA_ScriptModel::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_Config & packet);
}

CXFA_ScriptModel::CXFA_ScriptModel(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::ScriptModel,
                L"scriptModel") {}

CXFA_ScriptModel::~CXFA_ScriptModel() {}

WideStringView CXFA_ScriptModel::GetName() const {
  return L"scriptModel";
}

XFA_Element CXFA_ScriptModel::GetElementType() const {
  return XFA_Element::ScriptModel;
}
