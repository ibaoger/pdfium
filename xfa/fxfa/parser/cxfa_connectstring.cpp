// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_connectstring.h"

// static
bool CXFA_ConnectString::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_SourceSet & packet);
}

CXFA_ConnectString::CXFA_ConnectString(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::TextNode,
                XFA_Element::ConnectString,
                L"connectString") {}

CXFA_ConnectString::~CXFA_ConnectString() {}

WideStringView CXFA_ConnectString::GetName() const {
  return L"connectString";
}

XFA_Element CXFA_ConnectString::GetElementType() const {
  return XFA_Element::ConnectString;
}
