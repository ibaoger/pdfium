// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_passwordedit.h"

// static
bool CXFA_PasswordEdit::IsValidPacket(XFA_XDPPACKET packet) {
  return !!((XFA_XDPPACKET_Template | XFA_XDPPACKET_Form) & packet);
}

CXFA_PasswordEdit::CXFA_PasswordEdit(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::Node,
                XFA_Element::PasswordEdit,
                L"passwordEdit") {}

CXFA_PasswordEdit::~CXFA_PasswordEdit() {}

WideStringView CXFA_PasswordEdit::GetName() const {
  return L"passwordEdit";
}

XFA_Element CXFA_PasswordEdit::GetElementType() const {
  return XFA_Element::PasswordEdit;
}
