// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_encryptionmethod.h"

// static
bool CXFA_EncryptionMethod::IsValidPacket(XFA_XDPPACKET packet) {
  return !!((XFA_XDPPACKET_Template | XFA_XDPPACKET_Form) & packet);
}

CXFA_EncryptionMethod::CXFA_EncryptionMethod(CXFA_Document* doc,
                                             XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::NodeC,
                XFA_Element::EncryptionMethod,
                L"encryptionMethod") {}

CXFA_EncryptionMethod::~CXFA_EncryptionMethod() {}

WideStringView CXFA_EncryptionMethod::GetName() const {
  return L"encryptionMethod";
}

XFA_Element CXFA_EncryptionMethod::GetElementType() const {
  return XFA_Element::EncryptionMethod;
}
