// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_plaintextmetadata.h"

// static
bool CXFA_PlaintextMetadata::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_Config & packet);
}

CXFA_PlaintextMetadata::CXFA_PlaintextMetadata(CXFA_Document* doc,
                                               XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::PlaintextMetadata,
                L"plaintextMetadata") {}

CXFA_PlaintextMetadata::~CXFA_PlaintextMetadata() {}

WideStringView CXFA_PlaintextMetadata::GetName() const {
  return L"plaintextMetadata";
}

XFA_Element CXFA_PlaintextMetadata::GetElementType() const {
  return XFA_Element::PlaintextMetadata;
}
