// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_recordset.h"

// static
bool CXFA_RecordSet::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_SourceSet & packet);
}

CXFA_RecordSet::CXFA_RecordSet(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::Node,
                XFA_Element::RecordSet,
                L"recordSet") {}

CXFA_RecordSet::~CXFA_RecordSet() {}

WideStringView CXFA_RecordSet::GetName() const {
  return L"recordSet";
}

XFA_Element CXFA_RecordSet::GetElementType() const {
  return XFA_Element::RecordSet;
}
