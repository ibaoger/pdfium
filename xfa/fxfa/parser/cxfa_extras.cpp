// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_extras.h"

// static
bool CXFA_Extras::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(
      (XFA_XDPPACKET_SourceSet | XFA_XDPPACKET_Template | XFA_XDPPACKET_Form) &
      packet);
}

CXFA_Extras::CXFA_Extras(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::Node,
                XFA_Element::Extras,
                L"extras") {}

CXFA_Extras::~CXFA_Extras() {}

WideStringView CXFA_Extras::GetName() const {
  return L"extras";
}

XFA_Element CXFA_Extras::GetElementType() const {
  return XFA_Element::Extras;
}
