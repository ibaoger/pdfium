// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_modifyannots.h"

// static
bool CXFA_ModifyAnnots::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_Config & packet);
}

CXFA_ModifyAnnots::CXFA_ModifyAnnots(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::ModifyAnnots,
                L"modifyAnnots") {}

CXFA_ModifyAnnots::~CXFA_ModifyAnnots() {}

WideStringView CXFA_ModifyAnnots::GetName() const {
  return L"modifyAnnots";
}

XFA_Element CXFA_ModifyAnnots::GetElementType() const {
  return XFA_Element::ModifyAnnots;
}
