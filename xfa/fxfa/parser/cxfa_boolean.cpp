// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_boolean.h"

// static
bool CXFA_Boolean::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(
      (XFA_XDPPACKET_SourceSet | XFA_XDPPACKET_Template | XFA_XDPPACKET_Form) &
      packet);
}

CXFA_Boolean::CXFA_Boolean(CXFA_Document* doc, XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::Boolean,
                L"boolean") {}

CXFA_Boolean::~CXFA_Boolean() {}

WideStringView CXFA_Boolean::GetName() const {
  return L"boolean";
}

XFA_Element CXFA_Boolean::GetElementType() const {
  return XFA_Element::Boolean;
}
