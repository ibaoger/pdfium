// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_compressobjectstream.h"

// static
bool CXFA_CompressObjectStream::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_Config & packet);
}

CXFA_CompressObjectStream::CXFA_CompressObjectStream(CXFA_Document* doc,
                                                     XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::CompressObjectStream,
                L"compressObjectStream") {}

CXFA_CompressObjectStream::~CXFA_CompressObjectStream() {}

WideStringView CXFA_CompressObjectStream::GetName() const {
  return L"compressObjectStream";
}

XFA_Element CXFA_CompressObjectStream::GetElementType() const {
  return XFA_Element::CompressObjectStream;
}
