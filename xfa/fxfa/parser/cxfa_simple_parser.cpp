  ...
  if (wsTargetName == L"originalXFAVersion") {
    if (target_data.size() > 0 &&
        (pXFANode->GetDocument()->RecognizeXFAVersionNumber(target_data[0]) !=
         XFA_VERSION_UNKNOWN)) {
      if (target_data.size() > 1 && target_data[1] == L"v2.7-scripting:1")
        pXFANode->GetDocument()->SetFlag(XFA_DOCFLAG_Scripting, true);
    }
  } else if (wsTargetName == L"acrobat") {
    if (target_data.size() > 0 && target_data[0] == L"JavaScript") {
      if (target_data.size() > 1 && target_data[1] == L"strictScoping")
