// Copyright 2017 PDFium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Original code copyright 2014 Foxit Software Inc. http://www.foxitsoftware.com

#include "xfa/fxfa/parser/cxfa_behavioroverride.h"

// static
bool CXFA_BehaviorOverride::IsValidPacket(XFA_XDPPACKET packet) {
  return !!(XFA_XDPPACKET_Config & packet);
}

CXFA_BehaviorOverride::CXFA_BehaviorOverride(CXFA_Document* doc,
                                             XFA_XDPPACKET packet)
    : CXFA_Node(doc,
                packet,
                XFA_ObjectType::ContentNode,
                XFA_Element::BehaviorOverride,
                L"behaviorOverride") {}

CXFA_BehaviorOverride::~CXFA_BehaviorOverride() {}

WideStringView CXFA_BehaviorOverride::GetName() const {
  return L"behaviorOverride";
}

XFA_Element CXFA_BehaviorOverride::GetElementType() const {
  return XFA_Element::BehaviorOverride;
}
