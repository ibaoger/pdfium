...
TEST_F(FM2JSContextEmbedderTest, DISABLED_Round) {
  ...
  for (size_t i = 0; i < FX_ArraySize(tests); ++i) {
    ...
    EXPECT_FLOAT_EQ(tests[i].result, value->ToDouble())
        << "Program: " << tests[i].program;
  }
}
...
TEST_F(FM2JSContextEmbedderTest, PPmt) {
  ...
  struct {
    ...
    float result;
      ...
      {"PPmt(30000, 0.085, 295.50, 7, 3)", 261.6160716858},
...
TEST_F(FM2JSContextEmbedderTest, PV) {
  ...
  struct {
    ...
    float result;
      ...
      {"PV(400, 0.10 / 12, 30 * 12)", 45580.32799074439},
...
TEST_F(FM2JSContextEmbedderTest, UnitValue) {
  ...
  struct {
    ...
    float result;
      ...
      {"UnitValue(\"2in\")", 2}, {"UnitValue(\"2in\", \"cm\")", 5.08},
      // {"UnitValue(\"6\", \"pt\")", 432},
      // {"UnitType(\"A\", \"cm\")", 0},
...
TEST_F(FM2JSContextEmbedderTest, DISABLED_Parse) {
  ...
  CFXJSE_Value* value = GetValue();
  EXPECT_TRUE(value->IsNumber());
  EXPECT_FLOAT_EQ(1234567.89, value->ToFloat());
}
