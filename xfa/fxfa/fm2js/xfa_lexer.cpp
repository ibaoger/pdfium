...
void CXFA_FMLexer::Number(CXFA_FMToken* t,
                          const wchar_t* p,
                          const wchar_t*& pEnd) {
  double number = 0;
  // This will set pEnd to the character after the end of the number.
  if (p)
    number = wcstod(const_cast<wchar_t*>(p), const_cast<wchar_t**>(&pEnd));
...
void CXFA_FMLexer::String(CXFA_FMToken* t,
                          const wchar_t* p,
                          const wchar_t*& pEnd) {
...
void CXFA_FMLexer::Identifiers(CXFA_FMToken* t,
                               const wchar_t* p,
                               const wchar_t*& pEnd) {
  const wchar_t* pStart = p;
  uint16_t ch = 0;
