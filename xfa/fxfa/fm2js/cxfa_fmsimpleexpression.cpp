...
bool CXFA_FMSimpleExpression::ToJavaScript(CFX_WideTextBuf& javascript) {
  AutoRestorer<unsigned long> restorer(&CXFA_FMToJavaScriptDepth::depth_);
  return CXFA_FMToJavaScriptDepth::IncrementAndCheck();
}
...
bool CXFA_FMNullExpression::ToJavaScript(CFX_WideTextBuf& javascript) {
  AutoRestorer<unsigned long> restorer(&CXFA_FMToJavaScriptDepth::depth_);
  if (!CXFA_FMToJavaScriptDepth::IncrementAndCheck())
    return false;
