...
uint32_t CXFA_FMCallExpression::IsMethodWithObjParam(
  ...
  const XFA_FMSOMMethod* pEnd = gs_FMSomMethods + FX_ArraySize(gs_FMSomMethods);
  const XFA_FMSOMMethod* pMatchResult = std::lower_bound(
      gs_FMSomMethods, pEnd, methodName,
      [](const XFA_FMSOMMethod iter, const CFX_WideString& val) -> bool {
        return val.CompareNoCase(iter.m_wsSomMethodName) > 0;
      });
  if (pMatchResult < pEnd &&
      !methodName.CompareNoCase(pMatchResult->m_wsSomMethodName)) {
    parameters = pMatchResult->m_dParameters;
  }
  return parameters;
}
