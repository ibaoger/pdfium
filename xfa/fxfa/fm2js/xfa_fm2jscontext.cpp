...
void CXFA_FM2JSContext::Oneof(CFXJSE_Value* pThis,
  ...
  for (size_t i = 0; i < parameterValues.size(); i++) {
    if (simpleValueCompare(pThis, argOne.get(), parameterValues[i].get())) {
      bFlags = true;
      break;
    }
  }
...
void CXFA_FM2JSContext::unfoldArgs(
  ...
  for (int32_t i = 0; i < argc - iStart; i++) {
    if (argsValue[i]->IsArray()) {
      ...
      GetObjectDefaultValue(argsValue[i].get(), resultValues->at(index).get());
      index++;
    } else {
      resultValues->at(index)->Assign(argsValue[i].get());
      index++;
    }
  }
}
