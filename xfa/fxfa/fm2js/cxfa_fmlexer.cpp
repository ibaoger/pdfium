...
bool IsValidFormCalcCharacter(const wchar_t& c) {
  return c == 0 || (c >= 0x09 && c <= 0x0D) || (c >= 0x20 && c <= 0xd7FF) ||
         (c >= 0xE000 && c <= 0xFFFD);
}

bool IsValidIdentifierCharacter(const wchar_t& c) {
  return u_isalnum(c) || c == 0x005F ||  // '_'
         c == 0x0024;                    // '$'
}

bool IsValidInitialIdentifierCharacter(const wchar_t& c) {
...
CXFA_FMToken* CXFA_FMLexer::NextToken() {
  ...
  if (m_ptr > m_end) {
    ...
    m_pToken = ScanForNextToken();
  }
  return m_pToken.get();
}

std::unique_ptr<CXFA_FMToken> CXFA_FMLexer::ScanForNextToken() {
  ...
  while (1) {
    ...
    switch (*m_ptr) {
      ...
      case '"': {
        p->m_type = TOKstring;
        m_ptr = AdvanceForString(p.get(), m_ptr);
        return p;
      }
      case '0':
      case '1':
      ...
      case '9': {
        p->m_type = TOKnumber;
        m_ptr = AdvanceForNumber(p.get(), m_ptr);
        return p;
      }
      case '=':
        ++m_ptr;
      ...
      default: {
        ...
        m_ptr = AdvanceForIdentifier(p.get(), m_ptr);
        return p;
      }
    }
  }
...
const wchar_t* CXFA_FMLexer::AdvanceForString(CXFA_FMToken* t,
  ...
  while (p <= m_end) {
    ...
    if (*p == '"') {
      // Check for escaped "s, i.e. "". If the next legal character is not a ",
