...
void CXFA_FM2JSContext::Decode(CFXJSE_Value* pThis,
  ...
  if (argc == 1) {
    ...
    CFX_ByteStringC decoded = DecodeURL(toDecodeString.AsStringC());
    args.GetReturnValue()->SetString(decoded);
    return;
  }
  ...
  CFX_ByteStringC decoded;
  if (identifyString.EqualNoCase("html"))
    decoded = DecodeHTML(toDecodeString.AsStringC());
  else if (identifyString.EqualNoCase("xml"))
    decoded = DecodeXML(toDecodeString.AsStringC());
  else
    decoded = DecodeURL(toDecodeString.AsStringC());

  args.GetReturnValue()->SetString(decoded);
}

// static
CFX_ByteStringC CXFA_FM2JSContext::DecodeURL(
    const CFX_ByteStringC& szURLString) {
  ...
  return FX_UTF8Encode(wsResultBuf.AsStringC()).AsStringC();
}

// static
CFX_ByteStringC CXFA_FM2JSContext::DecodeHTML(
    const CFX_ByteStringC& szHTMLString) {
...
// static
CFX_ByteStringC CXFA_FM2JSContext::DecodeXML(
    const CFX_ByteStringC& szXMLString) {
  ...
  return FX_UTF8Encode(wsXMLBuf.AsStringC()).AsStringC();
}
...
void CXFA_FM2JSContext::Encode(CFXJSE_Value* pThis,
  ...
  if (argc == 1) {
    ...
    CFX_ByteStringC encoded = EncodeURL(toEncodeString.AsStringC());
    args.GetReturnValue()->SetString(encoded);
    return;
  }
  ...
  CFX_ByteString toEncodeString = ValueToUTF8String(argOne.get());
  CFX_ByteString identifyString = ValueToUTF8String(argTwo.get());
  CFX_ByteStringC encoded;
  if (identifyString.EqualNoCase("html"))
    encoded = EncodeHTML(toEncodeString.AsStringC());
  else if (identifyString.EqualNoCase("xml"))
    encoded = EncodeXML(toEncodeString.AsStringC());
  else
    encoded = EncodeURL(toEncodeString.AsStringC());

  args.GetReturnValue()->SetString(encoded);
}

// static
CFX_ByteStringC CXFA_FM2JSContext::EncodeURL(
    const CFX_ByteStringC& szURLString) {
  CFX_WideString wsURLString = CFX_WideString::FromUTF8(szURLString);
  CFX_WideTextBuf wsResultBuf;
  wchar_t strEncode[4];
...
// static
CFX_ByteStringC CXFA_FM2JSContext::EncodeHTML(
    const CFX_ByteStringC& szHTMLString) {
...
// static
CFX_ByteStringC CXFA_FM2JSContext::EncodeXML(
    const CFX_ByteStringC& szXMLString) {
  CFX_WideString wsXMLString = CFX_WideString::FromUTF8(szXMLString);
  CFX_WideTextBuf wsResultBuf;
  wchar_t strEncode[9];
...
void CXFA_FM2JSContext::Replace(CFXJSE_Value* pThis,
  ...
  for (int32_t u = 0; u < oneString.GetLength(); ++u) {
    char ch = oneString.GetCharAt(u);
    if (ch != twoString.GetCharAt(iFindIndex)) {
      resultString << ch;
      continue;
    }
    ...
    while (iFindIndex < iFindLen) {
      char chTemp = oneString.GetCharAt(iTemp);
      if (chTemp != twoString.GetCharAt(iFindIndex)) {
        iFindIndex = 0;
        break;
      }
...
void CXFA_FM2JSContext::Stuff(CFXJSE_Value* pThis,
  ...
  int32_t i = 0;
  while (i < iStart) {
    resultString << sourceString.GetCharAt(i);
    ++i;
  }
...
void CXFA_FM2JSContext::WordNum(CFXJSE_Value* pThis,
  ...
  CFX_ByteStringC stringC = WordUS(numberString.AsStringC(), iIdentifier);
  args.GetReturnValue()->SetString(stringC);
}

// static
void CXFA_FM2JSContext::TrillionUS(const CFX_ByteStringC& szData,
                                   std::ostringstream* pStrBuf) {
  ...
  if (iFirstCount == 3) {
    if (pData[iIndex] != '0') {
      *pStrBuf << pCapUnits[pData[iIndex] - '0'];
      *pStrBuf << pComm[0];
    }
    if (pData[iIndex + 1] == '0') {
      *pStrBuf << pCapUnits[pData[iIndex + 2] - '0'];
    } else {
      if (pData[iIndex + 1] > '1') {
        *pStrBuf << pLastTens[pData[iIndex + 1] - '2'];
        *pStrBuf << "-";
        *pStrBuf << pUnits[pData[iIndex + 2] - '0'];
      } else if (pData[iIndex + 1] == '1') {
        *pStrBuf << pTens[pData[iIndex + 2] - '0'];
    ...
    if (pData[iIndex] == '0') {
      *pStrBuf << pCapUnits[pData[iIndex + 1] - '0'];
    } else {
      if (pData[iIndex] > '1') {
        *pStrBuf << pLastTens[pData[iIndex] - '2'];
        *pStrBuf << "-";
        *pStrBuf << pUnits[pData[iIndex + 1] - '0'];
      } else if (pData[iIndex] == '1') {
        *pStrBuf << pTens[pData[iIndex + 1] - '0'];
...
// static
CFX_ByteStringC CXFA_FM2JSContext::WordUS(const CFX_ByteStringC& szData,
                                          int32_t iStyle) {
  const char* pData = szData.c_str();
  int32_t iLength = szData.GetLength();
  ...
  return CFX_ByteStringC(strBuf.str().c_str());
}
