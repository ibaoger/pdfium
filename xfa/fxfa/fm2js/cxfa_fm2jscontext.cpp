...
void CXFA_FM2JSContext::Decode(CFXJSE_Value* pThis,
  ...
  if (argc == 1) {
    ...
    CFX_ByteString decoded = DecodeURL(toDecodeWideString);
...
// static
CFX_ByteString CXFA_FM2JSContext::DecodeURL(const CFX_WideString& wsURLString) {
...
// static
CFX_ByteString CXFA_FM2JSContext::DecodeXML(const CFX_WideString& wsXMLString) {
  wchar_t strString[9];
  int32_t iStrIndex = 0;
...
void CXFA_FM2JSContext::Replace(CFXJSE_Value* pThis,
  ...
  for (int32_t u = 0; u < oneString.GetLength(); ++u) {
    ...
    while (iFindIndex < iFindLen) {
      char chTemp = static_cast<char>(oneString.GetAt(iTemp));
      if (chTemp != static_cast<char>(twoString.GetAt(iFindIndex))) {
        iFindIndex = 0;
        break;
      }
