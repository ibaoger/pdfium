...
bool CXFA_FFListBox::UpdateFWLData() {
  ...
  std::vector<CFWL_ListItem*> selItemArray;
  std::vector<int32_t> iSelArray = m_pDataAcc->GetSelectedItems();
  for (int32_t selected : iSelArray)
    selItemArray.push_back(pListBox->GetSelItem(selected));

  pListBox->SetSelItem(pListBox->GetSelItem(-1), false);
  for (CFWL_ListItem* pItem : selItemArray)
    static_cast<CFWL_ListBox*>(m_pNormalWidget)->SetSelItem(pItem, true);
...
bool CXFA_FFComboBox::UpdateFWLData() {
  ...
  std::vector<int32_t> iSelArray = m_pDataAcc->GetSelectedItems();
  if (!iSelArray.empty()) {
    static_cast<CFWL_ComboBox*>(m_pNormalWidget)->SetCurSel(iSelArray[0]);
  } else {
    CFX_WideString wsText;
    static_cast<CFWL_ComboBox*>(m_pNormalWidget)->SetCurSel(-1);
    m_pDataAcc->GetValue(wsText, XFA_VALUEPICTURE_Raw);
    static_cast<CFWL_ComboBox*>(m_pNormalWidget)->SetEditText(wsText);
  }
  m_pNormalWidget->Update();
  return true;
}
