...
const int32_t g_iBarcodeCount = FX_ArraySize(g_BarCodeData);

const BarCodeInfo* GetBarcodeTypeByName(const CFX_WideStringC& wsName) {
  if (wsName.IsEmpty())
    return nullptr;

  auto* it = std::lower_bound(
      g_BarCodeData, g_BarCodeData + g_iBarcodeCount,
      FX_HashCode_GetW(wsName, true),
      [](const BarCodeInfo& arg, uint32_t hash) { return arg.uHash < hash; });

  if (it != g_BarCodeData + g_iBarcodeCount && wsName == it->pName)
    return it;
...
void CXFA_FFBarcode::UpdateWidgetProperty() {
  ...
  if (pAcc->GetBarcodeAttribute_WideNarrowRatio(floatVal))
    pBarCodeWidget->SetWideNarrowRatio(static_cast<int32_t>(floatVal));
