...
bool TextAlignmentCentered(const FDE_TextAlignment align) {
...
void CFDE_TextOut::CalcLogicSize(const wchar_t* pwsStr,
  ...
  if (TextAlignmentCentered(m_iAlignment))
    fInc /= 2.0f;
  else if (m_iAlignment == FDE_TextAlignment::kTopLeft)
    fInc = 0.0f;
  ...
  rect.height = fHeight;
  if (m_dwStyles.last_line_height_)
...
bool CFDE_TextOut::RetrieveLineWidth(CFX_BreakType dwBreakStatus,
  ...
  if (!m_dwStyles.line_wrap_ && dwBreakStatus == CFX_BreakType::Line) {
    fWidth += fLineWidth;
  } else {
  ...
  while (iTxtLength-- > 0) {
    ...
    *pStr++ = wch;
    ...
    if (!CFX_BreakTypeNoneOrPiece(dwBreakStatus)) {
      ...
      if (bEndofLine &&
          (m_dwStyles.line_wrap_ || dwBreakStatus == CFX_BreakType::Paragraph ||
...
bool CFDE_TextOut::RetrievePieces(CFX_BreakType dwBreakStatus,
  ...
  for (int32_t i = 0; i < iCount; i++) {
    ...
    for (; j < iPieceChars; j++) {
      ...
      if (m_dwStyles.single_line_ || !m_dwStyles.line_wrap_) {
  ...
  m_pTxtBreak->ClearBreakPieces();
  return m_dwStyles.single_line_ || m_dwStyles.line_wrap_ || bNeedReload ||
