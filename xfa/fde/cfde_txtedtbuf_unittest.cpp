...
TEST_F(CFDE_TxtEdtBufTest, SetTextLessThenChunkSize) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(2), res.GetLength());
  EXPECT_EQ(L"Hi", res);
}
...
TEST_F(CFDE_TxtEdtBufTest, InsertAppendChunk) {
  ...
  CFX_WideString res = buf_->GetText();
  EXPECT_EQ(static_cast<FX_STRSIZE>(8), res.GetLength());
...
TEST_F(CFDE_TxtEdtBufTest, InsertBetweenChunks) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(17), res.GetLength());
}
...
TEST_F(CFDE_TxtEdtBufTest, DeleteMiddleText) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(11), res.GetLength());
}
...
TEST_F(CFDE_TxtEdtBufTest, DeleteEndText) {
  ...
  EXPECT_EQ(static_cast<FX_STRSIZE>(5), res.GetLength());
}
...
TEST_F(CFDE_TxtEdtBufTest, DeleteAllText) {
  ...
  CFX_WideString res = buf_->GetText();
  EXPECT_EQ(L"", res);
  EXPECT_EQ(static_cast<FX_STRSIZE>(0), res.GetLength());
}
