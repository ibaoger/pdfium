...
int32_t CFDE_TxtEdtPage::LoadPage(const CFX_RectF* pClipBox,
  ...
  do {
    ...
    if (pIter->GetAt() == nPageEnd && (dwBreakStatus == CFX_BreakType::None ||
                                       dwBreakStatus == CFX_BreakType::Piece)) {
      dwBreakStatus = pBreak->EndBreak(CFX_BreakType::Paragraph);
    }
    if (dwBreakStatus != CFX_BreakType::None &&
        dwBreakStatus != CFX_BreakType::Piece) {
      ...
      for (int32_t j = 0; j < nPieceCount; j++) {
        ...
        if (pPiece->m_dwStatus != CFX_BreakType::None &&
            pPiece->m_dwStatus != CFX_BreakType::Piece) {
