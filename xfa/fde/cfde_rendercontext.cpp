...
void CFDE_RenderContext::Render(CFDE_TxtEdtPage* pCanvasSet,
  ...
  for (size_t i = 0; i < pCanvasSet->GetTextPieceCount(); ++i) {
    const FDE_TEXTEDITPIECE& pObj = pCanvasSet->GetTextPiece(i);
    if (!rtDocClip.IntersectWith(pVisualSet->GetRect(pObj)))
      continue;

    int32_t iCount = pVisualSet->GetDisplayPos(pObj, nullptr, false);
    if (iCount < 1)
      return;
    if (char_pos.size() < static_cast<size_t>(iCount))
      char_pos.resize(iCount, FXTEXT_CHARPOS());

    iCount = pVisualSet->GetDisplayPos(pObj, char_pos.data(), false);
