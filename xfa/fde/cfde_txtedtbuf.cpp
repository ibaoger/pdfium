...
CFDE_TxtEdtBuf::CFDE_TxtEdtBuf() : m_nTotal(0) {
  m_chunks.push_back(pdfium::MakeUnique<ChunkHeader>());
  m_chunks.back()->nUsed = 0;
}
...
void CFDE_TxtEdtBuf::SetText(const CFX_WideString& wsText) {
  ...
  int32_t nNeedCount =
      ((nTextLength - 1) / kDefaultChunkSize + 1) - m_chunks.size();
  int32_t i = 0;
  ...
  int32_t nCopyedLength = kDefaultChunkSize;
  for (i = 0; i < nTotalCount && nLeave > 0; i++) {
    ...
    FXSYS_memcpy(chunk->wChars, lpSrcBuf, nCopyedLength * sizeof(FX_WCHAR));
...
CFX_WideString CFDE_TxtEdtBuf::GetRange(int32_t nBegin, int32_t nLength) const {
  ...
  FX_WCHAR* lpSrcBuf = chunkHeader->wChars + charIndex;
  while (nLeave > 0) {
...
void CFDE_TxtEdtBuf::Insert(int32_t nPos,
  ...
  if (charIndex != 0) {
    auto newChunk = pdfium::MakeUnique<ChunkHeader>();

    ChunkHeader* chunk = m_chunks[chunkIndex].get();
    int32_t nCopy = chunk->nUsed - charIndex;
    FXSYS_memcpy(newChunk->wChars, chunk->wChars + charIndex,
  ...
  if (chunkIndex != 0) {
    ...
    if (chunk->nUsed != kDefaultChunkSize) {
      chunkIndex--;
      int32_t nFree = kDefaultChunkSize - chunk->nUsed;
      int32_t nCopy = std::min(nLengthTemp, nFree);
      FXSYS_memcpy(chunk->wChars + chunk->nUsed, lpText,
                   nCopy * sizeof(FX_WCHAR));
      lpText += nCopy;
      nLengthTemp -= nCopy;
      chunk->nUsed += nCopy;
  ...
  while (nLengthTemp > 0) {
    ...
    int32_t nCopy = std::min(nLengthTemp, kDefaultChunkSize);
    FXSYS_memcpy(chunk->wChars, lpText, nCopy * sizeof(FX_WCHAR));
    lpText += nCopy;
    nLengthTemp -= nCopy;
    chunk->nUsed = nCopy;
...
void CFDE_TxtEdtBuf::Delete(int32_t nIndex, int32_t nLength) {
  ...
  if (nMovePart != 0) {
    ...
    FXSYS_memmove(chunk->wChars + nFirstPart - nDelete,
                  chunk->wChars + nFirstPart, nMovePart * sizeof(FX_WCHAR));
    chunk->nUsed -= nDelete;
    nLength -= nDelete;
    endChunkIndex--;
  }
...
std::tuple<int32_t, int32_t> CFDE_TxtEdtBuf::Index2CP(int32_t nIndex) const {
  ...
  if (nIndex == m_nTotal) {
    ...
    charIndex = m_chunks[chunkIndex]->nUsed;
  ...
  chunkIndex = i;
  charIndex = m_chunks[i]->nUsed - (nTotal - nIndex);
  return std::tuple<int32_t, int32_t>(chunkIndex, charIndex);
}
