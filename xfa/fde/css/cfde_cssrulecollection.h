...
class CFDE_CSSRuleCollection {
 public:
  ...
  const std::vector<std::unique_ptr<Data>>* GetTagRuleData(uint32_t dwTagHash) {
  ...
  const std::vector<std::unique_ptr<Data>>* GetUniversalRuleData() {
    return &m_pUniversalRules;
  }
  const std::vector<std::unique_ptr<Data>>* GetPseudoRuleData() {
 ...
 protected:
  void AddRulesFrom(CFDE_CSSStyleSheet* pStyleSheet,
                    ...
                    uint32_t dwMediaList,
                    CFGAS_FontMgr* pFontMgr);
  void AddRuleTo(std::map<uint32_t, std::vector<std::unique_ptr<Data>>>& pMap,
                 uint32_t dwKey,
