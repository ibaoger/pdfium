...
void CFDE_CSSRuleCollection::AddRuleTo(
    std::map<uint32_t, std::vector<std::unique_ptr<Data>>>& pMap,
    uint32_t dwKey,
    CFDE_CSSSelector* pSel,
    CFDE_CSSDeclaration* pDecl) {
  auto it = pMap.find(dwKey);
  if (it != pMap.end()) {
    it->second.push_back(NewRuleData(pSel, pDecl));
  } else {
    std::vector<std::unique_ptr<Data>> list;
    list.push_back(NewRuleData(pSel, pDecl));
    pMap[dwKey] = std::move(list);
  }
}
