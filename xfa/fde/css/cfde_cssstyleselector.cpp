...
bool CFDE_CSSStyleSelector::MatchSelector(const CFX_WideString& tagname,
  ...
  uint32_t taghash = FX_HashCode_GetW(tagname.c_str(), true);
  return pSel->GetNameHash() == taghash;
}

CFX_RetainPtr<CFDE_CSSComputedStyle> CFDE_CSSStyleSelector::ComputeStyle(
  ...
  return ApplyDeclarations(declArray, pDecl.get());
}

CFX_RetainPtr<CFDE_CSSComputedStyle> CFDE_CSSStyleSelector::ApplyDeclarations(
    const std::vector<const CFDE_CSSDeclaration*>& declArray,
    const CFDE_CSSDeclaration* extraDecl) {
  auto pComputedStyle = pdfium::MakeRetain<CFDE_CSSComputedStyle>();
  ...
  for (auto& prop : normals)
    ApplyProperty(prop->eProperty, prop->pValue.Get(), pComputedStyle.Get());
