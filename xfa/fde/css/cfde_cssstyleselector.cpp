...
void CFDE_CSSStyleSelector::MatchDeclarations(
  ...
  for (const auto& rule : matchedRules)
    matchedDecls->push_back(rule->pDeclaration);
}
...
void CFDE_CSSStyleSelector::ExtractValues(
  ...
  for (auto it = decl->begin(); it != decl->end(); it++) {
    const CFDE_CSSPropertyHolder* holder = it->get();
    if (holder->bImportant)
      importants->push_back(holder);
    else
      normals->push_back(holder);
  }
