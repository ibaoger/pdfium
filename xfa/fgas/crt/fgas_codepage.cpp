...
const charSET_MAP g_FXCharset2CodePageTable[] = {
...
uint16_t FX_GetCodePageFromCharset(uint8_t charset) {
  int32_t iEnd = sizeof(g_FXCharset2CodePageTable) / sizeof(charSET_MAP) - 1;
  ASSERT(iEnd >= 0);
  int32_t iStart = 0, iMid;
  do {
    iMid = (iStart + iEnd) / 2;
    const charSET_MAP& cp = g_FXCharset2CodePageTable[iMid];
