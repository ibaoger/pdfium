...
class IFGAS_StreamImp {
 public:
  ...
  virtual void WriteData(const CFX_WideString& str) = 0;
...
class CFGAS_FileReadStreamImp : public IFGAS_StreamImp {
 public:
  ...
  void WriteData(const CFX_WideString& str) override {}
...
class CFGAS_TextStream : public IFGAS_Stream {
 public:
  ...
  FX_STRSIZE ReadString(wchar_t* pStr,
                        ...
                        bool* bEOS) override;
  void WriteString(const CFX_WideString& str) override;
...
void CFGAS_FileWriteStreamImp::WriteData(const CFX_WideString& str) {
  if (str.GetLength() == 0)
    return;
...
FX_STRSIZE CFGAS_TextStream::ReadString(wchar_t* pStr,
  ...
  if (m_wCodePage == FX_CODEPAGE_UTF16LE ||
    ...
    FX_STRSIZE iLen =
        m_pStreamImp->ReadData(static_cast<uint8_t*>(pStr), iBytes);
    iMaxLength = iLen / 2;
    if (sizeof(wchar_t) > 2)
...
void CFGAS_TextStream::WriteString(const CFX_WideString& str) {
  if (!m_IsWriteStream)
    return;
  if (str.GetLength() == 0)
    return;
