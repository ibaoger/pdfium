...
bool CFGAS_GEFont::GetCharWidth(wchar_t wUnicode, int32_t& iWidth) {
  ...
  if (!m_pProvider || !m_pProvider->GetCharWidth(RetainPtr<CFGAS_GEFont>(this),
    ...
    int32_t iGlyph;
    std::tie(iGlyph, pFont) = GetGlyphIndex(wUnicode, true);
...
int32_t CFGAS_GEFont::GetGlyphIndex(wchar_t wUnicode) {
  ...
  std::tie(glyph, font) = GetGlyphIndex(wUnicode, true);
  return glyph;
}

std::pair<int32_t, RetainPtr<CFGAS_GEFont>> CFGAS_GEFont::GetGlyphIndex(
    wchar_t wUnicode,
    bool bRecursive) {
  ...
  if (it != m_FontMapper.end() && it->second && it->second.Get() != this) {
    RetainPtr<CFGAS_GEFont> font;
    std::tie(iGlyphIndex, font) = it->second->GetGlyphIndex(wUnicode, false);
  ...
  RetainPtr<CFGAS_GEFont> font;
  std::tie(iGlyphIndex, font) = pFont->GetGlyphIndex(wUnicode, false);
  if (iGlyphIndex == 0xFFFF)
    return {0xFFFF, nullptr};
