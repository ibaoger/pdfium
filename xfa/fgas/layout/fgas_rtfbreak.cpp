...
CFX_RTFBreakType CFX_RTFBreak::AppendChar_Arabic(CFX_RTFChar* pCurChar) {
  ...
  if (m_eCharType >= FX_CHARTYPE_ArabicAlef &&
    ...
    if (pLastChar) {
      m_Lines[m_current_line].m_iWidth -= pLastChar->m_iCharWidth;
...
bool CFX_RTFBreak::EndBreak_SplitLine(size_t next_line,
  ...
  int32_t iLast = m_Lines[m_current_line].CountChars() - 1;
  int32_t j = 0;
...
void CFX_RTFBreak::EndBreak_BidiLine(CFX_TPOArray* tpos,
  ...
  int32_t iCount = m_Lines[m_current_line].CountChars();
  if (!m_bPagination && m_Lines[m_current_line].m_iArabicChars > 0) {
    int32_t iBidiNum = 0;
    for (i = 0; i < iCount; i++) {
      pTC = &chars[i];
      pTC->m_iBidiPos = i;
...
void CFX_RTFBreak::EndBreak_Alignment(const CFX_TPOArray& tpos,
                                      ...
                                      CFX_RTFBreakType dwStatus) {
  CFX_RTFPieceArray* pCurPieces = &m_Lines[m_current_line].m_LinePieces;
  int32_t iNetWidth = m_Lines[m_current_line].m_iWidth;
  int32_t iGapChars = 0;
  int32_t iCharWidth;
  int32_t iCount = pCurPieces->GetSize();
  bool bFind = false;
...
void CFX_RTFBreak::SplitTextLine(size_t cur_line,
  ...
  for (auto& ch : m_Lines[next_line].m_LineChars) {
    if (ch.GetCharType() >= FX_CHARTYPE_ArabicAlef) {
      m_Lines[cur_line].m_iArabicChars--;
      m_Lines[next_line].m_iArabicChars++;
    }
    ch.m_dwStatus = CFX_RTFBreakType::None;
  }
}
    ...
    if (pCharPos) {
      ...
      if (dwCharType >= FX_CHARTYPE_ArabicAlef) {
        if (i < iLength) {
          ...
          if (pText->pWidths[cur_width_idx] < 0 && i + 1 < iLength)
