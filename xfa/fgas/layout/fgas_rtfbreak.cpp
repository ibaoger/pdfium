...
void CFX_RTFBreak::EndBreak_Alignment(const std::deque<FX_TPO>& tpos,
  ...
  int32_t iGapChars = 0;
  bool bFind = false;
  int32_t iCount = pdfium::CollectionSize<int32_t>(m_pCurLine->m_LinePieces);
  ...
  if (iGapChars > 0 && (m_iAlignment == CFX_RTFLineAlignment::Distributed ||
    ...
    if (iOffset > 0) {
      for (int32_t i = 0; i < iCount; ++i) {
        CFX_RTFPiece& ttp = m_pCurLine->m_LinePieces[i];
...
void CFX_RTFBreak::Reset() {
  m_eCharType = FX_CHARTYPE_Unknown;
  m_RTFLine[0].RemoveAll();
  m_RTFLine[1].RemoveAll();
}
...
CFX_RTFLine::CFX_RTFLine()
    : m_iStart(0), m_iWidth(0), m_iArabicChars(0), m_iMBCSChars(0) {}
