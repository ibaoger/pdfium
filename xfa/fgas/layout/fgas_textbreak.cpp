...
CFX_BreakType CFX_TxtBreak::EndBreak(CFX_BreakType dwStatus) {
  ...
  if (HasTxtLine()) {
    if (!m_TxtLine[m_iReady].m_LinePieces.empty()) {
      if (dwStatus != CFX_BreakType::Piece)
        m_TxtLine[m_iReady].m_LinePieces.back().m_dwStatus = dwStatus;
      return m_TxtLine[m_iReady].m_LinePieces.back().m_dwStatus;
    }
    return CFX_BreakType::None;
  }
  ...
  m_iReady = m_pCurLine == &m_TxtLine[0] ? 0 : 1;
  CFX_TxtLine* pNextLine = &m_TxtLine[1 - m_iReady];
...
int32_t CFX_TxtBreak::CountBreakPieces() const {
  return HasTxtLine()
             ? pdfium::CollectionSize<int32_t>(m_TxtLine[m_iReady].m_LinePieces)
             : 0;
}
...
const CFX_TxtPiece* CFX_TxtBreak::GetBreakPiece(int32_t index) const {
  ...
  if (index < 0 ||
      index >=
          pdfium::CollectionSize<int32_t>(m_TxtLine[m_iReady].m_LinePieces))
    return nullptr;
  return &m_TxtLine[m_iReady].m_LinePieces[index];
}

void CFX_TxtBreak::ClearBreakPieces() {
  if (HasTxtLine())
    m_TxtLine[m_iReady].Clear();
  m_iReady = -1;
}
