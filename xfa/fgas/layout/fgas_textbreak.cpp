...
CFX_BreakType CFX_TxtBreak::EndBreak(CFX_BreakType dwStatus) {
  ...
  if (iCount > 0) {
    ...
    if (HasTxtLine()) {
      pCurPieces = &m_TxtLine[m_iReady].m_LinePieces;
...
int32_t CFX_TxtBreak::CountBreakPieces() const {
  return HasTxtLine() ? m_TxtLine[m_iReady].m_LinePieces.GetSize() : 0;
}

const CFX_TxtPiece* CFX_TxtBreak::GetBreakPiece(int32_t index) const {
  if (!HasTxtLine())
    return nullptr;
  if (index < 0 || index >= m_TxtLine[m_iReady].m_LinePieces.GetSize())
    return nullptr;
  return m_TxtLine[m_iReady].m_LinePieces.GetPtrAt(index);
}

void CFX_TxtBreak::ClearBreakPieces() {
  if (HasTxtLine())
    m_TxtLine[m_iReady].RemoveAll(true);
  m_iReady = -1;
}
