...
void NupState::calculate_edit(int subx,
                              int suby,
                              NupPageEdit& ret) const  // {{{
{
  ...
  const float scalex = width / ret.sub.width, scaley = height / ret.sub.height;
  float subwidth = ret.sub.width * scalex, subheight = ret.sub.height * scaley;
...
bool NupState::nextPage(float in_width,
                        float in_height,
                        NupPageEdit& ret)  // {{{
{
  in_pages++;
...
static std::pair<Axis, Position> parsePosition(
    char a,
