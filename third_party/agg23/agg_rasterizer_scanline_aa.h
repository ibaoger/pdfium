...
class rasterizer_scanline_aa {
  enum status { status_initial, status_line_to, status_closed };
 ...
 public:
  ...
  AGG_INLINE void sort() { m_outline.sort_cells(); }
  ...
  bool sweep_scanline(Scanline& sl, bool no_smooth) {
    for (;;) {
  ...
  template <class VertexSource>
  void add_path_transformed(VertexSource& vs,
 ...
 private:
  ...
  void clip_segment(int x, int y) {
    ...
    if (m_prev_flags == flags) {
      ...
      int cy[4];
      unsigned n =
          clip_liang_barsky(m_prev_x, m_prev_y, x, y, m_clip_box, cx, cy);
      const int* px = cx;
      const int* py = cy;
      while (n--) {
        if (m_status == status_initial) {
