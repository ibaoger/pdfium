...
{
    if(nb >= m_max_blocks) {
      float** new_coords = FX_Alloc2D(float*, m_max_blocks + block_pool, 2);
      unsigned char** new_cmds =
          (unsigned char**)(new_coords + m_max_blocks + block_pool);
      if (m_coord_blocks) {
        FXSYS_memcpy(new_coords, m_coord_blocks, m_max_blocks * sizeof(float*));
        FXSYS_memcpy(new_cmds, m_cmd_blocks,
    ...
    m_coord_blocks[nb] = FX_Alloc(
        float,
        block_size * 2 + block_size / (sizeof(float) / sizeof(unsigned char)));
    m_cmd_blocks[nb]  =
...
void path_storage::curve4(float x_ctrl1,
                          float y_ctrl1,
                          float x_ctrl2,
                          float y_ctrl2,
                          float x_to,
