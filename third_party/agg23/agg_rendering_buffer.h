...
class rendering_buffer {
 public:
  ...
  int8u* row(unsigned y) { return m_rows[y]; }
  const int8u* row(unsigned y) const { return m_rows[y]; }
  int8u* next_row(void* p) { return (int8u*)p + m_stride; }
  const int8u* next_row(const void* p) const { return (int8u*)p + m_stride; }
  int8u const* const* rows() const { return m_rows; }
