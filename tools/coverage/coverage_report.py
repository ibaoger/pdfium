...
# Values are tuples of the actual binary to be run and a boolean indicating if
# this binary uses test_runner.py
COVERAGE_TESTS = {
    'pdfium_unittests': ('pdfium_unittests', False),
    'pdfium_embeddertests': ('pdfium_embeddertests', False),
    'corpus_tests': ('run_corpus_tests.py', True),
    'javascript_tests': ('run_javascript_tests.py', True),
    'pixel_tests': ('run_pixel_tests.py', True),
}
...
class CoverageExecutor(object):
  ...
  def find_acceptable_binary(self, binary_name, version_regex,
    ...
    found_binary = None
    found_major_version = 0
    found_minor_version = 0
    ...
    for binary in potential_binaries:
      ...
      if self.verbose:
        print 'Found version to be %d.%d' % (parsed_major_version,
                                             parsed_minor_version)
      if parsed_major_version > found_major_version or (
          parsed_major_version == found_major_version and
          parsed_minor_version > found_minor_version):
        if self.verbose:
          print 'Found new highest version number'
        found_binary = binary
        found_major_version = parsed_major_version
        found_minor_version = parsed_minor_version

    if found_major_version < min_major_version or \
        (found_major_version == min_major_version and
             found_minor_version < min_minor_version):
      if self.verbose:
        print('Highest version of %s discovered is %d.%d, which lower '
              'then the minimum %d.%d') % (binary_name, found_major_version,
                                           found_minor_version,
                                           min_major_version, min_minor_version)
      found_binary = None
  ...
  def generate_coverage(self, name, test_spec):
    ...
    specifications."""
    if self.verbose:
      print "Generating coverage for test '%s', using data '%s'" % (name,
                                                                    test_spec)
    (binary, use_test_runner) = test_spec
