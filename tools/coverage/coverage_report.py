...
class CoverageExecutor(object):
  ...
  def find_acceptable_binary(self, binary_name, version_regex,
    ...
    for binary in potential_binaries:
      ...
      # lcov and llvm-cov
      try:
        version_output = self.check_output([binary,
                                               '--version']).splitlines()
      ...
      if self.verbose:
        print 'Found version to be %d.%d' % (parsed_version[0],
                                             parsed_version[1])
      if parsed_version > found_version:
        if self.verbose:
          print 'Found new highest version number'
        found_binary = binary
        found_version = parsed_version

    if found_version < (min_major_version,  min_minor_version):
      if self.verbose:
        print('Highest version of %s discovered is %d.%d, which lower '
              'then the minimum %d.%d') % (binary_name,
                                           found_version[0],
                                           found_version[1],
                                           min_major_version, min_minor_version)
      found_binary = None
  ...
  def generate_coverage(self, name, spec):
    ...
    Args:
        name: Name associate with the test to be run. This is used as a label in
              the coverage data, so should be unique across all of the tests
              being run.
