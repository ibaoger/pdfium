...
class CoverageExecutor(object):
  ...
  def checkout_output(self, args, dry_run=False, env=None):
    """Wrapper for the check_output command that allows specifying if this is a
    dry run or not."""
    if dry_run:
  ...
  def call(self, args, dry_run=False, env=None):
    """Wrapper for the call command that allows specifying if this is a dry run
    or not."""
    if dry_run:
      print "Would have run '{}'".format(' '.join(args))
      return 0
  ...
  def calculate_coverage_tests(self, args):
    """Based on the arguments passed in, determine which tests are going to be
    run."""
    testing_tools_directory = os.path.join(self.source_directory, 'testing',
                                           'tools')
    # Values are tuples of the path to test binary and a boolean indicating if
    # this binary uses test_runner.py
    coverage_tests = {
  ...
  def determine_proper_llvm_cov(self):
    ...
    # passed in as single string instead of as separate list members.
    ...
    for binary in potential_binaries:
      ...
      # Older versions of llvm-cov return 1 on success for the --version
      # command, which causes Python to thinkit exited with a failure status.
      # Conveniently these versions also don't support the gcov command, so they
      ...
      # Technically the version is in x.y.z format, but only the x.y versions
      # matter for determining compatibility.
    ...
    # gcov command was implemented in 3.5, which is used for generating data
    # for lcov
  ...
  def determine_proper_lcov(self):
    ...
    for binary in potential_binaries:
      ...
      # Skipping binraries that fail on --version, since lcov should always
      # succeed.
      try:
        version_output = self.checkout_output([binary,
                                               '--version']).splitlines()
    ...
    # lcov 1.10 and earlier pass in problematic flags to the gcov tool being
    # used.
  ...
  def generate_coverage(self, name, test_spec):
    """Generate the coverage data for a test with the given name
    specifications."""
    if self.verbose:
    ...
    binary_args = [binary]
    if use_test_runner:
      # Test runner tests multi-thread in the wrapper script, not the test
      # binary, so need -j 1, otherwise multiple processes will be witting to
    ...
    if self.call(binary_args, dry_run=self.dry_run) and self.verbose:
      print ('Running {} appears to have failed, which might affect '
            'results').format(binary)
...
def main():
  ...
  parser.add_argument(
      ...
      '{} under CWD'.format(build_default),
  ...
  parser.add_argument(
      '--slow',
      help=
      'Run all tests, even those known to take a long time. Ignored if '
      'specific tests are passed in.',
      action='store_true')
  parser.add_argument(
      'tests',
      help=
      "Tests to be run, defaults to all. Valid entries are "
      "'pdfium_unittests', 'pdfium_embeddertests', 'corpus_tests', "
      "'javascript_tests', and 'pixel_tests'",
      nargs='*')
